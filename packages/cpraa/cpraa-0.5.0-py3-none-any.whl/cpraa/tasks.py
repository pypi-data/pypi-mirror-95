...
def get_one_distribution_linear(ls: LinearSolver, semantics: List[Semantics], solver="cvxopt") -> Distribution:
    ...
    return ls.any_solution(constraints, solver=solver)
...
def check_credulous_acceptance(z3i: Z3Instance, semantics: List[Semantics], argument: Node, threshold: float = 1):
    """
...
def check_skeptical_acceptance(z3i: Z3Instance, semantics: List[Semantics], argument: Node, threshold: float = 1):
    """
...
def maximize_marginal_probabilities(ls: LinearSolver, nodes: List[Node], semantics: List[Semantics], solver=None):
    """
    ...
    """
    constraints = ls_constraints(ls, semantics)
    return ls.optimize_marginal_prob(nodes, constraints, mode="max", solver=solver)


def minimize_marginal_probabilities(ls: LinearSolver, nodes: List[Node], semantics: List[Semantics], solver=None):
    """
    Compute the optimal distribution which minimizes the marginal probability of the given arguments while observing
    the given semantics' constraints.
    """
    constraints = ls_constraints(ls, semantics)
    return ls.optimize_marginal_prob(nodes, constraints, mode="min", solver=solver)
