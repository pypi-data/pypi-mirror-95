...
class CF(Semantics):
    ...
    def generate_linear_constraints(self, ls: LinearSolver):
        for edge in self.af.get_edges():
            assignment: List[Tuple[Node, bool]] = [(edge.node_from, True), (edge.node_to, True)]
            full_assignments = asg.sum_out(tuple(assignment), self.all_nodes)
...
class PrAdm(Semantics):
    ...
    def generate_linear_constraints(self, ls: LinearSolver):
        ...
        for node in self.all_nodes:
            ...
            if node_parents:
                attacker_assignment: List[Tuple[Node, bool]] = [(attacker, False) for attacker in node_parents]
                attacker_full_assignments = asg.sum_out(tuple(attacker_assignment), self.all_nodes)
            ...
            if node_children:
                attackee_assignment: List[Tuple[Node, bool]] = [(attackee, False) for attackee in node_children]
                attackee_full_assignments = asg.sum_out(tuple(attackee_assignment), self.all_nodes)
...
class PrCmp(Semantics):
    ...
    def generate_z3_constraints(self, z3i: Z3Instance):
        ...
        self.constraints_z3.extend(prCmp_z3_constraints(self.all_nodes, z3i))
