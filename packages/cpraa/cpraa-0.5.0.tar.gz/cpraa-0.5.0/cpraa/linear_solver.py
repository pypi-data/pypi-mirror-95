...
class LinearSolver:
    ...
    def any_solution(self, constraints: List[Constraint], solver=DEFAULT_SOLVER):
        c = [0] * self.dimension
        return self.optimal_distribution(c, constraints, solver)
    ...
    def optimal_distribution(self, cost_vector, constraints: List[Constraint], solver=DEFAULT_SOLVER):
        assert len(cost_vector) == self.dimension
        solver = config_solver(solver)
        ...
        t = Timer(output_string=solver + " solver took")
        # x = pypoman.solve_lp(c, A, b, solver=solver)
        result = cvxopt.solvers.lp(c, A, b, solver=solver)
        t.stop()
    ...
    def optimize_marginal_prob(self, nodes: List[Node], constraints: List[Constraint], mode="min",
                               solver=DEFAULT_SOLVER) -> Distribution:
        """
        ...
        :param solver: optional: the back-end solver to use
        ...
        return self.optimal_distribution(cost_vector, constraints, solver)
