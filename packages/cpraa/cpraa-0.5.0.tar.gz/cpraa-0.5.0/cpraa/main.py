...
#  - use pysmt (https://github.com/pysmt/pysmt) instead of directly interacting with z3
#
#  - Flag for extensions
...
def main():
    ...
    base_group.add_argument("-cs", "--complement_semantics", nargs="+", default=[], metavar="SEMANTICS",
                            help="probabilistic semantics with each constraint negated")
    ...
    base_group.add_argument("-b", "--backend_solver", metavar="SOLVER", type=str, default="Z3",
                            help="the solver used as backend (not all solvers can perform all tasks): Z3 (default), "
                                 "CVXOPT, GLPK, MOSEK (commercial solver)")
    ...
    semantics = []
    arguments = None
    solver = args.backend_solver.lower()  # 'z3', 'cvxopt', 'glpk' or 'mosek'
    ...
    if args.one_distribution:
        ...
        elif solver == "cvxopt" or solver == "mosek" or solver == "glpk":
            distribution = tasks.get_one_distribution_linear(ls, semantics, solver)
        else:
            arg_parser.error("unknown solver '" + solver + "'. Valid options are Z3, CVXOPT, GLPK or MOSEK.")
    ...
    if args.minimize_probability:
        ...
        distribution = tasks.minimize_marginal_probabilities(ls, arguments, semantics, solver=solver)
        if distribution:
    ...
    if args.maximize_probability:
        ...
        print("Computing optimal distribution maximizing the marginal probabilities of", repr_list(args.arguments),
              "while satisfying the following semantics:", repr_list(semantics))
        distribution = tasks.maximize_marginal_probabilities(ls, arguments, semantics, solver=solver)
        if distribution:
            distribution.print_formatted(args.distribution_output_format)
        else:
            print("No distribution satisfying the given semantics exists.")
    ...
    if args.credulous_acceptance is not None:
        ...
        if solver != "z3":
            arg_parser.error("credulous acceptance checking only available via Z3 solver")
        ...
        for argument in arguments:
            ...
            (acceptable, distribution) = tasks.check_credulous_acceptance(z3i, semantics, argument, threshold)
            if acceptable:
    ...
    if args.skeptical_acceptance is not None:
        ...
        if not arguments:
            arg_parser.error("no arguments given to check acceptance, provide one or more with -a")
        if solver != "z3":
            arg_parser.error("skeptical acceptance checking only available via Z3 solver")
        ...
        for argument in arguments:
            print("Checking skeptical acceptance of argument", argument, "with threshold", threshold,
                  "under the following semantics:", repr_list(semantics))
            (acceptable, counterexample) = tasks.check_skeptical_acceptance(z3i, semantics, argument, threshold)
            if acceptable:
    ...
    if args.credulous_acceptance_all is not None:
        ...
        for argument in af.get_nodes():
            (acceptable, distribution) = tasks.check_credulous_acceptance(z3i, semantics, threshold, argument)
            if acceptable:
    ...
    if args.skeptical_acceptance_all is not None:
        ...
        not_accepted_args = []
        for argument in af.get_nodes():
            acceptable, counterexample = tasks.check_skeptical_acceptance(z3i, semantics, threshold, argument)
            if acceptable:
                accepted_args.append(argument)
