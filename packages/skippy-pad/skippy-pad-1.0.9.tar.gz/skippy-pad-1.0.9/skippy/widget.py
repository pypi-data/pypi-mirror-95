...
from skippy import utils.critical
from skippy import utils.profile
...
class ToolbarWidget(QWidget):
    def __init__(self, parent):
        ...
        new_action = QAction(
            QIcon(
                os.path.join(config.ASSETS_FOLDER, config.settings["theme"], "new.png")
            ),
            "New",
            self,
        )
        ...
        open_action = QAction(
            QIcon(
                os.path.join(config.ASSETS_FOLDER, config.settings["theme"], "open.png")
            ),
            "Open",
            self,
        )
        ...
        upload_action = QAction(
            QIcon(
                os.path.join(
                    config.ASSETS_FOLDER, config.settings["theme"], "upload.png"
                )
            ),
            "Upload",
            self,
        ...
        upload_as_action = QAction(
            QIcon(
                os.path.join(
                    config.ASSETS_FOLDER, config.settings["theme"], "upload_as.png"
                )
            ),
            "Upload as...",
            self,
        ...
        close_action = QAction(
            QIcon(
                os.path.join(
                    config.ASSETS_FOLDER, config.settings["theme"], "close.png"
                )
            ),
            "Close",
            self,
        ...
        load_session_action = QAction(
            QIcon(
                os.path.join(
                    config.ASSETS_FOLDER, config.settings["theme"], "load_session.png"
                )
            ),
            "Load Session...",
            self,
        ...
        save_session_action = QAction(
            QIcon(
                os.path.join(
                    config.ASSETS_FOLDER, config.settings["theme"], "save_session.png"
                )
            ),
            "Save Session as...",
            self,
        ...
        exit_action = QAction(
            QIcon(
                os.path.join(config.ASSETS_FOLDER, config.settings["theme"], "exit.png")
            ),
            "Exit",
            self,
        )
        ...
        undo_action = QAction(
            QIcon(
                os.path.join(config.ASSETS_FOLDER, config.settings["theme"], "undo.png")
            ),
            "Undo",
            self,
        )
        ...
        redo_action = QAction(
            QIcon(
                os.path.join(config.ASSETS_FOLDER, config.settings["theme"], "redo.png")
            ),
            "Redo",
            self,
        )
        ...
        cut_action = QAction(
            QIcon(
                os.path.join(config.ASSETS_FOLDER, config.settings["theme"], "cut.png")
            ),
            "Cut",
            self,
        )
        ...
        copy_action = QAction(
            QIcon(
                os.path.join(config.ASSETS_FOLDER, config.settings["theme"], "copy.png")
            ),
            "Copy",
            self,
        )
        ...
        paste_action = QAction(
            QIcon(
                os.path.join(
                    config.ASSETS_FOLDER, config.settings["theme"], "paste.png"
                )
            ),
            "Paste",
            self,
        ...
        toggle_theme_action = QAction(
            QIcon(
                os.path.join(
                    config.ASSETS_FOLDER, config.settings["theme"], "toggle.png"
                )
            ),
            "Toggle Theme",
            self,
        ...
        login_action = QAction(
            QIcon(
                os.path.join(
                    config.ASSETS_FOLDER, config.settings["theme"], "login.png"
                )
            ),
            "Login",
            self,
        ...
        logout_action = QAction(
            QIcon(
                os.path.join(
                    config.ASSETS_FOLDER, config.settings["theme"], "logout.png"
                )
            ),
            "Logout",
            self,
...
class LoginStatusWidget(QWidget):
    ...
    def set_text(self):
        username = utils.profile.get_profile()[0]
...
class FilesDialog(QDialog):
    def __init__(self, widget, parent=None):
        ...
        self.setWindowTitle(f"skippy - {config.version}")
        self.move(300, 300)
        self.resize(200, 100)
        self.show()
...
class FileWidget(QWidget):
    ...
    def set_title(self, text):
        self.widget.data["files"][text] = self.widget.data["files"].pop(self.title)
        self.title = text
...
class LoginDialog(QDialog):
    ...
    def login(self):
        ...
        utils.profile.set_profile(login,password)
        self.parent.show()
        self.close()
...
class GetSites(QObject):
    ...
    def run(self):
        for e in pyscp.wikidot.User(utils.profile.get_profile()[0]).member:
