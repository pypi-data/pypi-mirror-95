...
def ввод_данных():
  ...
  if all(i in 'случайно' for i in rand_or_request):
    ...
    вес_мешков = np.random.randint(7, 11, len(количество_мешков)) #Масса каждого мешка
    
    новые_мешки = ['10:00', '12:00', '14:00']
    
    количество_новых_мешков = np.arange(10) #Количество мешков   
    вес_новых_мешков =  np.random.randint(7, 11, len(количество_новых_мешков))
...
def рассчитать_план_на_день(общее_число_ботов, количество_выживших, количество_эпох, коэфициент_мутаций):
  ...
  for it in range(epohs): # Проходим по всем эпохам
      ...
      if (it == 1500): # Меняем коэфициет мутации после 1500-ой эпохи
          mut = 0.1 
      if (it == 2000): # Меняем коэфициет мутации после 2000-ой эпохи
          mut = 0.05
      if (it == 2500): # Меняем коэфициет мутации после 2000-ой эпохи
          mut = 0.02
      if (it == 3000): # Меняем коэфициет мутации после 2000-ой эпохи
          mut = 0.01
