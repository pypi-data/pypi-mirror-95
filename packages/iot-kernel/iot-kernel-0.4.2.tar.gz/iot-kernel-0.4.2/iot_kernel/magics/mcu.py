...
@line_magic
def projects_magic(kernel, _):
    """Projects of currently connected device."""
    sep = '\n'
    kernel.print(sep.join(kernel.device.projects))
...
def info_magic(kernel, _):
    ...
    if len(kernel.device.projects) > 0:
        kernel.print(fmt.format('projects', kernel.device.projects[0]))
    if len(kernel.device.projects) > 1:
        for p in kernel.device.projects[1:]:
