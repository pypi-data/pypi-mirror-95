    ...
    for line in f:
        ...
        if not new_shell:
            ...
            if(idx_prim == num_of_primitives):
                shells.append(gp.shell(l,exp,coef,coord))
...
def get_all_shells_from_pubchempy_name(name):
    c = pcp.get_compounds('glucose', 'name')
    cid = c.cid
    c = pcp.Compound.from_cid(cid,record_type='3d')
    dic = c.to_dict(properties=['atoms'])
    shells = []
    for atom in dic['atoms']:
        shells += get_set_of_shells(atom['element'],(atom['x'],atom['y'],atom['z']),"aug-cc-pVDZ")
    return shells

def get_all_shells_from_pubchempy_cid(cid):
    c = pcp.Compound.from_cid(cid,record_type='3d')
    dic = c.to_dict(properties=['atoms'])
    shells = []
    for atom in dic['atoms']:
...
def get_all_shells_from_xyz(molecule):
    molecule = molecule.split()
    natoms = int(len(molecule)/4)
    shells = []
    for atom in range(natoms):
        shells += get_set_of_shells(molecule[atom*4],(float(molecule[atom*4+1]),float(molecule[atom*4+2]),float(molecule[atom*4+3])),"aug-cc-pVDZ")
    return shells
