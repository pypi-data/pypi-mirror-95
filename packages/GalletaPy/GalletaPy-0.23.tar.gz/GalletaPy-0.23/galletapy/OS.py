...
def VRR_OS2(i,j,k,l,N,pairAB,pairCD,ii,jj):
    ...
    RCD = np.linalg.norm(pairCD.g_a.coord-pairCD.g_b.coord)
    RPQ = np.linalg.norm(pairAB.coord[ii]-pairCD.coord[jj])
    return 2*np.pi**(5/2)/(pairAB.exp[ii]*pairCD.exp[jj]*np.sqrt(pairAB.exp[ii]+pairCD.exp[jj]))*np.exp(-pairAB.alpha[ii]*RAB**2)*np.exp(-pairCD.alpha[jj]*RCD**2)*gp.F_n(N,alpha*RPQ**2)
