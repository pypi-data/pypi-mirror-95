...
class Backend:
    ...
    def connect_btn_callback(self, current_muse_id, use_EEG, use_PPG, use_ACC, use_gyro):
        ...
        aux_outlet = self._get_aux_outlet()
        ...
        push_aux = partial(self._push, outlet=aux_outlet, offset=AUX_PORT_OFFSET) if use_gyro else None
    ...
    def _get_aux_outlet(self):
        aux_info = StreamInfo('Muse', 'Right AUX', MUSE_NB_AUX_CHANNELS, MUSE_SAMPLING_AUX_RATE,
                               'float32', 'Muse%s' % self.get_muse_address())
        aux_info.desc().append_child_value("manufacturer", "Muse")

        aux_outlet = StreamOutlet(aux_info, LSL_AUX_CHUNK)
    ...
    def _push(self, data, timestamps, outlet, offset=0):
        for ii in range(data.shape[1]):
            ...
            if self.is_udp_streaming:
                ...
                else:  # ppg or acc or gyro or aux
