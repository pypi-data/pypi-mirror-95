...
class MuseBLE(object):
    ...
    def __init__(self, client,
        ...
        self.callback_aux = callback_aux
    ...
    async def connect(self, timeout=20):
        ...
        if self.callback_aux is not None:
            await self._subscribe_aux()
    ...
    async def _subscribe_aux(self):
        await self._subscribe_to_uuid(uuid=MUSE_GATT_ATTR_AUX, callback=self._handle_aux)
    ...
    def _handle_aux(self, sender, packet):
        """Handle incoming aux data.
        sampling rate: ~17 x second (4 samples in each message, roughly 50Hz)"""
        if sender != 43:  # handle 0x14
            print("ERROR aux" + str(sender))
            return
        timestamps = [time()] * 1
        ...
        self.callback_aux(samples, timestamps)
