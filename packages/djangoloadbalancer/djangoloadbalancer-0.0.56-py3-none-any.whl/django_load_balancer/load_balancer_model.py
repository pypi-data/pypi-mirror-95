...
def send_query_to_listener(query):
    ...
    connection = multiprocessing.connection.Client(address, authkey=bytes(LOAD_BALANCER['ADDRESS']['AUTHKEY'], 'utf-8'))
...
def loadbalance_function(name):
    def loadbalanced_function(self, *args, **kwargs):
        query = Query(Wait.WAIT.value, QueryType.QUERYSET.value, method=name, args=args, kwargs=kwargs,
                      model=self.model)
...
class LoadBalancerModel(models.Model):
    ...
    def save(self, *args, **kwargs):
        query = Query(wait=Wait.WAIT.value, type=QueryType.NO_QUERYSET.value, method='save', model=self, args=args,
                      kwargs=kwargs)
        result = send_query_to_listener(query)
        return result

    def delete(self, *args, **kwargs):
        query = Query(wait=Wait.WAIT.value, type=QueryType.NO_QUERYSET.value, method='delete', model=self, args=args,
                      kwargs=kwargs)
        result = send_query_to_listener(query)
        return result
