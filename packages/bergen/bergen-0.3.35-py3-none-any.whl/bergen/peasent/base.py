...
from bergen.types.node.ports.int import IntPort
from bergen.types.node.ports.model import ModelPort
...
class BasePeasent(ABC):
    ...
    def enable(self, allow_empty_doc=False, **implementation_details):
        ...
        def real_decorator(function):
            ...
            for key, value in function_inputs.items():
                ...
                if issubclass(the_class, ArnheimModel):
                    inputs.append(ModelPort.fromParameter(value, the_class))
                if issubclass(the_class, int):
                    inputs.append(IntPort.fromParameter(value))
            ...
            for key, value in function_outputs.items():
                if issubclass(value, ArnheimModel):
                    outputs.append(ModelPort(value, key=key))
                if issubclass(value, int):
                    outputs.append(IntPort(value, key=key))
