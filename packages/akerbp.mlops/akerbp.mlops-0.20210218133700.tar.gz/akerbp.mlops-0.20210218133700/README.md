...
- Become familiar with the model template (see folder `model_code`) and make
  ...
  later) 
- Generate config file `mlops_settings.yaml` in your repo's root folder: 
  ```python
  import akerbp.mlops.core.config as c
  c.generate_default_project_settings()
  ```
  and fill in user settings. After that you can validate your settings:
  ```python
  c.validate_project_settings()
  ```
  Note that if you have an existing settings file, it's a good idea to validate it when you upgrade to a new MLOps version.
