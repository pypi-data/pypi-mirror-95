...
class Client(object):
  ...
  def _send(self, method, resource, *args, namespace=None, name=None, **kwargs):
    if 'url' in kwargs:
      endpoint = kwargs['url']
    else:
      ...
      if len(endpointInfo) > 0:
        ...
        group = 'core'
      resourceMatch = self.resources[resource['kind']][version]
      resourceDefinition = None
      for r in resourceMatch:
        if 'all' in r['categories'] and r['group'] == group:
          resourceDefinition = r
      if resourceDefinition is None:
        resourceDefinition = resourceMatch[0]
      if resourceDefinition['group'] == 'core':
        endpoint = "/api/%s/" % (version)
      else:
        endpoint = "/apis/%s/%s/" % (resourceDefinition['group'],version)

      if resourceDefinition['namespaced'] and namespace is not None:
        endpoint += "namespaces/%s/" % (namespace) 
        endpoint += "/".join(endpointInfo)+"%s" % (resourceDefinition['name'])
      else:
        endpoint += "/".join(endpointInfo)+"%s" % (resourceDefinition['name'])
    if name is not None:
      endpoint += "/%s" % (name)
  ...
  def watch(self, resource, name=None, namespace=None, version="v1"):
    watchEndpoints = self.getEndpoints(resource, 'watch', name=name, namespace=namespace, version=version)
    watchers = []
