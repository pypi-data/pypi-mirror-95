...
class CSVReader:
    ...
    def __init__(self, file_path, missing_header, separator, callback=lambda: None):
        ...
        with open(file_path) as csv_file:
            ...
            # count elements to calculate loading percentage
            total_elements = sum(1 for row in csv_reader)
            csv_file.seek(0)

            # finally fetching
            for line_count, row in enumerate(csv_reader):
                self.raw_data += [{column: row[index] for index, column in enumerate(self.columns)}]
                percentage = line_count / total_elements * 100
                callback(percentage)
            callback(100)
    ...
    def total_pages(self):
        return math.ceil(len(self.data) / 20)
