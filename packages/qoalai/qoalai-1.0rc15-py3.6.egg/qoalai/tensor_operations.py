...
def new_biases(length, 
    ...
    """
    return tf.Variable(tf.constant(0.05, 
                                    shape=[length], 
                                    dtype=data_type), 
                                    dtype=data_type, 
                                    name='bias_'+str(name))
...
def new_conv1d_layer(input, 
    ...
    weights = new_weights(shape=shape, 
                            name=name, 
                            data_type=data_type)
    layer = tf.nn.conv1d(input, 
                            filters=weights, 
                            stride=strides, 
                            padding=padding, 
                            name='convolution1d_' + str(name))

    if use_bias:
        biases = new_biases(length=filter_shape[2], 
                                name=name, 
                                data_type=data_type)
        layer += biases

    if use_batchnorm:
        layer = batch_norm(inputs=layer, 
                            training=is_training, 
                            name="batchnorm1d_" + str(name))
...
def new_conv2d_layer(input, 
    ...
    if use_bias:
        biases = new_biases(length=filter_shape[3], 
                            name=name, 
                            data_type=data_type)
        layer += biases
