...
def split_one_note(id, text):
    ...
    if previous_date is not None:
        split_note.append({'start': start, 'end': len(text), 'text': text[start:], 'date': normalize_date(previous_date, id, start, end)})
    else:
        logging.warning("No date detected for id: {} with text:\n{}".format(id, text))
