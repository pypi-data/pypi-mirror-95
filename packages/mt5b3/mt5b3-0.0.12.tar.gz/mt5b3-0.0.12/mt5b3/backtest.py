...
def set(assets,prestart,start,end,period,capital,file='backtest_file',verbose=False):
    ...
    if type(end)==datetime:
        ...
        return None
    if period==b3.DAILY or period==b3.INTRADAY or period==b3.H1:
        bts['type']=period
    else:
...
def getAfforShares(bts,dbars,assetId,money=None,volumeStep=None):
    ...
    return b3.getAfforShares(assetId,money,price,volumeStep)
...
def startBckt(bts): 
    ...
    dbars=dict()
    for asset in assets:
        dbars[asset]=b3.getBars(asset,bts['prestart'],bts['start'],bts['type'])
        bts['shares_'+asset]=0.0
    bars=b3.getBars(assets[0],bts['start'],bts['end'],bts['type'])
...
def checkBTS(bts):
    try:
        ...
        if type(bts['end'])!=datetime:
            print('end should be datetime')
            return False
        if bts['type']!=b3.DAILY and bts['type']!=b3.INTRADAY and bts['type']!=b3.H1:
            print('type should be daily or intraday or H1')
            return False
