...
def get_stocks_list(country=None):
    ...
    from ejtrader. Additionally, note that the country filtering can be applied, which is really useful since
...
def get_stock_recent_data(stock, country, as_json=False, order='ascending', interval='Daily'):
    ...
    Examples:
        >>> data = ejtrader.get_stock_recent_data(stock='bbva', country='spain')
        >>> data.head()
    ...
    resource_path = '/'.join((('/invest/resources', 'stocks.csv')))
...
def get_stock_historical_data(stock, country, from_date, to_date, as_json=False, order='ascending', interval='Daily'):
    ...
    Examples:
        >>> data = ejtrader.get_stock_historical_data(stock='bbva', country='spain', from_date='01/01/2010', to_date='01/01/2019')
        >>> data.head()
                     Open   High    Low  Close  Volume Currency
        Date
...
def get_stock_company_profile(stock, country='spain', language='english'):
    ...
    Examples:
        >>> company_profile = ejtrader.get_stock_company_profile(stock='bbva', country='spain', language='english')
        >>> company_profile
        company_profile = {
            url: 'https://www.investing.com/equities/bbva-company-profile',
            desc: 'Banco Bilbao Vizcaya Argentaria, S.A. (BBVA) is a ...'
...
def get_stock_financial_summary(stock, country, summary_type='income_statement', period='annual'):
    ...
    Examples:
        >>> data = ejtrader.get_stock_financial_summary(stock='AAPL', country='United States', summary_type='income_statement', period='annual')
        >>> data.head()
