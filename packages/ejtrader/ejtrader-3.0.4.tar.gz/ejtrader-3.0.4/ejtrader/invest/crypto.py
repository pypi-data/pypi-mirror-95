...
def get_cryptos():
    ...
    Returns:
        :obj:`pandas.DataFrame` - cryptos_df:
            ...
            indexed in Investing.com from the information previously retrieved by ejtrader and stored on a csv file.
...
def get_cryptos_list():
    ...
    Returns:
        :obj:`list` - cryptos_list:
            ...
            from the information previously retrieved by ejtrader and stored on a csv file.
...
def get_crypto_recent_data(crypto, as_json=False, order='ascending', interval='Daily'):
    ...
    Examples:
        >>> data = ejtrader.get_crypto_recent_data(crypto='bitcoin')
        >>> data.head()
    ...
    resource_path = '/'.join(('/invest/resources', 'cryptos.csv'))
...
def get_crypto_historical_data(crypto, from_date, to_date, as_json=False, order='ascending', interval='Daily'):
    ...
    Examples:
        >>> data = ejtrader.get_crypto_historical_data(crypto='bitcoin', from_date='01/01/2018', to_date='01/01/2019')
        >>> data.head()
                       Open     High      Low    Close  Volume Currency
        Date                                                           
