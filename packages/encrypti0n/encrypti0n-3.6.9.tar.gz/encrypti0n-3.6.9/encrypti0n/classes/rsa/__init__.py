...
class RSA(object):
	def __init__(self, 
		...
		if self.memory:
			...
			if directory != None:
				...
				self.directory = Formats.FilePath(private_key).base()
	...
	def generate_keys(self, log_level=0):
		...
		# checks.
		if not self.memory:
			if log_level >= 0: loader = system.console.Loader(f"Generating key {self.directory}")
			if os.path.exists(directory): 
				response["error"] = f"Key [{directory}] already exists."
				return response
		else:
			if log_level >= 0: loader = system.console.Loader("Generating key")
		...
		if not self.memory:
			os.mkdir(directory)
			utils.__save_file__(private_key_path, private_key_pem.decode())
			utils.__save_file__(public_key_path, public_key_pem.decode())
		...
		# response.
		loader.stop()
		return r3sponse.success_response(f"Successfully generated key {directory}.", {
	...
	def load_keys(self):
		...
		response = utils.__default_response__()
		if not self.memory:
			self.private_key_data = utils.__load_file__(self.private_key)
			self.public_key_data = utils.__load_file__(self.public_key)
		...
		if self.passphrase == None:
			...
			try:
				self.public_key_object = _RSA_.importKey(self.public_key_data, passphrase=self.passphrase)
		...
		# response.
		return r3sponse.success_response("Successfully loaded the key pair.")
	...
	def encrypt_file(self, path, layers=1):
		...
		if layers-1 > 0:
			...
			return r3sponse.success_response(f"Successfully encrypted file [{path}].")
	...
	def encrypt_directory(self, path, recursive=False, layers=1):
		...
		if recursive:
			...
			if layers-1 > 0:
				...
				return r3sponse.success_response(f"Successfully encrypted directory [{path}] (recursively).")
