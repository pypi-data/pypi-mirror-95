...
def config(tester, base_url):
    ...
    if tester or base_url:
        ...
        click.echo(
            click.style(f'请输入 unicase --tester <name> 或 unicase --base-url <base url> 进行配置', fg='bright_cyan', bold=True))
...
def upload(ctx, name, type):
    ...
    #     ctx.invoke(config)
    click.echo(click.style('功能正在开发中...', fg='bright_cyan', bold=True))
