...
def create_table1(
    ...
    try:
        ...
        with connection.cursor() as cursor:
            ...
            primary_key_str = ' AND '.join(map(lambda x: f'{x}=%s', primary_key))

            sql_txt = f'insert into {table_name} select * from {view_name} where {primary_key_str}'

            sql_txt1 = f'select {",".join(primary_key)} from {view_name}'
            with func(cursor=cursor, sql=sql_txt1) as result:
                for row in result:
                    cursor.execute(sql_txt, row)
                    logger.debug(f'Inserted: Where {primary_key_str}')
