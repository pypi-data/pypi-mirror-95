...
class TraceHook(Hook):
    ...
    def after_run(self, v):
        ...
        logging.info("########### once values: ", once_values)
        ...
        if self._writer is None:
            return
        if self._lstep == 1:
            logging.info("$$$$$$$$$$ headers, self._keys: %s", self._keys)
            logging.info("$$$$$$$$$$ headers, self._cb_keys: %s", self._cb_keys)
            logging.info(
                "$$$$$$$$$$ headers, self._once_keys: %s", self._once_keys
            )
        ...
        for v, func in zip(values, self._summaries):
            if isinstance(func, collections.Callable):
                logging.info("==== is func, v = %s, func = %s", v, func)
                temp_values.append(func(v))
            else:
                logging.info("==== not func, v = %s", v)
                temp_values.append(v)

        logging.info("########### temp values: ", temp_values)
