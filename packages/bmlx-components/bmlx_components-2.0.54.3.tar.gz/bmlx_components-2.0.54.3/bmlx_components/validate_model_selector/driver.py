...
MODEL_PARSE_PATTERN = "(hdfs://bigocluster)*([a-zA-Z0-9\/\-_]*\/[0-9]{10})"
...
class ModelSelectorDriver(Driver):
    ...
    def _parse_pushed_model(self, config_info) -> model_pb2.PushedModel:
        ...
        if ret is None:
            raise RuntimeError(
                "Failed to parse pushed_model: %s" % config_info["path"]
            )
    ...
    def _parse_source_model_info(self, origin_model_path: Text):
        ...
        logging.info(
            "_parse_source_model_info: %s, json_content: %s"
            % (read_in, content)
        )
        model = content["model"]
    ...
    def pre_execution(
        ...
        logging.info(
            "pushed_model_storage_base_path: %s", pushed_model_storage_base_path
        )
        ...
        if pushed_model.origin_model_path:
            ...
            artifact = Artifact(type_name=custom_artifacts.FgConf.TYPE_NAME)
