...
class Network(object):
	...
	def info(self):
		...
		try: 
			error = info["error"]
			if "Rate limit exceeded:" in str(error["title"]):
				return r3sponse.error_response(f"(https://ipinfo.io/json) {error['title']}: {error['message']} Define environment variable $IPINFO_API_KEY to specify an api key.")
			else:
	...
	def free_port(self, start=6080):
		for i in range(10000):
			...
			if not self.port_in_use(port):
				return r3sponse.success_response(f"Successfully found a free port.", {
					"port":port,
				})
		return r3sponse.error_response(f"Unable to find a free port.")
		#
