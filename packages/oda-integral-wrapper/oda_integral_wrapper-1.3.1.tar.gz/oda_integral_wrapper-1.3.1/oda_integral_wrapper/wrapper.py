    ...
    def long_scw_list_call(self, in_total_scw_list, s_max=50, **arguments):
        ...
        while True:
            for n_call, scw_list in enumerate(scw_lists):
                if n_poll == 0:
                    print("At call %d we elaborate %d scw" % (n_call, len(scw_list)))
                ...
                if data is None:
                    if not _disp.is_submitted:
                        ...
                        _disp.poll(silent=silent, verbose=(not silent))
            ...
            #print(f"complete {n_complete} / {len(disp_by_call)}")
    ...
    def write_spectrum_fits_files(spectrum, source_name, subcases_pattern, grouping=[0, 0, 0], systematic_fraction=0,
        ...
        if np.sum(grouping) != 0:
            ...
            if grouping[2] > 0:
                ...
                print('Geometric grouping with step %.3f' % e_step)
                loc_e = e_min[ind1]
