...
SkeletonObject = NewType('SkeletonObject', Union[Group, None])
...
class Skeleton:
    ...
    def to_skeleton_object(self) -> SkeletonObject:
        ...
        skeleton_object: SkeletonObject = SkeletonObject()
        skeleton_object += joint_points
        return functools.reduce(
            function=lambda _skeleton_object, _skeleton_part: _skeleton_object + _skeleton_part,
            sequence=joint_lines,
            initial=skeleton_object
        )
    ...
    def get_line_colors(self) -> np.ndarray:
        ...
        if self.color == 'default':
            ...
            np.full(shape=line_colors_shape,
                    fill_value=self.color, dtype='uint32')
...
class SkeletonVisualizer:
    ...
    def __create_skeleton_plot(self, skeletons: np.ndarray, colors: list[Color]) -> SkeletonPlot:
        ...
        return functools.reduce(
            function=lambda skeleton_plot, skeleton: skeleton_plot + skeleton.to_skeleton_object(),
            sequence=map(
                lambda skeleton, color: Skeleton(
                    joint_coordinates=skeleton,
