...
import k3d
import numpy as np
from k3d.objects import Group, Line, Points, TextureText, Drawable
...
class DrawableSkeleton(Group):
    def __init__(self, joint_points: Points, joint_lines: List[Line], joint_names: List[TextureText]):
        drawable_objects: List[Drawable] = [joint_points] + joint_lines + joint_names
        super().__init__(drawable_objects)
        self.joint_points: Points = joint_points
        self.joint_lines: List[Line] = joint_lines
        self.joint_names: List[TextureText] = joint_names
...
class Skeleton:
    ...
    def to_drawable_skeleton(self) -> DrawableSkeleton:
        ...
        joint_names: List[TextureText] = self.get_joint_names()
    ...
    def get_joint_names(self) -> List[TextureText]:
        return [k3d.texture_text(
            text=self.joint_set.names[i],
            position=self.joint_coordinates[i],
            size=0.45, color=DEFAULT_COLORS.get('black'))
