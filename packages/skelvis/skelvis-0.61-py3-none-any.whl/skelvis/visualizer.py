...
class DrawableSkeleton(Group):
    ...
    def hide_joint_names(self):
        for joint_name in self.joint_names:
            joint_name.visible = False

    def show_joint_names(self):
        for joint_name in self.joint_names:
            joint_name.visible = True
...
class SkeletonVisualizer:
    def __init__(self, joint_set: JointSet, size_scalar: float = 1.0):
        ...
        self.joint_name_visibility_button = widgets.Button(description="Hide joint names")
    ...
    def visualize(self, skeletons: np.ndarray, colors: List[Color] = None):
        ...
        self.joint_name_visibility_button.on_click(callback=self.__hide_skeleton_joint_names, remove=True)
        display(objs=self.joint_name_visibility_button)
        self.plot.display()
    ...
    def __hide_skeleton_joint_names(self):
        for skeleton in self.skeletons:
            skeleton.hide_joint_names()
        self.joint_name_visibility_button.description = "Show joint names"
        self.joint_name_visibility_button.on_click(callback=self.__show_skeleton_joint_names(), remove=True)

    def __show_skeleton_joint_names(self):
        for skeleton in self.skeletons:
            skeleton.show_joint_names()
        self.joint_name_visibility_button.description = "Hide joint names"
        self.joint_name_visibility_button.on_click(callback=self.__hide_skeleton_joint_names(), remove=True)
