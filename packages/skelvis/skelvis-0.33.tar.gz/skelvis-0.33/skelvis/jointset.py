...
Colors = NewType('Colors', Union[list(Color), type(None)])
...
class JointSet(metaclass=ABCMeta):
    ...
    def generate_skeleton_object(self, skeleton: Skeleton) -> Group:
    ...
    def __get_skeleton_colors(self, skeleton: Skeleton) -> tuple[list[Color], list[Color]]:
        if skeleton.color == 'default':
            joint_colors = [0] * self.number_of_joints
            line_colors = [0] * (self.number_of_joints - 1)
            joint_colors[self.left_joint_indices] = 0xFF0000
            joint_colors[self.right_joint_indices] = 0x0000FF
            joint_colors[self.center_joint_indices] = 0xFFFFFF
            line_colors[self.left_line_indices] = 0xFF0000
            line_colors[self.right_line_indices] = 0x0000FF
            line_colors[self.center_line_indices] = 0xFFFFFF
            return joint_colors, line_colors
        else:
            return [skeleton.color] * self.number_of_joints, [skeleton.color] * (self.number_of_joints - 1)

    def __create_skeleton_lines(self, skeleton: Skeleton, skeleton_line_colors: list[int]):
        return [self.__create_line_between_joints(
            start=skeleton.joint_coordinates[self.limb_graph[i][0]],
            end=skeleton.joint_coordinates[self.limb_graph[i][1]],
            width=skeleton.part_size / 4.0,
            color=skeleton_line_colors[i]
        ) for i in range(len(self.limb_graph))]
    ...
    @staticmethod
    def __create_skeleton_joint_points(skeleton: Skeleton, skeleton_joint_colors: list[int]) -> Points:
        return k3d.points(
...
class MuPoTSJoints(JointSet):
    def __init__(self):
        ...
        self.left_joint_indices = [2, 3, 4, 8, 9, 10]
        self.right_joint_indices = [5, 6, 7, 11, 12, 13]
