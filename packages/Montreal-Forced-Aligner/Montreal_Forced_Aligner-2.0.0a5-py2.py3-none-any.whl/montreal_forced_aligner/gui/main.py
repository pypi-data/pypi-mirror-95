...
class MainWindow(QtWidgets.QMainWindow):  # pragma: no cover
    ...
    def __init__(self):
        ...
        self.list_widget.utteranceMerged.connect(self.detail_widget.refresh_utterances)
        ...
        self.detail_widget.mergeSelected.connect(self.list_widget.merge_utterances)
    ...
    def update_speaker(self, utterance, speaker):

        if utterance is None:
            return
        old_speaker = self.corpus.utt_speak_mapping[utterance]
        if old_speaker == speaker:
            return
        if not speaker:
            return
        self.corpus.utt_speak_mapping[utterance] = speaker
        self.corpus.speak_utt_mapping[speaker].append(utterance)
        self.corpus.speak_utt_mapping[old_speaker] = [x for x in self.corpus.speak_utt_mapping[old_speaker] if x != utterance]
        self.list_widget.refresh_corpus(utterance)
