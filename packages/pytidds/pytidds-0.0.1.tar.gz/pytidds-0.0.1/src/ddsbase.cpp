            ...
            if (cnt % DDS_BLOCKSIZE == 0) {
                if (data == NULL) {
                    if ((data = (unsigned char *)malloc(DDS_BLOCKSIZE)) == NULL)
                        ERRORMSG();
                } else if ((data = (unsigned char *)realloc(
                                data, cnt + DDS_BLOCKSIZE)) == NULL)
                    ERRORMSG();
...
unsigned char *readPVMvolume(
    ...
    unsigned int *depth, unsigned int *components, float *scalex, float *scaley,
    float *scalez, unsigned char **description, unsigned char **courtesy,
    ...
    if ((volume =
        ...
        ERRORMSG();
    if (data + bytes != ptr + (*width) * (*height) * (*depth) * numc + len1 +
                            len2 + len3 + len4)
        ERRORMSG();
