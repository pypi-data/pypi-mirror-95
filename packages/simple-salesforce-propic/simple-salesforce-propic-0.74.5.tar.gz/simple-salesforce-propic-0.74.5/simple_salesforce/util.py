...
def call_salesforce(url, method, session, headers, **kwargs):
    ...
    while True:
        try:
            ...
            print(e)
            print('[Propic User] INFO: Detected Salesforce ConnectionError, now sleeping for a while and retrying...')
            time.sleep(0.1)
            count += 1
            if count > 100:
                raise Exception('[Propic User] ERROR: Failed on retrying connection multiple times')
