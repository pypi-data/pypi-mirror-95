    ...
    def put_directory(self, target_path, local_directory, **kwargs):
        ...
        try:
            for path, remote_path, files in gathered_files:
                _path = os.path.join(target_path, remote_path)
                if not kwargs.get('resume', False) or not self.exists(_path):
                    self.mkdir(_path)
                for name in files:
                    res = self.put_file(_path, os.path.join(path, name), **kwargs, executor=executor)
        ...
        if kwargs.get('resume', False):
            if self.exists(remote_path):
                ...
                try:
                    ...
                    pass
            cached = utils.read_cache_file(cache_file_name, datetime.timedelta(hours=kwargs.get('cache_validity', 8)))
            try:
                index = int(cached)
            except BaseException:
                pass
                ...
                if chunk_index == chunk_count - 1 and kwargs.get('executor', None):
                    def task():
                        ...
                        if res:
                            ...
                            file_handle.close()
                            if kwargs.get('show_progress_bar', True):
                                progress(len(data) if chunk_count > 1 else size)
                        sys.stdout.write("\n")
                        return res
