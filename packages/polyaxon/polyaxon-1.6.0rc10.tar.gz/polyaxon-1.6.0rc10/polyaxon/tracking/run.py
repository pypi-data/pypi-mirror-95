...
class Run(RunClient):
    ...
    Args:
        ...
        is_new: bool, optional,
            Force creation of a new run instead of discovering the cached run or refreshing an instance from the env var
        name: str, optional,
            When `is_new` or `is_offline` is set to true, a new instance is created and you can initialize that new run with a name.
        name: str, optional,
            When `is_new` or `is_offline` is set to true, a new instance is created and you can initialize that new run with a description.
        tags: str or List[str], optional,
            When `is_new` or `is_offline` is set to true, a new instance is created and you can initialize that new run with tags.
    ...
    def __init__(
        ...
        # Track run env
        if self._artifacts_path and self._track_env:
            self.log_env()
        ...
        if is_new and self._artifacts_path and self._track_logs:
    ...
    def _set_exit_handler(self, is_new: bool = False):
        ...
        elif self._is_offline or is_new:
            self._start()
