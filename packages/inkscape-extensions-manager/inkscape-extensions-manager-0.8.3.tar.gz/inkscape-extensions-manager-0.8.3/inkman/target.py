...
class BasicTarget(object):
    ...
    def _list_installed(self):
        # XXX: Add file type filter so random files are not considered 'orphans'
            ...
            for fname in files:
                ...
                if fname.endswith('.json'):
                    with open(fpath, 'rb') as fhl:
                        ...
                        item = PackageItem(info, remote=self.archive._remote_file)
...
class ExtensionsTarget(BasicTarget):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.get_pip() is None:
            try:
                logging.warning("Installing virtualenv...")
                call('virtualenv', self.path, p='python3')
    ...
    def pip_install(self, filename):
        ...
        pip = self.get_pip()
        if pip is None:
            logging.error("No PIP command available, failed to install.")
            return None
        try:
    ...
    def _list_installed(self):
        ...
        # First collect a list of pythong packages installed
