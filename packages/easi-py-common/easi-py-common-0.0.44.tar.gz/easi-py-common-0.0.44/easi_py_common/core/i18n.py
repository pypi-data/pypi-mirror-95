...
def verify_language(lang: Optional[str]) -> str:
    if not lang and current_app.header_language_function is not None:
        lang = current_app.header_language_function()
    if not lang:
        lang = request.headers.get(current_app.header_language_name)
    if lang == LANG_ZH:
        lang = LANG_CN
    lang = current_app.i18n_language_mapping.get(lang) or lang
    return lang if lang in LANG_LIST else (current_app.i18n_language_default or LANG_EN)


def trans_data(data_cn, data_en, lang=None):
    lang = verify_language(lang)
...
def tt(translate_key: str, lang: Optional[str] = None) -> Dict:
    ...
    :return:
    """
    return current_app.i18n.get(verify_language(lang), Dict()).get(translate_key, Dict())
...
class BaseErrorCode(Enum):
    def __init__(self, error_code_prefix='error_code'):
...
def init_app(app: Flask, lang_list: List[str], header_language_name: str = 'language',
    ...
    for lang in lang_list:
        ...
        lang_yml = yaml.load(lang_yml_path, Loader=yaml.FullLoader)
