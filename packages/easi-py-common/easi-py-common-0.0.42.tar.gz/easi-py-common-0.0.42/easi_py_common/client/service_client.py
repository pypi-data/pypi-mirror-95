...
def service_mapping_get(uri, timeout=None):
    return service_mapping(uri, method=HTTP_METHOD_GET, timeout=timeout)


def service_mapping_post(uri, timeout=None, request_by_alias: bool = True, body_key='data'):
    return service_mapping(uri, method=HTTP_METHOD_POST, timeout=timeout,
                           request_by_alias=request_by_alias, body_key=body_key)


def service_mapping_put(uri, timeout=None, request_by_alias: bool = True, body_key='data'):
    return service_mapping(uri, method=HTTP_METHOD_PUT, timeout=timeout,
                           request_by_alias=request_by_alias, body_key=body_key)


def service_mapping_delete(uri, timeout=None):
