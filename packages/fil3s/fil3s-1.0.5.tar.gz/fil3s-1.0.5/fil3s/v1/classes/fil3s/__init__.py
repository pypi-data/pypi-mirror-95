...
class Formats():
    ...
    class FilePath(object):
        ...
        def check(self, 
            ...
            #
        def modification_time(self, format='%d-%m-%y %H:%M.%S'):
            fname = pathlib.Path(self.path)
            try:
                mtime = datetime.fromtimestamp(fname.stat().st_mtime)
            except:
                mtime = datetime.fromtimestamp(fname.stat().ct_mtime)
            timestamp = str(mtime.strftime(format))
            if format in ['s', 'seconds']:
                timestamp = Formats.Date().to_seconds(timestamp, format=format)
            return timestamp
