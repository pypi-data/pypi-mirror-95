...
class Files():
	...
	class Array(object):
		...
		def save(self, array=None, ensure_ascii=False, indent=4):
			...
			utils.__check_memory_only__(self.file_path.path)
			self.array = array
			return Files.save(self.fp.path, array, format="json", indent=indent, ensure_ascii=ensure_ascii)
		def load(self, default=None):
			utils.__check_memory_only__(self.file_path.path)
			if not os.path.exists(self.file_path.path) and default != None: 
				self.save(default)
			array = Files.load(self.file_path.path, format="json")
			self.array = array
			return array
	...
	class Dictionary(object):
		...
		def save(self, dictionary=None, path=None, ensure_ascii=False, indent=4):
			...
			self.dictionary = dictionary
			return Files.save(self.fp.path, dictionary, format="json", indent=indent, ensure_ascii=ensure_ascii)
		def load(self, default=None):
			utils.__check_memory_only__(self.file_path.path)
			if not os.path.exists(self.file_path.path) and default != None: 
				self.save(default)
			dictionary = Files.load(self.file_path.path, format="json")
			self.dictionary = dictionary
			return dictionary
