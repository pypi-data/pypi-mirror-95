...
class Files():
	...
	def save(path, data, format="str"):
		...
		if format == "str":
			...
			file.write(Formats.denitialize(data))
			file.close()
		elif format == "json":
			with open(str(path), "w") as json_file:
				json.dump(Formats.denitialize(data), json_file, indent=4, ensure_ascii=False)
		elif format == "bytes":
	...
	def chmod(path=None, permission=None, recursive=False, sudo=False):
	...
	class Array(object):
		...
		def save(self, array=None, ensure_ascii=False, indent=4):
			...
			array = Formats.denitialize(array)
			try:
				with open(str(self.file_path.path), 'w+') as json_file:
		...
		def load(self, default=None):
			...
			array = None
			try: 
				with open(str(self.file_path.path), 'r+') as json_file:
	...
	class Dictionary(object):
		...
		def save(self, dictionary=None, path=None, ensure_ascii=False, indent=4):
			...
			if path == None: path = self.file_path.path
			dictionary = Formats.denitialize(dictionary)
			# save.
			try:
				with open(str(path), 'w+') as json_file:
	...
	class Bytes(object):
		def __init__(self, 
		   ...
		   # init.
			...
			#
		def load(self):
			with open(str(self.file_path.path), 'rb') as file_data:
		...
		def save(self, bytes=None):
			...
			with open(str(self.file_path.path), 'wb') as file_data:
