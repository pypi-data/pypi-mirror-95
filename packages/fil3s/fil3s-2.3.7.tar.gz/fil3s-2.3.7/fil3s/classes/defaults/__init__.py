...
class Files():
	...
	def chmod(path=None, permission=None):
		if path == None: raise ValueError("Define parameter: path.")
		if permission == None: raise ValueError("Define parameter: permission.")
		return gfp.permission.set(path=path, permission=permission)
	def chown(path=None, owner=None, group=None):
		if path == None: raise ValueError("Define parameter: path.")
		if permission == None: raise ValueError("Define parameter: permission.")
		return gfp.ownership.set(path=path, owner=owner, group=group)
	...
	class File(object):
		...
		def __contains__(self, key):
			if isinstance(key, (list, Formats.Array)):
				for i in key:
					if key in self.data:
						return True
				return False
	...
	class Script(File):
		def __init__(self, 
			...
			# check self instance.
			if isinstance(data, Files.Script):
				data = data.data
			File.__init__(self,
				path=path,
				data=data,
		...
		def execute(self, 
			...
			try:
				...
				return r3sponse.error(f"Failed to execute script ({script}), (output: {output}), (error: {error_}).")
			...
			return r3sponse.success(f"Succesfully executed script ({script}), (output: {output}), (error: {error_}).", {
				"output":output,
				"process":proc,
			})
		# object instance.
	...
	class Array(object):
		...
		def __gt__(self, array):
			if not isinstance(array, self.__class__):
	...
	class Dictionary(object):
		...
		def unpack(self, 
			...
			for key in keys:
				value = None
				try: value = self.dictionary[key]
				except KeyError: value = defaults[key]
				if value == None:
		...
		def __gt__(self, dictionary):
			if not isinstance(dictionary, self.__class__):
		...
		def __eq__(self, dictionary):
			if not isinstance(dictionary, self.__class__):
				return False
			return dict(self.sort()) == dict(dictionary.sort())
		...
		def __contains__(self, key):
			if isinstance(key, (list, Formats.Array)):
				...
				return key in list(self.dictionary.keys()) 
		# support item assignment.
		...
		def instance(self, serialize=False):
			return "Dictionary"
		# support self assignment.
		def assign(self, dictionary:dict):
			self.dictionary = dictionary
			return self
		#  
	#
	...
	class Directory(object):
		...
		def __contains__(self, path):
			paths = self.paths()
			if isinstance(path, (list, Formats.Array)):
				for i in path:
					if path in paths:
						return True
				return False
	...
	class Bytes(object):
		...
		def __eq__(self, bytes_):
			if not isinstance(bytes_, self.__class__):
				return False
		...
		def __contains__(self, bytes_):
			if isinstance(bytes_, (list, Formats.Array)):
				for i in bytes_:
					if i == self.bytes:
						return True
				return False
...
class Formats():
	...
	def denitialize(
		...
		#if obj == None and objects == None: raise exceptions.InvalidUsage("Define either parameter obj (#1) or objects.")
		if objects == None:
			if file_paths and isinstance(obj, Files.Directory):
				obj = str(obj)
			elif file_paths and isinstance(obj, Formats.FilePath):
				obj = str(obj)
			elif isinstance(obj, Formats.String):
				obj = str(obj)
			elif isinstance(obj, Formats.Boolean):
				obj = bool(obj)
			elif isinstance(obj, Formats.Integer):
				obj = obj.value
			elif isinstance(obj, Files.Array):
				new = []
				for i in obj.array:
					new.append(Formats.denitialize(i, file_paths=file_paths))
				obj = new
			elif isinstance(obj, Files.Dictionary):
				new = {}
				for k,v in obj.dictionary.items():
					new[k] = Formats.denitialize(v, file_paths=file_paths)
				obj = new
			return obj
		else:
			if isinstance(objects, dict) or isinstance(objects, Dictionary):
				...
				for i,default in objects.items(): 
					try:
						objs[i] = self.unserialize(i, file_paths=file_paths)
					except:
						objs[i] = default
			else:
				objs = []
				for i in objects: objs.append(self.unserialize(i, file_paths=file_paths))
				return objs
	#
	...
	class FilePath(object):
		...
		def instance(self):
			...
			#
		# support self assignment.
		def assign(self, path:str, load=False):
			self.path = path
			self.ownership = self.Ownership(path=self.path, load=load)
			self.permission = self.Permission(path=self.path, load=load)
			return self
	...
	class String(object):
		...
		def __gt__(self, string):
			if not isinstance(string, self.__class__):
				raise exceptions.FormatError(f"Can not compare object {self.__class__} & {string.__class__}.")
			return len(self.string) > len(string.string)
		def __ge__(self, string):
			if not isinstance(string, self.__class__):
				raise exceptions.FormatError(f"Can not compare object {self.__class__} & {string.__class__}.")
			return len(self.string) >= len(string.string)
	...
	class Boolean(object):
		...
		def __eq__(self, boolean):
			if not isinstance(boolean, self.__class__):
				return False
	...
	class Date(object):
		...
		# support 'in' operator.
		def __contains__(self, string):
			if isinstance(string, (list, Formats.Array)):
				for i in string:
					if string in self.timestamp:
						return True
				return False
			else:
