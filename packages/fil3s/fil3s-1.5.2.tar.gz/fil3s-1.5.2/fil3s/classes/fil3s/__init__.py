    ...
    class Directory(object):
        def __init__(self, path=None, hierarchy={}):
            ...
            if path == False: self.file_path = False # used in local memory (not fysical)
                ...
                self.file_path = Formats.FilePath(path, check_existance=hierarchy!={})
        ...
        def paths(self, dirs_only=False, files_only=False, empty_dirs=True, recursive=False, path=None, banned=[], banned_names=[".DS_Store"]):
            ...
            if recursive:
                ...
                for root, dirs, files in os.walk(path):
                    ...
                    if not files_only:
                        emptydirs = []
                        if empty_dirs:
                            emptydirs = [dirpath for (dirpath, dirnames, filenames) in os.walk(path) if len(dirnames) == 0 and len(filenames) == 0]
                        for name in dirs+emptydirs:
                            if name not in banned_names:
                                l_path = os.path.join(root, name)
                                #print("{} vs {} = {}".format(l_path, banned, l_path not in banned and l_path+"/" not in banned))
                                if l_path not in banned and l_path+"/" not in banned:
                                    paths += self.paths(recursive=True, path=l_path, dirs_only=dirs_only, files_only=files_only, banned=banned, banned_names=banned_names)
                return paths
            else:
                ...
                if empty_dirs:
                    emptydirs = [dirpath for (dirpath, dirnames, filenames) in os.walk(path) if len(dirnames) == 0 and len(filenames) == 0]
                for l_path in paths+emptydirs:
                    if l_path not in banned_names:
                        l_path = f'{path}/{l_path}'.replace("//",'/')
                        if (not dirs_only and not files_only) or  (dirs_only and os.path.isdir(l_path)) or (files_only and not os.path.isdir(l_path)):
                            if l_path not in banned and l_path+"/" not in banned:
    ...
    class FilePath(object):
        ...
        def base(self, back=1):
            ...
            for i in splitted:
                ...
                count += 1
            """
        def basename(self, back=1):
            return self.name(path=self.base(back=back))
        ...
        def clean(self, path=None, remove_double_slash=True, remove_first_slash=False, remove_last_slash=False):
            if path == None: path = self.path
            while True:
