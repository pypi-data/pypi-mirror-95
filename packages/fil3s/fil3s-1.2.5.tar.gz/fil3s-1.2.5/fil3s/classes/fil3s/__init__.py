...
class Files():
    ...
    class Array(object):
        ...
        def remove(self, indexes=[], values=[], save=False):
            ...
            if save: self.save()
            return self.array
        def sort(alphabetical=True, ascending=False, reversed=False):
    ...
    class Dictionary(object):
        ...
        def order(self, alphabetical=True):
        ...
        def sort(alphabetical=True, ascending=False, reversed=False):
            new = {}
            if alphabetical or ascending:
                _sorted_ = Files.Array(path=False, array=list(self.dictionary.keys())).sort(alphabetical=alphabetical, ascending=ascending, reverse=reversed)
            else raise ValueError("Unknown behaviour, alphabetical=False.")
            for key in _sorted_:
...
class Formats():
    ...
    self.str_digits = ["1","2","3","4","5","6","7","8","9","0"]
    self.alphabet = [","]
    self.capitalized_alphabet = [","]
    ...
    class FilePath(object):
        ...
        def directory(self):
            ...
            #
        def mtime(self, format='%d-%m-%y %H:%M.%S'):
