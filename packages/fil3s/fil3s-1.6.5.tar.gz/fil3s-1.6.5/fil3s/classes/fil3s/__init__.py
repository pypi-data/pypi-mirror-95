...
class Formats():
    ...
    class Integer(object):
        ...
        def increase_version(self):
            ...
            # version 2.
            #
            return subprocess.check_output(f"""version='{self.value}' && echo $version | awk -F. -v OFS=. 'NF==1{print ++$NF}; NF>1{if(length($NF+1)>length($NF))$(NF-1)++; $NF=sprintf("%0*d", length($NF), ($NF+1)%(10^length($NF))); print}'""").decode().replace("\n","")
