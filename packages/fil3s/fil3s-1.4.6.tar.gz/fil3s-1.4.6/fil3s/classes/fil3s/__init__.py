    ...
    class Directory(object):
            ...
            if recursive:
                ...
                for l_path in paths:
                    if l_path not in banned_names:
                        ...
                        if (dirs_only and os.path.isdir(l_path)) or (files_only and not os.path.isdir(l_path)):
                            if l_path not in banned and l_path+"/" not in banned:
        ...
        def find(self, matches:list, path=None, recursive=False, log_level=0):
            ...
            for string in matches:
                dir, gfp = Files.Directory(path), Formats.FilePath("")
                if not dir.file_path.exists():
                    raise ValueError(f"Path {path} does not exist.")
                elif not dir.file_path.directory():
                    raise ValueError(f"Path {path} is not a directory.")
                for i_path in dir.paths(recursive=recursive, files_only=True, banned_names=[".DS_Store", ".git"]):
                    data = None
                    try:
