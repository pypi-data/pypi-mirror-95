...
def execute(
	...
	try:
		...
		return r3sponse.error_response(f"Failed to execute command ({command}), (output: {output}), (error: {error_}).")
	...
	Files.delete(path)
	return r3sponse.success_response(f"Succesfully executed command ({command}), (output: {output}), (error: {error_}).", {
		"output":output,
		"pid":process.pid,
		"process":proc,
	})
