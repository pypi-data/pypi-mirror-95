...
class Env(object):
	...
	def fill(self, string):
		...
		for i in range(string.count("$")):
			c += 1
			if string[0] == "$" and c == 1:
				variable_id, _ = Formats.String(string.split("$")[1]).before_after_first_occurence(slicer=["/","+",">","<","?",";",":","|","\\","]","[","{","}","=","+","(",")","!","@","#","$","%","^","&","*","±","~","`","'",'"'," "])
			else:
				variable_id, _ = Formats.String(string.split("$")[c]).before_after_first_occurence(slicer=["/","+",">","<","?",";",":","|","\\","]","[","{","}","=","+","(",")","!","@","#","$","%","^","&","*","±","~","`","'",'"'," "])
	...
	def set(self, id, value, format="unknown"):
		...
		if format in [str, "str", "string"]:
			return self.set_string(id, value)
		elif format in [bool, "bool", "boolean"]:
			return self.set_boolean(id, value)
		elif format in [int, float, "int", "float", "integer", "double"]:
			return self.set_integer(id, value)
		elif list in [list, "list", "array"]:
			return self.set_array(id, value)
		elif dict in [dict, "dict", "dictionary", "json"]:
			return self.set_dictionary(id, value)
		else: raise ValueError(f"Unkown format: {format}.")
	def set_string(self, id, value):
		return os.environ[id] = value
	...
	def set_integer(self, id, value):
		return os.environ[id] = str(value)
	def set_array(self, id, value):
		return os.environ[id] = Formats.Array(value).string(joiner=",")
