...
class WebServer(objects.Thread):
	...
	def set(self, group=None, id=None, data=None):
		...
		try:
			...
			return r3sponse.error_response(f"Failed to serialize {response}: {response.text}, status code {response.status_code}.")
	...
	def app(self):
		...
		def get():
			...
			try:
				...
				return r3sponse.error_response(f"There is no value cached for {tag}.").json()
