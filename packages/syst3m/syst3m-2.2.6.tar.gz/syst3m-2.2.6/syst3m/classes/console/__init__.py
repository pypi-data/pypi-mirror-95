...
# execute.
def execute(
	# the command in str format.
	command="ls .",
	# joiner for when command is in list format.
	joiner="\n",
	# serialize to dict. (r3sponse.ResponseObject) (slice first dict).
	serialize=False,
):
	path = f"/tmp/tmp_script_{Formats.String('').generate()}"
	if isinstance(command, list):
		command = Files.Array(array=command).string(joiner=joiner)
	...
	try:
		...
		return r3sponse.error(f"Failed to execute command ({command}), (output: {output}), (error: {error_}).")
	...
	if serialize:
		...
		return r3sponse.success(f"Succesfully executed command ({command}), (output: {output}), (error: {error_}).", {
			"output":output,
			"pid":process.pid,
			"process":proc,
		})
