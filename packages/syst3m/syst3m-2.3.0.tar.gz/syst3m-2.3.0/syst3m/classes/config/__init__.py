...
def __check_alias__(
	...
	if (("--force" in sys.argv or "--forced" in sys.argv or overwrite) and present) or (present or not os.path.exists(path)):
...
ALIAS = "syst3m"
SOURCE_NAME = "syst3m"
SOURCE_PATH = Formats.FilePath(__file__).base(back=3)
...
# download.
	...
	if os.path.exists("/usr/bin/pip3"): 
		os.system(f"/usr/bin/pip3 install -r {Formats.FilePath(__file__).base(back=3)}/requirements/requirements.txt --user {os.environ.get('USER')}")
	else:
		os.system(f"pip3 install -r {Formats.FilePath(__file__).base(back=3)}/requirements/requirements.pip")
