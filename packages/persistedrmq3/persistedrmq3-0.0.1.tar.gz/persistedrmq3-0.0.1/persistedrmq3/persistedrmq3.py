...
class Message(BaseModel):
    id: str = Field('', title="消息唯一ID")
    to: str = Field('', title="收件人")
    tp: str = Field('', title="消息类型")
    rms: List[str] = Field('', title="删除消息id列表")
    payload: str = Field('', title="内容")


class MessageList(BaseModel):
    msgs: List[Message] = Field([], title="消息列表")
...
class PersistedRmq(object):
                ...
                if target in self.connections:
                    ...
                    for cid, sock in socks.items():
                        try:
                            ...
                            message_model = Message().parse_raw(str(msg, encoding='utf8'))
                            messages = MessageList(msgs=[message_model,])
        ...
        while True:
            try:
                ...
                if message_type == MESSAGE_TYPE_REPLY:
                    message_ids = message_obj.rms
                    await self.comfirm_issued(chn, message_ids=message_ids)
                else:
                    await self.publish(message_obj.to, message_obj.id, message)
                    if after_dispached:
    ...
    async def publish(self, to_chn: str, msg_id: str, message: str):
        dispatch_message = ":".join([to_chn, msg_id])
        await self.persisted_message(to_chn, msg_id, message)
