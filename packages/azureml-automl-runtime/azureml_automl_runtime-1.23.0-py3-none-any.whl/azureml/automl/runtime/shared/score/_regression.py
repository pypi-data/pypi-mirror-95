...
class NormMeanAbsoluteError(RegressionMetric, ScalarMetric):
    ...
    def compute(self):
        ...
        if self._y_min == self._y_max:
            warning_format = "Skipping NormMeanAbsoluteError calculation. All targets are equal. {}"
            if np.array_equal(self._y_test, self._y_pred):
                self._logger.warning(warning_format.format("Returning best score."))
                return constants.REGRESSION_RANGES[constants.NORM_MEAN_ABS_ERROR][0]
            else:
                self._logger.warning(warning_format.format("Returning worst score."))
...
class NormMedianAbsoluteError(RegressionMetric, ScalarMetric):
    ...
    def compute(self):
        """Compute the score for the metric."""
        if self._y_min == self._y_max:
            warning_format = "Skipping NormMedianAbsoluteError calculation. All targets are equal. {}"
            if np.array_equal(self._y_test, self._y_pred):
                self._logger.warning(warning_format.format("Returning best score."))
                return constants.REGRESSION_RANGES[constants.NORM_MEDIAN_ABS_ERROR][0]
            else:
                self._logger.warning(warning_format.format("Returning worst score."))
...
class NormRMSE(RegressionMetric, ScalarMetric):
    ...
    def compute(self):
        """Compute the score for the metric."""
        if self._y_min == self._y_max:
            warning_format = "Skipping NormRMSE calculation. All targets are equal. {}"
            if np.array_equal(self._y_test, self._y_pred):
                self._logger.warning(warning_format.format("Returning best score."))
                return constants.REGRESSION_RANGES[constants.NORM_RMSE][0]
            else:
                self._logger.warning(warning_format.format("Returning worst score."))
...
class NormRMSLE(RegressionMetric, ScalarMetric):
    ...
    def compute(self):
        ...
        if self._y_min == self._y_max:
            warning_format = "Skipping NormRMSLE calculation. All targets are equal. {}"
            if np.array_equal(self._y_test, self._y_pred):
                self._logger.warning(warning_format.format("Returning best score."))
                return constants.REGRESSION_RANGES[constants.NORM_RMSLE][0]
            else:
                self._logger.warning(warning_format.format("Returning worst score."))
