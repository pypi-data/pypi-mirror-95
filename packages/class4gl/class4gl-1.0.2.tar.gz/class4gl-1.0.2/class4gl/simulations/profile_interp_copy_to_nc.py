...
arguments.append(dict(arg='--timeseries_forcing', help="put timeseries forcing from model_output objects under --path_forcing into the model_input object") )
# arguments.append(dict(arg='--path_timeseries_forcing', help="put timeseries forcing from model_output objects under --path_forcing into the model_input object") )
...
# # for debugging, comment out up to 'def execute' above and uncomment the line below
# if True:
    ...
    EXP_DEFS  =\
    {
      ...
      'BASE_ITER_SHR_ADV_EFERA5_SMHWB_ENT_MED':{'sw_ac' : ['adv',],'sw_ap': True,'sw_lit': False},
      'BASE_ITER_SHR_ADV_EFERA5_SMHWB_ENT2_MED':{'sw_ac' : ['adv',],'sw_ap': True,'sw_lit': False},
    ...
    if args.global_chunk_number is not None:
        ...
        if len(all_records_morning_select.chunk.unique()) > 1:
            stations_chunks_sizes = all_records_morning_select.drop(columns=['STNID','chunk']).groupby(['STNID','chunk']).size()
            #import pdb; pdb.set_trace()
            print(stations_chunks_sizes)
            current_station_chunk = stations_chunks_sizes.index[int(args.global_chunk_number)]
            print(current_station_chunk)
            run_stations = pd.DataFrame([all_stations.table.loc[current_station_chunk[0]]])
            print(run_stations)
            run_station_chunk = current_station_chunk[1]
    ...
    if len(records_morning) == 0:
        raise IOError("No initialization records records found in "+\
                      args.path_forcing+' (subset: '+args.subset_forcing+')')
    
    # note that if runtime is an integer number, we don't need to get the afternoon
    # profiles. 
    
    load_end_profiles = True #\
    #         (args.runtime == 'from_profile_pair') or \
    #         (args.timeseries_forcing is not None)
    if load_end_profiles:
        print('Fetching afternoon records for determining the simulation runtimes')
        # records_morning_tsf = get_records(run_stations,\
        #                               args.path_forcing,\
        #                               subset=args.subset_forcing,
        #                               refetch_records=False,
        #                               )
        ...
        if len(records_afternoon) == 0:
            raise IOError("No final state records found in "+\
                          args.path_forcing+' (subset: end )')
        
        # # print(records_morning_tsf.index)
        # # print(records_afternoon.index)
        # # align afternoon records with the noon records, and set same index
        # print('hello')
        # print(len(records_afternoon))
        # print(len(records_morning_tsf))
        # print("aligning morning_tsf and afternoon records")
        # records_afternoon.index = records_morning_tsf.index
        # records_afternoon['dates'] = records_morning_tsf['ldatetime'].dt.date
        # records_afternoon = records_afternoon.drop(columns=['STNID','chunk'])
        # records_afternoon = records_afternoon.reset_index()
        # records_afternoon = records_afternoon.set_index(['STNID','chunk','dates'])
        # ini_index_dates = records_afternoon.index

        # # records_afternoon['dates'] = records_afternoon['ldatetime'].dt.date
        # # records_afternoon.set_index(['STNID','dates'],inplace=True)
        # # ini_index_dates = records_morning_tsf.set_index(['STNID','dates']).index
        # # records_afternoon = records_afternoon.loc[ini_index_dates]
        # # records_afternoon.index = records_morning_tsf.index


        # records_morning['dates'] = records_morning['ldatetime'].dt.date
        # records_morning = records_morning.drop(columns=['chunk'])
        # records_morning = records_morning.drop(columns=['STNID'])
        # records_morning = records_morning.reset_index()
        # records_morning.set_index(['STNID','chunk','dates'],inplace=True)

        # index_intersect = ini_index_dates.intersection(records_morning.index)

        # records_afternoon = records_afternoon.loc[index_intersect].sort_index()
        # records_morning = records_morning.loc[index_intersect].sort_index()

        # records_morning = records_morning.reset_index()
        # records_afternoon = records_afternoon.reset_index()
        # records_morning.set_index(['STNID','chunk','index'],inplace=True)
        # records_afternoon.index = records_morning.index

    # tempdata_ini = pd.DataFrame(c4gldata[source["sim"]].frames['stats']['records_all_stations_ini'].drop(columns=['STNID','chunk']).reset_index().reset_index().copy())
    # tempdata_ini['dates'] = tempdata_ini.ldatetime.dt.date
    # tempdata_ini = tempdata_ini.set_index(['STNID','dates'])

    # # print(source["sim"],len(tempdata_ini))
    # index_intersect = index_intersect.intersection(tempdata_ini.index)
    # print(len(tempdata_ini.index))
    ...
    for istation,current_station in run_stations.iterrows():
        ...
        print( 'chunk' in records_morning.index.names)
        print(records_morning)

        skip_chunk = False
        fn_morning = args.path_forcing+'/'+format(current_station.name,'05d')+'_'+args.subset_forcing+'.yaml'
        has_chunks = not os.path.isfile(fn_morning)
        if has_chunks:
        ...
        if os.path.isfile(fn_morning):
            ...
            fn_afternoon = \
                 args.path_forcing+'/'+format(current_station.name,'05d')+\
                 '_'+str(run_station_chunk)+'_end.yaml'
        ...
        print(args.global_chunk_number,current_station.name,run_station_chunk,)

        if not skip_chunk:

            fn_morning = \
                 args.path_forcing+'/'+format(current_station.name,'05d')+\
                 '_'+str(run_station_chunk)+'_ini.yaml'
            fn_morning_nc = fn_morning[:-4]+'nc'
            nc_morning = nc4.Dataset(fn_morning_nc,'w')
            nc_morning.createDimension('z',501)
            nc_morning.createVariable('z','f',('z',))
            z_co = np.arange(0,5010.,10)
            nc_morning.variables['z'][:] = z_co
            nc_morning.createDimension('time',len(records_morning_station_chunk))
            ...
            fn_afternoon = \
                 args.path_forcing+'/'+format(current_station.name,'05d')+\
                 '_'+str(run_station_chunk)+'_end.yaml'
            fn_afternoon_nc = fn_afternoon[:-4]+'nc'
            nc_afternoon = nc4.Dataset(fn_afternoon_nc,'w')
            nc_afternoon.createDimension('z',501)
            nc_afternoon.createVariable('z','f',('z',))
            z_co = np.arange(0,5010.,10)
            nc_afternoon.variables['z'][:] = z_co
            nc_afternoon.createDimension('time',len(records_morning_station_chunk))
            # for ncvar in ['air_ap_z','air_ap_theta','air_ap_q','air_ap_u','air_ap_v']:
            #     nc_afternoon.createVariable(ncvar,'f',('time','z',))

            print(len(records_morning_station_chunk))
            vertvars = ['theta','q','u','v','p',]
            for ncvar in vertvars:
                nc_afternoon.createVariable('air_ap_'+ncvar,'f',('time','z',))
                nc_morning.createVariable('air_ap_'+ncvar,'f',('time','z',))
            ...
            for (STNID,chunk,index),record_morning in records_morning_station_chunk.iterrows():
                    ...
                    print('reading ',file_afternoon, int(record_afternoon.index_start), int(record_afternoon.index_end),)
                    ...
                    for ncvar in vertvars:
                        ...
                        try:
                            nc_afternoon.variables['air_ap_'+ncvar][isim] = \
                                        np.interp(z_co,\
                                                  c4gli_afternoon.air_ap.z,\
                                                  c4gli_afternoon.air_ap[ncvar])
    
                            nc_morning.variables['air_ap_'+ncvar][isim] = \
                                        np.interp(z_co,\
                                                  c4gli_morning.air_ap.z,\
                                                  c4gli_morning.air_ap[ncvar])
                        except:
                            print('interpolation failed. Something was wrong with the profile')
    
                    # c4gli_morning.update(source='EXT_EXP',ts=pd.DataFrame({'wthetae':list(c4gli_afternoon.out.wthetae), 'wqe':list(c4gli_afternoon.out.wqe), }))
                    ...
                    # # if args.timeseries_forcing is not None:
                    # #     record_afternoon = records_afternoon.loc[(STNID,chunk,index)]
                    # #     c4gli_afternoon = get_record_yaml(file_afternoon, 
                    # #                                       int(record_afternoon.index_start),
                    # #                                       int(record_afternoon.index_end),
                    # #                                     mode='model_output')
                    # #     stop
                    #     # c4gli_morning.update(source='ext_exp',time_series=pd.DataFrame
                    #     #                     
                    #     #                     
                    #     #                     } 
