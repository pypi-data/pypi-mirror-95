...
def execute(**kwargs):
    ...
    EXP_DEFS  =\
    {
      ...
      'BASE_ITER_SHR_ADV_EFERA5_SMHWB_ORI':{'sw_ac' : ['adv',],'sw_ap': True,'sw_lit': False,  'sw_shearwe': True },
      'BASE_ITER_SHR_ADV_EFERA5_SMHWB_LOW':  {'sw_ac' : ['adv',],'sw_ap': True,'sw_lit': False,'sw_shearwe': True },
      'BASE_ITER_SHR_ADV_EFERA5_SMHWB_HIG':  {'sw_ac' : ['adv',],'sw_ap': True,'sw_lit': False,'sw_shearwe': True },
      'BASE_ITER_SHR_ADV_EFERA5_SMHWB_G90':  {'sw_ac' : ['adv',],'sw_ap': True,'sw_lit': False,'sw_shearwe': True },
      'BASE_ITER_SHR_ADV_EFERA5_SMHWB_GE19':  {'sw_ac' : ['adv',],'sw_ap': True,'sw_lit': False,'sw_shearwe': True },
      'BASE_ITER_SHR_ADV_EFERA5_SMHWB_L90':  {'sw_ac' : ['adv',],'sw_ap': True,'sw_lit': False,'sw_shearwe': True },
      'BASE_ITER_SHR_ADV_EFERA5_SMHWB_Q50':  {'sw_ac' : ['adv',],'sw_ap': True,'sw_lit': False,'sw_shearwe': True},
      'BASE_ITER_SHR_ADV_EFERA5_SMHWB_MED':  {'sw_ac' : ['adv',],'sw_ap': True,'sw_lit': False,'sw_shearwe': True},
      'BASE_ITER_SHR_ADV_EFERA5_SMHWB_WFC':  {'sw_ac' : ['adv',],'sw_ap': True,'sw_lit': False,'sw_shearwe': True},
      'BASE_ITER_SHR_ADV_EFERA5_SMHWB_WLT':  {'sw_ac' : ['adv',],'sw_ap': True,'sw_lit': False,'sw_shearwe': True},
    ...
    # records_afternoon['dates'] = records_morning['ldatetime'].dt.date
    ...
    for iexpname,expid in enumerate(experiments):
        ...
        for istation,current_station in run_stations.iterrows():
            ...
            records_afternoon_station = records_afternoon.loc[(current_station.name):(current_station.name)]

            for varkey in ['h','theta']:
                records_morning_station['d'+varkey+'dt'] = \
                        (records_afternoon_station[varkey] - records_morning_station[varkey])/\
                        (records_morning.runtime*records_morning.dt)*3600.
            # print(records_morning_station[['ldatetime','h','theta','dhdt','dthetadt','runtime']])
            # print(records_afternoon_station[['ldatetime','h','theta']])
            # print(records_morning.runtime)
            ...
            dat = records_morning_station.query('(ldatetime.dt.month == midsummermonth) or (ldatetime.dt.month == (midsummermonth+1)) or (ldatetime.dt.month == (midsummermonth+2))')

            wg_ge90 = dat[(dat.blptb_daymean >= (273.15+19.) ) & (dat.blptb_daymean >= dat.blptb_daymean_ge90)].wg.median()
            wg_ge19 = dat[(dat.blptb_daymean >= (273.15+19.) )].wg.median()
            print(wg_ge90)
            if np.isnan(wg_ge90):
                stop
            # wg_ge90 = dat[(dat.blptb_daymean >= dat.blptb_daymean.quantile(0.90)) & (dat.blptb_daymean >= (273.15+19.))].wg.median()
                # wg_lt90.append(dat[dat.blptb_daymean < dat.blptb_daymean.quantile(0.90)].wg.median())
                # wg_q10.append(dat[dat.blptb_daymax <=  dat.blptb_daymax.quantile(0.10)].wg.median())
                # wg_q50.append(dat[(dat.blptb_daymax >=  dat.blptb_daymax.quantile(0.45)) & (dat.blptb_daymax <=  dat.blptb_daymax.quantile(0.55))].wg.median())
            wg_median = dat.wg.median()
                # wg_q75.append(dat[dat.blptb_daymax >= dat.blptb_daymax.quantile(0.75)].wg.median())
                # wg_q25.append(dat[dat.blptb_daymax <=  dat.blptb_daymax.quantile(0.25)].wg.median())
            ...
            if not skip_chunk:
                ...
                for (STNID,chunk,index),record_morning in records_morning_station_chunk.iterrows():
                        ...
                        if expname[-4:] == "_GE19": 
                            ...
                            c4gli_morning.update(source=expname,pars={'wg' : wg_ge19})
                            c4gli_morning.update(source=expname,pars={'w2' : wg_ge19})
                        if expname[-4:] == "_G90": 

                            c4gli_morning.update(source=expname,pars={'wg' : wg_ge90})
                            c4gli_morning.update(source=expname,pars={'w2' : wg_ge90})
                        ...
                        if expname[-4:] == "_MED": 
                            c4gli_morning.update(source=expname,pars={'wg' : wg_median})
                            c4gli_morning.update(source=expname,pars={'w2' : wg_median})

                        if expname[-4:] == "_WFC": 

                            c4gli_morning.update(source=expname,pars={'wg' : c4gli_morning.pars.wfc})
                            c4gli_morning.update(source=expname,pars={'w2' : c4gli_morning.pars.wfc})

                        if expname[-4:] == "_WLT": 

                            c4gli_morning.update(source=expname,pars={'wg' : c4gli_morning.pars.wwilt})
                            c4gli_morning.update(source=expname,pars={'w2' : c4gli_morning.pars.wwilt})
