...
parser.add_argument('--path_output',
                    help='output directory in which the output is stored')#,default='/user/data/gent/gvo000/gvo00090/D2D/data/C4GL/')
...
if not cleanup:
    if os.path.exists(batch_args.path_output):
        print("Output directory already exists: "+batch_args.path_output+". ")
        odir_exists = True
if odir_exists:
    ...
    if cleanup:
        if os.path.exists(batch_args.path_output):
            print("Warning! Output directory '"+batch_args.path_output+"' exists! I'm removing it in 10 seconds!' Press ctrl-c to abort.")
            sleep(10)
            os.system("rm -R "+batch_args.path_output)
    if batch_args.multi_processing_mode == 'qsub':
        ...
        for iargkey,argkey in enumerate(batch_args.__dict__.keys()):
            if ((argkey not in ['multi_processing_mode','cpu_count','pbs_string','cleanup_output_directories']) and \
                ...
                (batch_args.__dict__[argkey] is not None)
                    ...
                    if iargkey == 0:
                        command +=' -v C4GLJOB_'+argkey+'='+str(batch_args.__dict__[argkey])
                    else:
        ...
        for key in ['exec','multi_processing_mode','cpu_count','pbs_string','cleanup_output_directories']:
