...
if args.make_figures:
    ...
    for varkey in ['h','theta','q']:                                                    
        ...
        # if varkey in ['h']:
        #     dias[varkey]._ax.set_ylim((0.3, 0.8))
        #     dias[varkey]._ax.set_xlim((None, 0.6))
        #     #dias[varkey]._ax.set_xlim((None, 1.512789250011835))
        # elif varkey in ['theta']:
        #     dias[varkey]._ax.set_ylim((0.3, 0.8))
        #     dias[varkey]._ax.set_xlim((0.4, 1.0))
        # else:
        #     dias[varkey]._ax.set_ylim((0.6, 1.3))
        #     dias[varkey]._ax.set_xlim((0.2, 1.0))

        #dias[varkey]._ax.set_ylim((0.30017906582982024, 1.512789250011835))
        # dias[varkey].ax.set_xlim((None, 1.512789250011835))
        # dias[varkey]._ax.set_xlim((None, 1.312789250011835))
        # dias[varkey]._ax.set_ylim((None, 1.312789250011835))
        # dias[varkey]._ax.set_xlim((None, 1.512789250011835))
        i +=1
    ...
    for ikoepp,koepp in koeppenlookuptable_sel.iterrows():
        xy_box = ((0.059+np.floor(idx/4)*0.325),0.127- 0.09*((np.mod(idx,4)/(len(koeppenlookuptable_sel)/4))))
        xy_text = list(xy_box)
        xy_text[0] += 0.009
        xy_text[1] -= 0.00
    ...
    i = 1
    for varkey in ['h','theta','q']:                                                    
        darkcolors = ['darkgreen','saddlebrown']
        lightcolors = ['green','brown']
        for ikey,key in enumerate((args.experiments.strip().split(' '))):
            ...
            axes[varkey].contour(xi, yi, zi_int.reshape(xi.shape),levels=[0.25,0.5,0.75] ,
                    colors=[darkcolors[ikey],lightcolors[ikey],darkcolors[ikey]],linewidths=[1,2,1])
            axes[varkey].contourf(xi, yi, zi_int.reshape(xi.shape),levels=[0.25,0.75] ,
                    colors=[darkcolors[ikey]],alpha=0.5,)
       ...
       #      c4gldata[key].frames['stats']['records_all_stations_ini']['KGCname']
            ...
            if ikey == 0:
                axes[varkey].scatter(obs,mod, label='All',s=0.1,alpha=0.07,color='g')
            ...
            if ikey == 0:
                ann = axes[varkey].annotate('ERA5:\n'+annotate_text, xy=(0.05, .98 ), xycoords='axes fraction',fontsize=9, horizontalalignment='left', verticalalignment='top',color='darkgreen' )
            elif ikey == 1:
                ann = axes[varkey].annotate('ERA-I:\n'+annotate_text, xy=(0.40, .27 ), xycoords='axes fraction',fontsize=9, horizontalalignment='left', verticalalignment='top',color='saddlebrown' )
    ...
    if args.figure_filename is not None:
        fig.savefig(args.figure_filename,dpi=500); print("Image file written to:",args.figure_filename)
