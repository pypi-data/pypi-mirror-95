...
def session_scoped(method: typing.Callable) -> typing.Callable:
    ...
    # TODO martin.slouf 2021-02-20 -- remove global variable
    session_scoped.session_factory = None

    @functools.wraps(method)
    def _session_scoped(*args, **kwargs) -> object:
        assert isinstance(session_scoped.session_factory, SessionFactory)
        # reuse the existing session or create a new one
        session = session_scoped.session_factory.session
        try:
