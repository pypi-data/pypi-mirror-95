    ...
    def likelihood(self, inf_params):
        ...
        if self._params['K'] == 1:
            for i in range(self._data['data'].shape[0]):
                lk[1, :] += torch.log(inf_params["mixture_weights"]) + dist.Poisson(inf_params["mixture_weights"]).log_prob(self._data['data'][i,:])
            return lk
        ...
        for k in range(self._params['K']):
            ...
            for i in range(self._data['data'].shape[0]):
                ...
                lk[k, :] += torch.log(inf_params["mixture_weights"][k]) + dist.Poisson(lamb).log_prob(self._data['data'][i,:])
        return lk

    def calculate_cluster_assignements(self,inf_params):
        lk = self.likelihood(inf_params)
        summed_lk = log_sum_exp(lk)
        res = {"assignment_probs" : torch.exp(lk - summed_lk).detach().numpy()}
        return res
