print("hello from snir's package 0.1.5")
...
directory = f"{site.getsitepackages()[1]}\\snir\\"
reset = '\033[0m'
...
def startup_sound():
    playsound(directory + "startup.wav", block=True)
...
def success_sound():
    playsound(directory + "success.wav", block=True)


def click_sound():
    playsound(directory + "first_click.wav", block=True)


def click_sound1():
    playsound(directory + "second_click.wav", block=True)
