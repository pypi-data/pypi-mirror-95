...
unary_classes = (numpy_ipps.AutoCorrelate,)
binary_classes = (numpy_ipps.Correlate,)
...
@pytest.mark.parametrize("dtype", numpy_ipps.policies.default_candidates)
@pytest.mark.parametrize("feature", unary_classes)
def test_ipps_convolve(logger_fixture, benchmark, order, dtype, feature):
    if dtype not in numpy_ipps.Convolve.dtype_candidates:
        return

    feature_obj = numpy_ipps.Convolve(size=1 << order, dtype=dtype)
    src1 = (1 + numpy.random.rand(1 << order)).astype(dtype)
    src2 = (1 + numpy.random.rand(1 << order)).astype(dtype)
    dst = numpy.empty((1 << (order + 1)) - 1, dtype=dtype)
...
@pytest.mark.parametrize("dtype", numpy_ipps.policies.default_candidates)
@pytest.mark.parametrize("feature", unary_classes)
def test_numpy_convolve(logger_fixture, benchmark, order, dtype, feature):
    if dtype not in numpy_ipps.Convolve.dtype_candidates:
        return
