...
from OpFlowLab.GUI.opticalFlow.nvidiaOptions import nvidiaOFOptions
...
class opticalFlowTab(QWidget):
    ...
    def __init__(self, parent):
        ...
        self.opticalFlowLabel = QLabel("Optical flow executable")

        self.folderLayout.addWidget(self.opticalFlowLabel, 0, 0)
        self.executablePathWidget = pathWidget(pathtype="file",
                                               defaultFolder="",
                                               filter="Executable (*.exe);;",
                                               desc="optical flow executable")
        self.folderLayout.addWidget(self.executablePathWidget, 0, 1)
    ...
    def onProcessButtonClicked(self):
        if self.thread.isRunning():
            ...
            if self.config.hasInstance():
                valid = True
                valid = valid and self.executablePathWidget.validatePath()
                if valid is True:
                    processLocation = self.executablePathWidget.getPath()
    ...
    def onTypeChange(self, value):
        ...
        if value == "NvidiaOF":
            ...
            self.messageSignal.emit("Nvidia hardware optical flow can only be used on Nvidia GPUs from the Turing generation or later", "warn")
        elif value == "PyrLK":
