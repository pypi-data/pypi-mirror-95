...
def main():
    ...
    parser.add_argument("-c", "--command", metavar="command",
                        help=("jalankan 'command' dengan argument berupa\n"
                              "url yang dipilih.\n\n"
                              "contoh: axel2 {}"))
    parser.add_argument("-d", "--debug", action="store_true", help="aktifkan mode debug")
    ...
    try:
        ...
        if id:
            ...
            key1 = extractor.choice([
                f"{k} (direct)" if isinstance(v, str) and allDirectRules.search(v) else k
