...
def recursive_choice(extractor, result):
    ...
    key1 = extractor.choice([
        f"{k} (direct)" if isinstance(
            v, str) and allDirectRules.search(v) else k
        for k, v in result.items()
    ])
    url = result[key1]
    if url.startswith("re:"):
        id = extractor.getPath(url.removeprefix("re:"))
        logging.info(f"Mengekstrak link unduhan: {id}")
        result = extractor.extract(id)
        url = recursive_choice(extractor, result)
    return url
...
def main():
    ...
    parser.add_argument("-e", "--extract-direct", help="bypass situs download", metavar="url")
    ...
    parser.add_argument("-c", "--command", metavar="command",
                        help=("jalankan 'command' dengan argument berupa\n"
                              "url yang dipilih.\n\n"
                              "contoh: axel2 {}"))
            ...
            for index in range(1, len(egn)):
                try:
                    ...
                    extractor.add_argument(*arg, action="store_true",
                                           help=f"site: {pa.scheme}://{pa.netloc}")
                    break
