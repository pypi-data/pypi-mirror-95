    ...
    if photoZ is None:  # TODO: Get correct redshift
        try:
            if "z=" in tns_info:
                ...
                # return
                print("Not using redshift. TRYING ARBITRARY GUESS REDSHIFT = 0.1")
                redshift = None# 0.1  # return  0.019 #
...
def classify_lasair_light_curves(object_names=('ZTF18acsovsw',), plot=True, figdir='.', date=''):
    light_curve_list = []
    ...
    ras = []
    decs = []
    peakmags_g, peakmags_r, peakmjds = [], [], []
    for object_name in object_names:
        ...
        peakmjds.append(mjd[np.argmax(flux)])
    ...
    for objidx in range(len(predictions)):
        for i, name in enumerate(classification.class_names):
            if name == 'TDE':
                ...
                maxtime = (peakmjds[objidx] - classification.trigger_mjds[objidx] - 5) / (1 + redshifts[objidx])
                try:
                    prob_at_max = predictions[objidx][:,i][int(maxtime / 3)]
                except IndexError:
                    prob_at_max = predictions[objidx][:,i][-1]
                # maxpbflux_idx = classification.X[objidx].max(axis=0)[0:2].argmax()
                # maxfluxidx = classification.X[objidx].argmax(axis=0)[maxpbflux_idx]
                # maxtime = classification.timesX[objidx][maxfluxidx]
                # prob_at_max = predictions[objidx][:,i][maxfluxidx]

                prob = max(predictions[objidx][:,i])  # prob_at_max  #
                ...
                objids.append(object_name)
                probabilities.append(prob)
                radec.append((ras[objidx], decs[objidx]))
                peakmags.append((peakmags_g[objidx], peakmags_r[objidx]))
    print(radec)
    np.savez(f'plot_nuclear_transients_{date}', real_tde_probs=np.array(real_tde_probs), probabilities=np.array(probabilities), names=np.array(names), radec=np.array(radec), peakmags=np.array(peakmags), objids=np.array(objids))

    high_prob_dir = os.path.join(figdir, f'high_prob_{date}')
    if not os.path.exists(high_prob_dir):
        os.makedirs(high_prob_dir)
    classification.plot_light_curves_and_classifications(indexes_to_plot=np.where(np.array(probabilities) > 0.45)[0], step=True, use_interp_flux=False, figdir=high_prob_dir, plot_matrix_input=True)
    ...
    sns.set_style("white")
    print(probabilities)
    sns.distplot(probabilities, color='dodgerblue', label=f'Nuclear Transients in past 30 days (from {date})')
    plt.xlim(0, 1)
...
if __name__ == '__main__':
    ...
    date = '181119-ignore-non-detections'
    import urllib
    f = urllib.request.urlopen('https://lasair.roe.ac.uk/lasair/static/ztf/streams/AllnucleartransientsandTDEcandidates')
    myfile = f.read().decode('utf-8')
    nuclear_transients = np.unique([row.split(',')[0].replace('"', '') for row in myfile.split('"objectId": ')])
    figdir = f'nuclear_transients_{date}'
    ...
    # classify_lasair_light_curves(object_names=nuclear_transients, figdir=figdir, date=date)

    nuc_tran_plot = np.load(f'plot_nuclear_transients_{date}.npz')
    ...
    np.savetxt(f'TDE_candidates_{date}.csv', np.hstack([radecs, peakmags, np.array([probabilities, objids]).T])[probabilities > 0.45], fmt='%s')
    ...
    sns.set_style("white")
    print(probabilities)
    sns.distplot(probabilities, color='dodgerblue', label=f'Nuclear Transients in past 30 days (from {date})', kde=False, norm_hist=False)
    plt.xlim(0, 1.)
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(f'prob2_unnormalised_{date}.pdf')
    ...
    plt.xlim(0., 1.0)
    # plt.ylim(0, 200)
    plt.xlabel("TDE Classification probability at peak brightness, $p$", fontsize=15)
    plt.ylabel("Number of objects with at least $p$ probability", fontsize=15)
    plt.tight_layout()
    plt.savefig(f'cumulativeTDE_{date}.pdf')

    plt.figure()
    for i in range(len(real_tde_probs)):
        plt.text(real_tde_probs[i]-0.02, 50, names[i], rotation=90, color='red')
        plt.axvline(x=real_tde_probs[i], linewidth=0.8, label=names[i], color='dodgerblue')
    plt.hist(probabilities, 100, cumulative=-1, color='dodgerblue', histtype='step', linewidth=1.5)
    plt.xlim(0.3, 1.0)
    plt.ylim(0, 70)
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(f'cumulativeTDE_{date}_zoomed.pdf')



    # plot lots of cumulative
    ...
    for i in range(len(real_tde_probs)):
        # plt.text(real_tde_probs[i]-0.02, 50, names[i], rotation=90, color='red')
        if i == 0:
            plt.axvline(x=real_tde_probs[i], linewidth=1, color='dodgerblue', label='At peak brightness')
        else:
            plt.axvline(x=real_tde_probs[i], linewidth=1, color='dodgerblue')
    plt.hist(probabilities, 100, cumulative=-1, color='dodgerblue', histtype='step', linewidth=1.5)

    date = 'last-year-5days-pre-peakprob'
    nuc_tran_plot = np.load(f'plot_nuclear_transients_{date}.npz')
    real_tde_probs = nuc_tran_plot['real_tde_probs']
    probabilities = nuc_tran_plot['probabilities']
    names = nuc_tran_plot['names']
    ...
    for i in range(len(real_tde_probs)):
        # plt.text(real_tde_probs[i]-0.02, 50, names[i], rotation=90, color='red')
        if i == 0:
            plt.axvline(x=real_tde_probs[i], linewidth=1, color='tab:red', label='5 days before peak')
        else:
            plt.axvline(x=real_tde_probs[i], linewidth=1, color='tab:red')
    plt.hist(probabilities, 100, cumulative=-1, color='tab:red', histtype='step', linewidth=1.5)
    ...
    plt.xlim(0.3, 1.0)
    plt.ylim(0, 70)
    plt.xlabel("TDE Classification probability, $p$", fontsize=17)
    plt.ylabel("Number of objects with at least $p$ probability", fontsize=17)
    plt.legend(frameon=True, prop={'size': 17})
    plt.tight_layout()
    plt.savefig(f'cumulativeTDE_zoomed_all.pdf')
