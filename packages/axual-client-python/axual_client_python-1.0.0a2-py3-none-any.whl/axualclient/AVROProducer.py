...
class AxualAVROProducer:
    ...
    def __init__(self,
        ...
        *args and **kwargs :
        ...
        schema_registry_client = SchemaRegistryClient({'url': disco['schema.registry.url'],
                                                       'ssl.ca.location': app_config.ssl_config.root_ca_location})

        if os.path.isfile(schema_value):
            with open(schema_value, "r") as f:
                schema_str = f.read()
        else:
        ...
        if schema_key is not None:
            if os.path.isfile(schema_key):
                with open(schema_key, "r") as f:
                    schema_key_str = f.read()
            else:
                schema_key_str = schema_key
            avro_key_serializer = AvroSerializer(schema_key_str,
                                                 schema_registry_client,
                                                 key_dict,
                                                 # Converts input to dict, but should not be needed when supplying dicts
                                                 conf={'auto.register.schemas': False}
            ...
            avro_key_serializer = StringSerializer('utf-8')
        ...
        self._producer = SerializingProducer(self.configuration, *args, **kwargs)
    ...
    def produce(self, *args, callback=None, **kwargs):
        ...
        *args and **kwargs :
        ...
        while self.is_switching:
            sleep(1)
        return self._producer.produce(self.topic, *args, on_delivery=callback, **kwargs)
    ...
    def abort_transaction(self, *args, **kwargs):
        return self._producer.abort_transaction(*args, **kwargs)

    def begin_transaction(self, *args, **kwargs):
        return self._producer.begin_transaction(*args, **kwargs)

    def commit_transaction(self, *args, **kwargs):
        return self._producer.commit_transaction(*args, **kwargs)

    def flush(self, *args, **kwargs):
        return self._producer.flush(*args, **kwargs)

    def init_transactions(self, *args, **kwargs):
        return self._producer.init_transactions(*args, **kwargs)

    def list_topics(self, *args, **kwargs):
        return self._producer.list_topics(*args, **kwargs)

    def poll(self, *args, **kwargs):
        return self._producer.poll(*args, **kwargs)

    def send_offsets_to_transaction(self, *args, **kwargs):
        return self._producer.send_offsets_to_transaction(*args, **kwargs)
