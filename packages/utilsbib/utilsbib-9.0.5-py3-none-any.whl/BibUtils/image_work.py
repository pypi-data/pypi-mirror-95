...
class Face_cropper:
    ...
    def isolate_mouth_cv(self, filename, mouth_only=True, preImg=None, pre_shape=None, crop_lips=False):
        pos_origin = []
        try:
            if preImg is None:
                img = cv2.imread(filename)
...
def add_makeup_check(file, face_cropper, m_type=0, col=None):
    ...
    #img_cv = np.array(img)
    #img_cv = img_cv[:, :, ::-1].copy()
    result_rgb = img#cv2.cvtColor(img, cv2.COLOR_RGB2BGR)
    return original_open_cv_image, result_rgb
