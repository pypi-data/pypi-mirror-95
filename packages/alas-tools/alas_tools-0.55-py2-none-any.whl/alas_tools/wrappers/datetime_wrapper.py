...
from datetime import datetime as otherdatetime
...
class DatetimeWrapper(datetime.datetime):
    ...
    def get_today(country_code='cl'):
        local_now = DatetimeWrapper.utc_to_tz(
            date.today(), country_code
        )
    ...
    def get_now(country_code='cl'):
        local_now = DatetimeWrapper.utc_to_tz(
            otherdatetime.utcnow(), country_code
        )
    ...
    def datetime_to_str(dt, format=None):
        return DatetimeWrapper.datetime_to_str(dt, format)

    @staticmethod
    def get_timezones_from_country(country_code):
        _filter = filter(lambda x: x['code'] == country_code, countries_timezones)
        return _filter[0]['timezones'] if len(_filter) > 0 else None

    @staticmethod
    def utc_to_tz(utc_dt, country_tz):
        timezones = DatetimeWrapper.get_timezones_from_country(country_tz)
        return pendulum.instance(utc_dt).in_timezone(timezones[0]) if timezones else utc_dt

    @staticmethod
    def str_to_datetime(dt_str, format=None):
        return DatetimeWrapper.strptime(dt_str, '%Y-%m-%dT%H:%M:%S' if not format else format)

    @staticmethod
    def datetime_to_str(dt, format=None):
        return dt.strftime('%Y-%m-%d %H:%M:%S' if not format else format)
