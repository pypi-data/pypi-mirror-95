...
# argument functions.
def argument_present(arguments, default=False):
...
def arguments_present(arguments, default=False):
	if isinstance(arguments, str): return argument_present(argument, default=default)
	else:
		for argument in arguments:
			if argument_present(argument, default=default):
				return True
		return default
...
class CLI(object):
	...
	class Arguments(object):
		...
		def iterate(self):
			return sys.argv
		def present(self, argument, default=False):
			if isinstance(argument, str):
				return argument_present(argument, default=False)
			else:
				return arguments_present(argument, default=False)
		def get(self, 
			...
			if isinstance(argument, str):
				...
				for x in sys.argv:
					...
					try:
						if x == argument: 
							...
							# check format.
							if "*" not in format:
								if Formats.get(value, serialized=True) not in formats and Formats.get(value, serialized=False) not in formats:
									format_error = f"Provided an incorrect [{argument}] format: [{Formats.get(value, serialized=True)}], valid format options: {format}."
									if json:
				...
				if pack:
					arguments = []
					for i in argument: 
						arguments.append(self.get(i, required=required, index=index,default=default, mode=mode, chapter=chapter, notes=notes, json=json))
					return arguments	
				else:
					for i in argument: 
						a = self.get(i, required=required, index=index,default=default, mode=mode, chapter=chapter, notes=notes, json=json)
						if a != default: return a
					return default
		def check(self, exceptions=["--log-level", "--create-alias"], json=False):
