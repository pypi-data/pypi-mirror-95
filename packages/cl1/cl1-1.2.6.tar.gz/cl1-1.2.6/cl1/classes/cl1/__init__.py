...
class CLI(object):
	...
	class Arguments(object):
		...
		def get(self, argument, required=True, index=1, pack=True, default=None, ):
			if isinstance(argument, str):
				return get_argument(argument, required=required, index=index,default=default)
			else:
				if pack:
					arguments = []
					for i in argument: 
						arguments.append(get_argument(i, required=required, index=index,default=default))
					return arguments	
				else:
					for i in argument: 
						a = get_argument(i, required=required, index=index,default=default)
