...
def arguments_present(arguments, default=False, count=1):
	if isinstance(arguments, str): return argument_present(argument, default=default, count=count)
	else:
		for argument in arguments:
	...
	class Arguments(object):
		...
		def get(self, 
			...
			def clean_string(string="", remove_first_space=False, remove_last_space=False):
				while True:
					if remove_first_space and len(string) > 0 and string[0] == " ": string = string[1:]
					...
					try:
						if x == argument: 
							...
							if c == count:
								...
								# check format.
								if "*" not in format:
									x_format = Formats.get(value, serialized=False)
									error = False
									...
									if error or x_format not in formats:
