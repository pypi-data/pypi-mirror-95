...
class NextPictureManager:
    ...
    def get_next_picture(self):
        ...
        if os.path.exists(cur_pic.absolute_path):
            ...
            log.debug(f"{cur_pic.absolute_path} chosen to be shown as next picture.")
            return cur_pic
        else:
    ...
    def _load_pictures(self) -> List[PictureData]:
        ...
        if not self.config.shuffle:
            ...
            log.debug(
                f"The first 100 pictures out of {len(sample_list)} are "
                f"{[x.absolute_path for x in sample_list[:100]]}."
            )
        return sample_list
