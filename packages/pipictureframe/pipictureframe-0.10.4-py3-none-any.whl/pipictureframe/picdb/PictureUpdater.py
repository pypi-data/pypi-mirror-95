...
def _clean_db(session) -> bool:
    ...
    for count, pic in enumerate(all_pics, 1):
        ...
        if count % 1000 == 0:
            log.info(f"Checked {count} db entries for missing hdd files.")
...
def _add_and_update_pics(pic_file_gen, session) -> bool:
    ...
    for count, pic_file in enumerate(pic_file_gen, 1):
        if count % 1000 == 0:
            log.info(f"Checked {count} files for necessary db updates.")
        try:
            ...
            session.rollback()
        log.info(f"{num_changed} pics added or updated to/in db.")
