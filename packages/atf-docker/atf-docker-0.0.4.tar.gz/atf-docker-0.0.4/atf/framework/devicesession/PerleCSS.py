...
class PerleCSS(ConsoleServerSession):
    r"""
    The PerleCSS class provides support for Perle console servers.
    """
    ###
    # Class Definitions
    ###
    ...
    # Regex Patterns
    (REGEX_TELNET_BUSY) = "Selected hunt group busy"
    """Pattern expected on a telnet busy state."""
    ...
    def __init__(self,  deviceSessionConfig=None, diagLevel=logging.WARNING):
        ...
        # Perle console connections do not support SSH
        if (self.config != None and
            ...
            raise (FrameworkError(DeviceSession.ERR_FAILED_SESSION_CONNECT,
                'An SSH connection type is not supported for the Perle')) 
        
        # Perle console connections are mapped ports, adjust config port
        try:
            if (self.config != None):
                self.config.port = '%d' % \
            ...
            raise (FrameworkError(DeviceSession.ERR_FAILED_SESSION_CONNECT,
                'Perle port and consolePort not well defined')) 
    ...
    def connect(self):
        ...
        try:
            ...
            self.tty.expect(PerleCSS.REGEX_TELNET_BUSY,
                            timeout=PerleCSS.CONNECT_TIMEOUT)
        ...
        try:
            self.tty.expect(DeviceSession.REGEX_TELNET_CONNECTSTRING,
                            timeout=PerleCSS.EXPECT_TIMEOUT)
