...
from typing import (
    Any, Callable, Dict, Iterable, List, Optional, Set, Tuple, Type, cast)
...
class BoxLevel(Level):
    ...
    * :meth:`~earwax.BoxLevel.describe_current_box`
    ...
    def walls_between(self, end: Point[int], start: Optional[Point[int]] = None) -> int:
        """Return the number of walls between the two points.

        :param end: The target coordinates.

        :param start: The coordinates to start at.

            If this value is ``None``, the current coordinates will be used.
        """
        walls: int = 0
        if start is None:
            start = self.coordinates.floor()
        boxes: Set[Box[Any]] = set()
        while start != end:
            box: Optional[Box[Any]] = self.get_containing_box(start)
            if box not in boxes:
                if box.is_wall(start):
                    walls += 1
                boxes.add(box)
        return walls
