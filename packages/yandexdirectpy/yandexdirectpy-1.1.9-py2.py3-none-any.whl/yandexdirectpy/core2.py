...
#     # Redirect user to webpage for getting a token and returns
#     global TOKEN
#
...
def send_api_request(query_body, headers: dict, field_names: list, current_login: str, skip_errors: bool = True):
    ...
    if response.status_code == 200:
        df_new = pd.read_csv(StringIO(
            str(response.content, 'utf-8')), sep='\t', names=field_names, dtype=data_types)
...
def get_report_fun(
    ...
    for current_login in login:
        ...
        data = send_api_request(
            query_body, headers, field_names, cur_token, skip_errors=skip_errors)
...
def get_campaign_list(
    ...
    if statuses is None:
        states = ["ACCEPTED", "DRAFT", "MODERATION", "REJECTED"]
