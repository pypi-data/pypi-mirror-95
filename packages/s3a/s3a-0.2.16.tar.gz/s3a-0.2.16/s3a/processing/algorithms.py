...
def cv_template_match(template: np.ndarray, image: np.ndarray, viewbox: np.ndarray,
  ...
  # for rot in ((cv.ROTATE_90_CLOCKWISE, cv.ROTATE_90_COUNTERCLOCKWISE),
  #             (cv.ROTATE_90_COUNTERCLOCKWISE, cv.ROTATE_90_CLOCKWISE),
  #             (cv.ROTATE_180, cv.ROTATE_180)):
  #   out = cv.matchTemplate(grayImg, cv.rotate(template, rot[0]), metric)
  #   res += cv.rotate(out, rot[1])
