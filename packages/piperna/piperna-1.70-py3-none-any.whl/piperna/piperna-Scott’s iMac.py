    ...
    def unbam_executable(self):
        ...
        for sample in self.runsheet_data:
            if self.runmode=="single":
                ...
                fastqline = """-fq %s -fg2 """% (sample['fastq1'], sample['fastq2'])
            commandline = """\nsamtools sort -n -@ %s -m %s %s | bedtools -i - %s""" % (self.threads, self.ram+'G', sample['bam'], fastqline)
            #print(commandline.__class__.__name__)
            command.append([sample['sample'], commandline])
        return command
...
def unbam_prep(folder, output, typeofseq, organized_by):
    ...
    fastq1 = [os.path.join(re.sub(folder, output, os.path.dirname(file)), re.sub(".bam", "_R1.fastq.gz", os.path.basename(file))) for file in dat]
    if(typeofseq=="pe"):
        fastq2 = [os.path.join(re.sub(folder, output, os.path.dirname(file)), re.sub(".bam", "_R2.fastq.gz", os.path.basename(file))) for file in dat]
