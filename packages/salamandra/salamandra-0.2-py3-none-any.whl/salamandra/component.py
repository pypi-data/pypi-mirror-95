...
def sort_(pattern=None, addition=None, is_str=True):
    # return key to sort, such as subcomponents names as ABC123ABC123 (for default)
    if not pattern:
        pattern = r'([^\d]+)(\d+)([^\d]+)(\d+)'
    if not addition:
        addition = '0_0'
    ...
    if is_str:
        key = lambda t: my_fun(*re.match(pattern, t + addition).groups())
    else:
        key = lambda t: my_fun(*re.match(pattern, t.get_object_name() + addition).groups())
    return key
...
class Component(Object):
    ...
    def get_descendants_hierarchy(self):
        ...
        for inst, dev in self.get_subcomponents():
            dic_of_subcomp = dev.get_descendants_hierarchy()
    ...
    def uniq(self, count=None, numbering=None):
        ...
        for inst in sorted(self.subcomponent_names(), key=sort_()):
    ...
    def legalize(self):
        ...
        instance_not_existed = self.check_if_instances_not_exist()
        if instance_not_existed:
            raise Exception('cant find component "{}"'.format(instance_not_existed))
    ...
    def check_pins_nets_names_asymmetry(self):
        ...
        for str_, obj in list(self.__pins.items())+list(self.__nets.items())+list(self.__pinbusses.items())+list(self.__netbusses.items()):
    ...
    def check_if_instances_not_exist(self):
        try:
            ...
            if not e.args[0][:19] == 'cant find component':
                raise
            return e.args[0][22:]
    ...
    def print_verilog(self, include_subcomponents=False):
        if include_subcomponents:
            for comp in self.get_descendants(inclusive=True):
    ...
    def write_verilog_to_file(self, path, include_subcomponents=False):
    ...
    def write_verilog(self):
        ...
        ports = ', '.join(sorted(self.verilog_port_list(), key=sort_()))
        mod_def += ports + ');'
        ...
        for pin in sorted(self.get_pins(), key=sort_(is_str=False)):
        ...
        for net in sorted(self.get_nets(filter=lambda n: not isinstance(n, GNet)), key=sort_(is_str=False)):
                    ...
                    for inst_pin in inst_bus.all_bits():
                        ...
                        if (inst, inst_pin_name) in self.__virtual_pins:
                            ...
                            if self.__is_inst_bus_connected_any(inst, inst_bus):
                                raise Exception(inst+Component.PIN_SEPARATOR+inst_pin_name+' is '+inst_pin.verilog_type()
    ...
    def print_netlist(self):
        for line in self.write_netlist():
            print(line)

    def write_netlist_to_file(self, path):
        with open(path, 'w') as f:
            for line in self.write_netlist():
                f.write(line)
                f.write('\n')
    ...
    def write_netlist(self):
        ...
        # subcircuits
        for sub in sorted(self.get_descendants(), key=sort_(is_str=False)):  # sort by names
            ...
            # instances
            for inst in sorted(sub.subcomponent_names(), key=sort_()):
                dev = sub.get_subcomponent(inst)
                l = '\t' + inst + ' ('
                ports = []
                for inst_pin in [p for p in dev.get_pins() if not p.is_part_of_bus()]:
                    inst_pin_name = inst_pin.get_object_name()
                ...
                params = []
                if dev.get_spice_params():
                    params = [p+'='+str(v) for p, v in dev.get_spice_params().items()]
                l += ' ' + (' '.join(params))
