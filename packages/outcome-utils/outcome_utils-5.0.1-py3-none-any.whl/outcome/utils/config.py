...
ValidConfigType = Union[str, int, float, bool, list]
...
class DefaultBackend(ConfigBackend):
    ...
    defaults: Dict[str, ValidConfigType]

    def __init__(self, defaults: Dict[str, ValidConfigType]):
...
class TomlBackend(ConfigBackend):
    ...
    @classmethod
    def get_config(cls, path: ValidPath, aliases: Optional[Dict[str, str]] = None) -> Dict[str, ValidConfigType]:
    ...
    @classmethod
    def flatten_keys(cls, value: object, key: Optional[str] = None) -> Dict[str, ValidConfigType]:
