...
export default function createResultManager(queryBuilder) {
    ...
    function fetchPage(pageToFetch) {
        var query = queryBuilder.getExternal();
        var baseQuery = queryBuilder.getQueryByType(query, 'baseQuery');
        var cursor = data.baseQuery[pageToFetch];
        if (cursor) {
            return queryBuilder.fetch(baseQuery, cursor).then(function (resp) {
    ...
    function fetchAll() {
        var query = queryBuilder.getExternal();
        var baseQuery = queryBuilder.getQueryByType(query, 'baseQuery');
        var byDayQuery = queryBuilder.getQueryByType(query, 'byDayQuery');
        var promises = [queryBuilder.fetch(baseQuery)];
        var hasAggregations = query.aggregations.length > 0;
...
//# sourceMappingURL=resultManager.jsx.map