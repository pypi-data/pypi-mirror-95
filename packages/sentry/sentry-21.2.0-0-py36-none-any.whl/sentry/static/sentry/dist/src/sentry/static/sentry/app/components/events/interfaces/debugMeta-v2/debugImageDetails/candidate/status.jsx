...
function Status(_a) {
    ...
    switch (download.status) {
        case CandidateDownloadStatus.OK: {
            return (<StatusTooltip label={t('Download Details')} description={location} disabled={!location || source === INTERNAL_SOURCE}>
        ...
        case CandidateDownloadStatus.MALFORMED: {
            ...
            return (<StatusTooltip label={t('Download Details')} description={details} disabled={!details}>
          <Tag type="error">{t('Failed')}</Tag>
        </StatusTooltip>);
        }
        case CandidateDownloadStatus.NOT_FOUND: {
            var details = download.details;
            return (<StatusTooltip label={<React.Fragment>
              {t('No debug file was not found at this location')}
              {':'}
            </React.Fragment>} description={<React.Fragment>
              <div>{location}</div>
              {details && <div>{details}</div>}
            </React.Fragment>} disabled={!location || source === INTERNAL_SOURCE}>
          <Tag>{t('Not Found')}</Tag>
        </StatusTooltip>);
        }
        case CandidateDownloadStatus.NO_PERMISSION: {
            var details = download.details;
            return (<StatusTooltip label={t('Permission Error')} description={details} disabled={!details}>
          <Tag type="warning">{t('Permissions')}</Tag>
        </StatusTooltip>);
        }
        case CandidateDownloadStatus.DELETED: {
            return (<StatusTooltip label={t('This file was deleted after the issue was processed.')}>
          <Tag type="success">{t('Deleted')}</Tag>
        </StatusTooltip>);
        }
        case CandidateDownloadStatus.UNAPPLIED: {
            return (<StatusTooltip label={t('This issue was processed before this debug information file was available. To apply new debug information, reprocess this issue. ')}>
