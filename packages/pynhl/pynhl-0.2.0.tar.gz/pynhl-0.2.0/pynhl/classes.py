...
class Schedule:
    ...
    def __init__(self, team_id):
        ...
        endpoint = f"api/v1/teams/{team_id}?expand=team.schedule.next"
        url = BASE + endpoint
    ...
    def datetime_info(self):
        ...
        try:
            ...
            x = games["gameDate"]
            next_game_date = dt.strptime(x, "%Y-%m-%dT%H:%M:%SZ").strftime("%B %d, %Y")
            next_game_time = dt.strptime(x, "%Y-%m-%dT%H:%M:%SZ").strftime("%H:%M")
...
class Scoring:
    ...
    def __init__(self, team_id):
        ...
        endpoint = f"api/v1/schedule?teamId={team_id}&hydrate=scoringplays"
        url = BASE + endpoint
        self.response = requests.get(url)
        self.data = self.response.json()
...
class Linescore:
    ...
    def __init__(self, team_id):
        """Returns the json data of the defined team's last/current game"""
        endpoint = f"api/v1/schedule?teamId={team_id}&expand=schedule.linescore"
        url = BASE + endpoint
        self.response = requests.get(url)
        self.data = self.response.json()
