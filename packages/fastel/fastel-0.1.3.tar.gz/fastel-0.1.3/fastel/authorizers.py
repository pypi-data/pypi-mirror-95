...
from revjwt import decode  # ignore: type

from . import exceptions


class Credential:
    def __init__(self, client: BaseDocument, user: Any = None) -> None:
        self.client = client
        self.user = user
...
class BaseAuth:
    ...
    def __or__(self, other: BaseAuth) -> BaseAuth:
...
class JWBaseAuth(BaseAuth):
    ...
    def verify(
        ...
        if self.client_model_class:
            try:
                client = self.client_model_class.objects.get(
