...
from utils.utils_vtk import write_vtk
...
def GenerateAtlas(model_gen, max_age, atlas, device, pool_mean=None):
    prior_sulc_min = -12.
    prior_sulc_max = 14.
    prior_curv_min = -1.3
    prior_curv_max = 1.0
    ...
    n_atlases = 7
    ages = np.asarray([1,90,180,270,360,540,720])/1000 * max_age
    for i in range(n_atlases):
        fixed_sulc = model_gen(torch.unsqueeze(torch.tensor([ages[i]/max_age], dtype=torch.float32, device=device), 1))
        surf = {'vertices': atlas['vertices'],
                'faces': atlas['faces'],
                'sulc': (fixed_sulc.detach().cpu().numpy()[:,0] + 1.)/2.* (prior_sulc_max-prior_sulc_min) + prior_sulc_min,
                'curv': (fixed_sulc.detach().cpu().numpy()[:,1] + 1.)/2.* (prior_curv_max-prior_curv_min) + prior_curv_min}
        if pool_mean is not None:
            surf['pool_mean'] = pool_mean.detach().cpu().numpy()*100.
        write_vtk(surf, '/media/fenqiang/DATA/unc/Data/GroupwiseReg/generated_atlas/'+ \
