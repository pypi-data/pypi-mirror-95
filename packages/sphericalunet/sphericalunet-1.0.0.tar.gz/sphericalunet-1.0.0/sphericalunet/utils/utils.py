...
def get_orthonormal_vectors(n_ver, rotated=0):
    ...
    template = read_vtk('neigh_indices/sphere_'+str(n_ver)+'_rotated_'+str(rotated)+'.vtk')
...
def get_patch_indices(n_vertex):
    ...
    """
    indices_files = sorted(glob.glob('neigh_indices/*_indices.npy'))
    weights_files = sorted(glob.glob('neigh_indices/*_weights.npy'))
    ...
    for i in range(n_vertex):
        indices_file = 'neigh_indices/'+ str(i) + '_indices.npy'
        weights_file = 'neigh_indices/'+ str(i) + '_weights.npy'
...
def get_z_weight(n_vertex, rotated=0):
    sphere = read_vtk('neigh_indices/sphere_'+str(n_vertex)+'_rotated_'+str(rotated)+'.vtk')
