...
class AutoEvaluator:
    def __init__(self, x_train: np.ndarray, y_train: np.ndarray, x_val: np.ndarray, y_val: np.ndarray,
        ...
        if enable_window_slicing:
            ...
            self.x_train, self.y_train, self.train_sample_origin = self._create_slices(x_train, y_train,
                                                                                       window_stride, window_size)
            self.x_val, self.y_val, self.val_sample_origin = self._create_slices(x_val, y_val,
                                                                                 window_stride, window_size)
    ...
    def evaluate(self):
        ...
        for model in all_models:
            ...
            p = model.predict(self.x_train, np.argmax(self.y_train, axis=1), self.x_val, self.y_val,
