	...
	* payment_method
	...
	def toprinterargs(self, **kwargs):
		...
		for i in range(total_cnt):
			...
			subitemnames[i].value = b''#sub_item_str.encode('big5')
			itemnames[i].value = b''#self.orders[i].title[:].encode('big5')
			itemcounts_list.append(1)#self.orders[i].amount)
			item_price = 10#self.orders[i].price
