...
var ESASkyJSView = widgets.DOMWidgetView.extend({
	...
	render: function () {
		...
		div.innerHTML = "<iframe id=" + this.modelId + " width='100%' height='800' style='border: none;' src='https://skyint.esac.esa.int?hide_welcome=true&hide_sci_switch=true&hide_banner_info=true'</iframe>";
	...
	handle_custom_message: function (msg) {
		...
		if(this.oldMsgIdToFront != msg.msgId){
			if(this.modelId == currActiveId){
				...
				document.getElementById(this.modelId).contentWindow.postMessage(msg, 'https://skyint.esac.esa.int')
			}
		}
	}
