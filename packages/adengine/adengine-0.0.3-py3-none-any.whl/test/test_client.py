...
from adengine.client import Client, ActivityTypes
...
def server():
    cmd = [
        ...
        '--activity_types_config', os.path.join(TEST_DATA, 'activity_types.json')
    ]
...
def test_client_normal(client):
    ...
    task_ids = [client.put(activity, password, ActivityTypes.SMALL) for activity, password in tasks]
    sleep_delta = 1
    while len(task_ids):
...
def test_client_errors(client):
    ...
    task_ids = {client.put(activity, password, ActivityTypes.SMALL): error_type for activity, password, error_type in tasks}
    sleep_delta = 1
    while len(task_ids):
        time.sleep(sleep_delta)
        for task_id in tuple(task_ids):
...
def test_queue_is_full(client):
    ...
    with pytest.raises(QueueIsFull):
        for i in range(QUEUE_SIZE * 2):
            tasks.append(client.put(activity, password, ActivityTypes.SMALL))
...
def test_task_not_completed(client):
    activity, password = read_file_binary(os.path.join(TEST_DATA, 'with_password.pka')), '123'
    task_id = client.put(activity, password, ActivityTypes.SMALL)
    with pytest.raises(TaskNotCompleted):
        client.get(task_id)


def extract_data(activity: bytes, password: str or None, activity_type: ActivityTypes) -> ActivityData:
    client = Client(UNIX_SOCKET)
    task_id = client.put(activity, password, activity_type)
    while True:
        time.sleep(1)
        try:
            result = client.get(task_id)
...
def test_process_safe():
    ...
    with mp.Pool(WORKERS_NUM) as pool:
        result = pool.starmap(extract_data, itertools.repeat((activity, password, ActivityTypes.SMALL), tasks_num))
...
@pytest.mark.parametrize(
    ['activity', 'password', 'activity_type', 'total_percentage'],
    [
        pytest.param(
...
def test_net_stabilization_delay(client, activity, password, activity_type, total_percentage):
    task_id = client.put(activity, password, activity_type)
    activity_data = None
