...
QUEUE_SIZE = 10
WORKERS_NUM = 2
EXTRACT_TIMEOUT = datetime.timedelta(seconds=5)
RESULT_TTL = datetime.timedelta(hours=1)


@pytest.fixture(scope='module')
def launch_engine():
    scheduler = Scheduler(QUEUE_SIZE, WORKERS_NUM, USE_VIRTUAL_DISPLAY, RESULT_TTL)
    scheduler.start()
    yield scheduler
    scheduler.stop()
...
def test_engine_normal(launch_engine):
    ...
    task_ids = [launch_engine.put(activity, password, EXTRACT_TIMEOUT) for activity, password in tasks]
    sleep_delta = 1
    while len(task_ids):
...
def test_engine_errors(launch_engine):
    ...
    task_ids = {launch_engine.put(activity, password, EXTRACT_TIMEOUT): error_type for
...
def test_queue_is_full(launch_engine):
    ...
    with pytest.raises(QueueIsFull):
        for i in range(QUEUE_SIZE * 2):
            tasks.append(launch_engine.put(activity, password, EXTRACT_TIMEOUT))
...
def test_task_not_completed(launch_engine):
    activity, password = read_file_binary(os.path.join(TEST_DATA, 'with_password.pka')), '123'
    task_id = launch_engine.put(activity, password, EXTRACT_TIMEOUT)
...
def test_result_ttl():
    activity, password = read_file_binary(os.path.join(TEST_DATA, 'with_password.pka')), '123'
    result_ttl = EXTRACT_TIMEOUT * 2
    workers_num = 1

    with Scheduler(QUEUE_SIZE, workers_num, USE_VIRTUAL_DISPLAY, result_ttl) as scheduler:
        old_task_id = scheduler.put(activity, password, EXTRACT_TIMEOUT)
        time.sleep(result_ttl.total_seconds())
        young_task_id = scheduler.put(activity, password, EXTRACT_TIMEOUT)
...
class Client(threading.Thread):
    ...
    def run(self):
        ...
        running_tasks = [self._scheduler.put(activity, password, EXTRACT_TIMEOUT) for activity, password in self._tasks]
...
def test_pt_session_restart():
    ...
    with Scheduler(
        ...
        running_tasks = [scheduler.put(activity, password, EXTRACT_TIMEOUT) for i in range(tasks_num)]
...
@pytest.mark.parametrize(
    ['activity', 'password', 'extract_timeout', 'net_stabilization_delay', 'total_percentage'],
    [
        pytest.param(
...
def test_net_stabilization_delay(launch_engine, activity, password, extract_timeout,
                                 net_stabilization_delay, total_percentage):
    task_id = launch_engine.put(activity, password, extract_timeout, net_stabilization_delay)
    activity_data = None
