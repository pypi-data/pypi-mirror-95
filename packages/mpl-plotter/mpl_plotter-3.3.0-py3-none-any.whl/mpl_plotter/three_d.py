...
class plot:
    ...
    def method_camera(self):
        if not isinstance(self.azim, type(None)):
            self.ax.view_init(azim=self.azim)
        if not isinstance(self.elev, type(None)):
            self.ax.view_init(azim=self.elev)
...
class line(plot, dim_01):
    ...
    def __init__(self,
                 ...
                 # Figure, axis
                 fig=None, ax=None, figsize=None, shape_and_position=111, azim=None, elev=None,
                 # Setup
