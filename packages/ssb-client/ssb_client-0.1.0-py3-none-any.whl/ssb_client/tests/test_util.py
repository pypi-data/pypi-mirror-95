...
from ssb_client.util import load_ssb_secret, ConfigException
...
def test_load_secret():
    with patch('ssb_client.util.open', mock_open(read_data=CONFIG_FILE), create=True):
...
def test_load_exception():
    with pytest.raises(ConfigException):
        with patch('ssb_client.util.open', mock_open(read_data=CONFIG_FILE_INVALID), create=True):
            load_ssb_secret()
