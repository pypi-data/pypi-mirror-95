...
def find_new_moon(d):       # used in doublepage
    ...
    if PreviousNewMoon != None and PreviousFullMoon != None:
        ...
        #t4 = ts.utc(PreviousFullMoon + datetime.timedelta(days=28))
        #t5 = ts.utc(PreviousFullMoon + datetime.timedelta(days=30))
        #t, y = almanac.find_discrete(t4, t5, almanac.moon_phases(eph))
        #for i in range(len(t)):
        #    if y[i] == 2:       # 2 = Full Moon
        #        NextFullMoon = t[i].utc_datetime()
