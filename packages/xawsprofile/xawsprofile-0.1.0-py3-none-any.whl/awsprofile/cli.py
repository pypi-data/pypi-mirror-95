...
from awsprofile import aws
from awsprofile import __version__ as VERSION
...
def set_profile(name, alias):
    ...
    if profile_name is None:
        sys.stderr.write("profile {name} or alias not found\n")
        exit(1)
    if alias:
        aws.save_alias(profile_name, alias)
    sys.stdout.write("export AWS_PROFILE=\"{profile_name}\"".format(profile_name=profile_name))
