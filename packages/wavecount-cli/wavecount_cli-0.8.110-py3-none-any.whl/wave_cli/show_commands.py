    ...
    if dev_list and len(dev_list):
        if serial_number:
            for device in dev_list:
                ...
                prim_key = dev_info['symmetricKeys']['primaryKey'] if _.has(dev_info, 'symmetricKeys.primaryKey') else ' '
                click.secho()
            ...
            for device in dev_list:
                ...
                if len(firm_upd_state) < len(t_upd + ' ' * (block_len - len(t_upd) + 10)):
                    ...
                    firm_upd_state = ' {0}..'.format(firm_upd_state[0:(len(t_upd + ' ' * (block_len - len(t_upd) + 10)) - 3)])
