...
{	// fill in a WAVEFORMATEXTENSIBLE structure
	...
	play_buf_bytes = pwfex->Format.nAvgBytesPerSec * quisk_sound_state.latency_millisecs / 1000 * 2;
	dev->play_buf_size = play_buf_bytes / (dev->num_channels * dev->sample_bytes);
...
{	// Open the soundcard for capture.  Return non-zero for error.
	...
	dscbd.dwFlags = 0;
	dscbd.dwBufferBytes = play_buf_bytes;
	...
	if (quisk_sound_state.verbose_sound) {
		...
		QuiskPrintf("Created capture buffer size %d bytes for device %s, descr %s\n",
			play_buf_bytes, dev->name, dev->stream_description);
	}
	return 0;
}
...
{	// Open the soundcard for playback.  Return non-zero for error.
	...
	dsbdesc.dwBufferBytes = play_buf_bytes;
...
{ // cSamples can be NULL to discard samples
	...
	bytes_per_frame = dev->num_channels * dev->sample_bytes;
        ...
        if (readPos >= dev->dataPos)
	        ...
	        bytes = readPos - dev->dataPos + play_buf_bytes;
...
#endif
	dev->dataPos += bytes;
        if (dev->dataPos >= play_buf_bytes)
                dev->dataPos -= play_buf_bytes;
	nSamples = 0;
		...
		complex double * cSamples, int report_latency, double volume)
{
	...
	if (unavail < 0)
		unavail += play_buf_bytes;
        dev->play_delay = dev->dataPos - playPos;       // includes unavail
        if (dev->play_delay < 0)
                dev->play_delay += play_buf_bytes;
	...
	case 1: // Normal run state
		...
		frames = (play_buf_bytes - dev->play_delay - unavail) / bytes_per_frame;
		// Check for underrun
