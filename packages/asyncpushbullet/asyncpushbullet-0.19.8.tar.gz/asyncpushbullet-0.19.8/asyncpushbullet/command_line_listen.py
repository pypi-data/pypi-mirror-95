...
class ExecutableAction(Action):
    ...
    def __init__(self, path_to_executable, args_for_exec=(), loop: asyncio.BaseEventLoop = None, timeout=None):
        super().__init__()
        self.path_to_executable:Path = Path(path_to_executable).absolute()
    ...
    async def on_push(self, push: dict, pb: AsyncPushbullet):
        ...
        async def _on_proc_loop():
            # Launch process
            try:
                print("Launching process", str(self.path_to_executable), *self.args_for_exec)
                proc = await asyncio.create_subprocess_exec(str(self.path_to_executable),
                                                            *self.args_for_exec,
    ...
    def transform_push_to_stdin_data(self, push: dict) -> bytes:
        """Convert the push to something the process can read, json in this case"""
                ...
                async def _hndl_resp(_resp):
                    ...
                    if _resp.get("type") == "file":
                        ...
                        await pb.async_push_file(file_name=file_name,
                                                 ...
                                                 title=title,
                                                 body=body)
                    elif _resp.get("file") is not None:
                        file_path = _resp.get("file")
                        await pb.async_push_file_simple(file_path=file_path,use_transfer_sh=False,quiet=True,title=title, body=body)
