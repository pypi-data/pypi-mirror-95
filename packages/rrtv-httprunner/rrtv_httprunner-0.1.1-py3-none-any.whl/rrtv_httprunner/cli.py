...
def main_hrun_alias():
    """ command alias
        hrun = rrtv_httprunner run
    """
    if len(sys.argv) == 2:
        if sys.argv[1] in ["-V", "--version"]:
            # hrun -V
            sys.argv = ["rrtv_httprunner", "-V"]
        elif sys.argv[1] in ["-h", "--help"]:
            pytest.main(["-h"])
            sys.exit(0)
        else:
...
def main_har2case_alias():
    """ command alias
        har2case = rrtv_httprunner har2case
    """
