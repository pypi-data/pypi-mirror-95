...
class RustBackendGenerated:
    ...
    def answer_card(self, *, card_id: int, current_state: pb.SchedulingState, new_state: pb.SchedulingState, rating: pb.AnswerCardIn.Rating.V, answered_at: int, milliseconds_taken: int) -> pb.Empty:
        input = pb.AnswerCardIn(card_id=card_id, current_state=current_state, new_state=new_state, rating=rating, answered_at=answered_at, milliseconds_taken=milliseconds_taken)
        output = pb.Empty()
    ...
    def graphs(self, *, search: str, days: int) -> bytes:
        ...
        return self._run_command(36, input)

    def get_graph_preferences(self) -> bytes:
        input = pb.Empty()
        return self._run_command(37, input)
