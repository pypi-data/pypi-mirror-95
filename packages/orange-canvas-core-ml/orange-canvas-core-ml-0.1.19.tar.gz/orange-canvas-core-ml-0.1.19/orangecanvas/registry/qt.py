...
from AnyQt.QtGui import QStandardItemModel, QStandardItem, QColor, QBrush, QDesktopServices
from AnyQt.QtCore import QObject, Qt, QUrl
...
def whats_this_helper(desc, include_more_link=False):
    ...
    def get_url():
        ...
        try:
            name = re.findall(pattern, desc.name)[0]
        except:
            name = desc.name
        try:
            category = re.findall(pattern, desc.category)[0]
        except:
    ...
    if len(inputs) != 0:
        template.append(f'<b>输入:</b>')
        template.append('<ul>')
        for input in inputs:
            template.append(f'<li>{input.name}</li>')
        template.append('</ul>')

    if len(outputs) != 0:
        template.append(f'<b>输出:</b>')
        template.append('<ul>')
        for output in outputs:
            template.append(f'<li>{output.name}</li>')
        template.append('</ul>')

    if help_url and include_more_link:
        template.append(f"<a href='{get_url()}'>查看文档</a>")
