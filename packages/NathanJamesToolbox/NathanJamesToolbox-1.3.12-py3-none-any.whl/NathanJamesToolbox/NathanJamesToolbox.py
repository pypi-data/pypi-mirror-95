...
class mySQLToolbox:
    ...
    def readQuery(self, qry, _type):
        updated_path = self.sqlPath
        if _type == 'f':
            if isinstance(self.sqlPath, str):
                updated_path = self.sqlPath[:-1] if self.sqlPath[-1] == '/' else self.sqlPath
            filename = os.path.join(updated_path, qry)
