...
from central import CentralApi, constants as static, config, Cache, log, Response, cleaner
from central.utils import Utils
utils = Utils()
# from centralcli.constants import ClientArgs, static.StatusOptions, static.SortOptions
...
def get_format(
    ...
    """Return users desired display format based on command line flag"""
    if do_json:
        return "json"
    elif do_yaml:
        return "yaml"
...
def aps(
    ...
    status: static.StatusOptions = typer.Option(None, metavar="[up|down]", help="Filter by device status"),
    state: static.StatusOptions = typer.Option(None, hidden=True),  # alias for status
    ...
    sort_by: static.SortOptions = typer.Option(None, "--sort"),
...
@app.command()
def clients(
    filter: static.ClientArgs = typer.Argument('all', case_sensitive=False, ),
    ...
    outfile: Path = typer.Option(None, help="Output to file (and terminal)", writable=True),
    update_cache: bool = typer.Option(False, "-U", hidden=True),  # Force Update of cache for testing
    sort_by: static.SortOptions = typer.Option(None, "--sort", hidden=True,),  # TODO Unhide after implemented
    reverse: static.SortOptions = typer.Option(None, "-r", hidden=True,),  # TODO Unhide after implemented
