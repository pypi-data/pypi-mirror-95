...
from central import CentralApi, constants, caas, log, config, Cache, Response
from cli_levels import clishow, clido
from central.utils import Utils
utils = Utils()


# from centralcli.constants import (TemplateLevel1, RefreshWhat, arg_to_what)
...
@app.command()
def template(operation: constants.TemplateLevel1 = typer.Argument(...),
...
@app.command()
def refresh(what: constants.RefreshWhat = typer.Argument(...),
            debug: bool = typer.Option(False, "--debug", envvar="ARUBACLI_DEBUG", help="Enable Additional Debug Logging",
                                       callback=debug_callback),
            account: str = typer.Option("central_info",
                                        envvar="ARUBACLI_ACCOUNT",
...
if __name__ == "__main__":
    if len(sys.argv) > 2 and sys.argv[1] == 'show':
        sys.argv[2] = constants.arg_to_what(sys.argv[2])
