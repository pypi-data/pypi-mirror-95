...
class Utils:
    ...
    def output(
        ...
        if sanitize and raw_data and all(isinstance(x, dict) for x in raw_data):
            ...
            # outdata = [{k: d[k] if not d[k].startswith('Aruba9004_81_E') else f"PommoreGW" for k in d} for d in raw_data]
            gwnum = 1
            from random import randint as ri
            for d in raw_data.copy():
                for k in d:
                    if isinstance(d[k], str):
                        if d[k] and d[k].startswith('Aruba9004_81_E'):
                            d[k] = f"PommoreGW{gwnum}"
                            gwnum += 1
                        if d[k] and d[k] == "J9773A-80:C1:6E:CD:32:40":
                            d[k] = "SNANTX-IDF2-SW1"
                        if d[k] and d[k] == "38:17:c3:c1:51:36":
                            d[k] = "SNANTX-2NE-AP43"
                        if d[k] and "barry ln" in d[k].lower():
                            d[k] = f"{ri(1, 9999)} Privacy Ln"
                        if k in redact:
                            d[k] = "--redacted--"
                        if d[k]:
                            d[k] = d[k].replace("12610", "200").replace("5402", "101").replace("100 Main St", "1 ConsolePi Dr. ").replace("4629", "1203")
