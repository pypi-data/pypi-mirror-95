...
class download:
	...
	def getData(self):
		...
		if self.mode != False:
			...
			if url != '':
				if self.alias != 'mt':
					title = _k._uq.unquote(_k._c(r'(\/manga\/|\/komik\/|\/mg\/)(.*)' if self.alias != 'mb' else r'(\/baca-komik-)(.*)-bahasa-indonesia').findall(url)[0][1].translate(str.maketrans({'-':' ', '_':' ','/':''}))).title()
				else:
					title = _k._uq.unquote(_k._c(r'<h1 class="comics-title">(.*?)</h1>').findall(_k._g(url).text)[0])
