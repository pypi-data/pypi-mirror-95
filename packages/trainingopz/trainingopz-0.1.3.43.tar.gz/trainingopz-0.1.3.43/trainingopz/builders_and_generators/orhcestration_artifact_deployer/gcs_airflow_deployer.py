...
class GCSAirflowDeployer(AirflowDeployer):
    ...
    def _deploy(self, artifact):
        ...
        try:
            self._logger.info(f"INFO>>{datetime.now().__str__()}:Begin method {__name__}._deploy for {artifact};")
            ...
            self._logger.info(f"INFO>>{datetime.now().__str__()}:Begin method {__name__}._deploy;")
        except:
