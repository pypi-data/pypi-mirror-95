...
def get_data(table, where_columns_list = 'None', where_value_list = 'None', multiple=False,
		order_by_list = 'None', special_where = False):
	"""
	...
	if where_columns_list == 'None':
		if not special_where:
			query = generate_query(db_components['table_name'],'SELECT',db_components['columns'],order_by_list=order_by_list)
		else:
			query = special_generate_query(db_components['table_name'],'SELECT',db_components['columns'],order_by_list=order_by_list)
		# print ("query = ",query)
		...
		if not multiple:
			if not special_where:
				query = generate_query(db_components['table_name'],'SELECT',db_components['columns'],where_columns_list,order_by_list=order_by_list)
			else:
				query = special_generate_query(db_components['table_name'],'SELECT',db_components['columns'], where_columns_list, order_by_list=order_by_list)
			...
			multi_query = conditional_generate_query(db_components['table_name'],'SELECT',db_components['columns'],where_columns_list,
				len(where_value_list),order_by_list=order_by_list)
