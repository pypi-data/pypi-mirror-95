	...
	def check_parameter(self, parameter=None, name="parameter", empty_value=None):
		...
		if ":" in name:
			...
			if param_format not in formats:
				return self.error(f"Incorrect parameter [{name}:{param_format}] format, correct format(s): {Files.Array(path=False, array=formats).string(joiner=', ')}.")
	...
	class ResponseObject(object):
		def __init__(self, 
			...
			if isinstance(self.message, str):
				self.message = Formats.String(self.message).capitalized_word()
			if isinstance(self.error, str):
				self.error = Formats.String(self.error).capitalized_word()
			while True:
				if isinstance(self.message, str) and len(self.message) >= 1 and self.message[len(self.message)-1] in [" ", ".", ","]:
					self.message = self.message[:-1]
				elif isinstance(self.error, str) and len(self.error) >= 1 and self.error[len(self.error)-1] in [" ", ".", ","]:
					self.error = self.error[:-1]
				elif isinstance(self.error, str) and len(self.error) >= len("Error: ") and self.error[:len("Error: ")] in ["Error: "]:
					self.error = Formats.String(self.error[len("Error: "):]).capitalized_word()
				else: break
			
			# add dot.
			if isinstance(self.message, str)  and len(self.message) > 0 and self.message[len(self.message)-1] not in ["!", "?"]:
				self.message += "."
			if isinstance(self.error, str)  and len(self.error) > 0 and self.error[len(self.error)-1] not in ["!", "?"]:
				self.error += "."
