...
class SpikeLogger():
    ...
    def log_input(self, input_data:dict, input_type:str):
        ...
        ------------
         ...
         input_type: string
            The type of input sent by the user. The type must be "raw_input", that is, the data sent directly by the user, not necessarily those used by the model to make the prediction.
        """
        ...
        log = LogInput(log_type="logInput", user=self.user, data=input_data, input_type=input_type)
    ...
    def log_info(self):
        ...
        if not log_msg:
            raise ValueError("log_msg is empty")
        log = logInfo(log_type="logInfo", user=self.user, project=self.project, date=log_date)
        self.logger.log_struct(log.dict())
    ...
    def log_prediction(self, 
        ...
        pred = Prediction(prediction=prediction, time=process_time, prediction_date=log_date)
        log_in = LogInput(log_type="LogInput", user=self.user, data=input_features, input_type=input_type)
    ...
    def log_prediction_error(self, 
        ...
        log_date = str(datetime.now())
        log_in = LogInput(log_type="logInput", user=self.user, data=input_data, input_type=input_type)
        log = LogPredictionError(log_type="logPredictionError", msg=msg_error, model_id=model_id, date=log_date, input=log_in)
        self.logger.log_struct(log.dict())
        pass
