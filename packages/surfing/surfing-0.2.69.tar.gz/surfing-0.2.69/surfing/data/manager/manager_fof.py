...
from ..view.basic_models import FOFManually, HedgeFundNAV, FOFInvestorPosition, FOFEstimateFee, FOFEstimateInterest, FOFTransitMoney
...
class FOFDataManager(metaclass=Singleton):
    ...
    @staticmethod
    def _do_calc_v_net_value(nav: Union[pd.Series, pd.DataFrame, float], unit_total, acc_nav: pd.Series, init_water_line: pd.Series, trades, incentive_fee_ratio: Union[pd.Series, float], decimals: Union[pd.Series, float]) -> pd.Series:
        # 盈利
        excess_ret = acc_nav - init_water_line
        # 盈 或 亏
        earn_con = (excess_ret > 0).astype('int')
        # 费
        pay_mng_fee = ((trades * excess_ret * earn_con).round(2) * incentive_fee_ratio).round(2)
        # 净值
        mv = (nav * unit_total).round(2)
        mv -= pay_mng_fee
        v_nav = (mv / unit_total).round(decimals)
        return v_nav

    @staticmethod
    def _calc_virtual_net_value(fund_list):
        '''计算虚拟净值'''
        ...
        asset_allocation = asset_allocation.loc[(asset_allocation.fof_id == FOFDataManager._FOF_ID) & (asset_allocation.asset_type == HoldingAssetType.HEDGE), :]
        ...
        for row in nav.itertuples():
            ...
            if df.empty:
                continue
            # 盈利
            excess_ret = row.acc_unit_value - df.water_line
            # 盈 或 亏
            earn_con = (excess_ret > 0).astype('int')
            # 费
            pay_mng_fee = ((df.unit_total * excess_ret * earn_con).round(2) * incentive_fee_ratio[row.fund_id]).round(2)
            # 净值
            mv = (row.net_asset_value * df.unit_total.sum()).round(2)
            mv -= pay_mng_fee.sum()
            v_nav = (mv / df.unit_total.sum()).round(v_nav_decimals[row.fund_id])
    ...
    def init(self):
        ...
        v_nav_calcd = self._calc_virtual_net_value(hedge_fund_list)
        # 最后再ffill
    ...
    def calc_fof_nav(self, is_from_excel=False):
        if is_from_excel:
    ...
    def dump_fof_nav_and_pos_to_db(self) -> bool:
        ...
        # df = FOFDataManager().get_investor_return(fof_id=FOFDataManager._FOF_ID)
        # if df is not None:
        ...
        #         # merge on all columns
        #         df = df.drop(columns=['total_rr', 'v_nav']).reset_index().round(4).merge(now_df.round(4), how='left', indicator=True, validate='one_to_one')
        #         df = df[df._merge == 'left_only'].drop(columns=['_merge'])
        #     print(df)
        #     if not df.empty:
    ...
    def get_investor_return(fof_id: str, *, investor_id_list: Tuple[str] = (), start_date: Optional[datetime.date] = None, end_date: Optional[datetime.date] = None) -> Optional[pd.DataFrame]:
        ...
        for investor_id, datas in investor_mvs.items():
            ...
            avg_nav_cost = datas.init_mv.sum() / total_share
            investor_returns[investor_id] = {'datetime': datas.start_date.array[-1], 'fof_id': fof_id, 'v_nav': avg_v_nav, 'cost_nav': avg_nav_cost, 'total_rr': total_rr, 'amount': datas.amount.sum(), 'shares': total_share}
...
if __name__ == "__main__":
    ...
    fof_dm.calc_all()
    # fof_dm.calc_fof_nav()
    # fof_dm.dump_fof_nav_and_pos_to_db()
