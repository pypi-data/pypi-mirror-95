...
def print_results(events: list, cases: set, dry: bool):
    ...
    stats = "\n\t".join(
        "{} (#{}) - {} - {}".format(case_subject, case, customer, group) for customer, group, case, case_subject in cases
    )
...
def autocreate(
    ...
    putter = partial(
        ...
        create_case=_create_case if not dry else lambda *a, **kw: {"id": "DRY ID"},
    ...
    for customer, group, event in group_data(data, group_by):
        ...
        cases.add((customer["shortName"], group, case["id"], case["subject"]))
