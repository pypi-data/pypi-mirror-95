...
from th2_check2_recon.reconcommon import SharedGroupPermission


class RuleConfiguration:

    def __init__(self, name, enabled, shared_group_permission, match_timeout, match_timeout_offset_ns,
                 configuration) -> None:
        self.name = str(name)
        self.enabled = True if enabled.lower() == 'true' else False
        self.shared_group_permission = SharedGroupPermission[shared_group_permission]
