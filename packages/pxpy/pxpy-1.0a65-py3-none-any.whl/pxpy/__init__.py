...
class GraphBase(ctypes.Structure):
...
class Graph0(ctypes.Structure):
	...
	def get_type(self):
		return type("Graph", (GraphBase,), {"_fields_": self.get_fields()})
		
	def cast(self):
		self.__class__ = self.get_type()
		return self

class STGraphBase(ctypes.Structure):
...
class STGraph0(ctypes.Structure):
	...
	def get_type(self):
		return type("STGraph", (STGraphBase,), {"_fields_": self.get_fields()})
		
	def cast(self):
		self.__class__ = self.get_type()
		return self

class ModelBase(ctypes.Structure):
...
#		return self.__marginals[int(self.__woff[self.graph.nodes + e] + x0 * self.states[x1] + x1)]
	...
	def export_onnx(self,fname,probmodel=False):
...
#		ONNX_OP_LIST.append(DBGn)
	...
	def export_onnx_marginal_inference(self,filename):
		self.export_onnx(filename,True)
		
	def export_onnx_MAP_inference(self,filename):
		self.export_onnx(filename,False)
...
class Model0(ctypes.Structure):
	...
	def get_type(self):
		return type("Model", (ModelBase,), {"_fields_": self.get_fields()})
...
def create_model(weights, graph, states, stats = StatisticsType.overcomplete, vtype=default_vtype, itype=default_itype):
	...
	model_type = m0.get_type()
	return model_type(weights, graph, states, stats, m0.itype, m0.vtype)

def create_graph(G, nodes=None, target=None, vtype=default_vtype, itype=default_itype):
...
def train(data=None, graph=None, mode = ModelType.mrf, inference = InferenceType.belief_propagation, iters = 1000, infer_iters=None, seed = 0, k = 3, input_model = None, initial_stepsize = 0.1, opt_progress_hook = 0, progress_hook = 0, regularization = 0, proximal_operator = 0, T = 1, threshold = 0.0, lambda_proximal = 0, lambda_regularization = 0, shared_states = False, optimizer = Optimizer.accelerated_proximal_gradient, zero_init=False, clique_size = 3, states = None, vtype=default_vtype, itype=default_itype):
	...
	if graph is not None and (not (isinstance(graph, GraphType) or isinstance(graph, GraphBase))):
		raise TypeError('ERROR: graph must be an instance of GraphType enum or Graph class')

	if input_model is not None and not isinstance(input_model, ModelBase):
