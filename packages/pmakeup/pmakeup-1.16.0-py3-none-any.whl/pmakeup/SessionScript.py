...
class SessionScript(abc.ABC):
    ...
    @show_on_help.add_command('files')
    def find_regex_match_in_file(self, pattern: str, *p: path, encoding: str = "utf8", flags: Union[int, re.RegexFlag] = 0) -> \
    re.Match[AnyStr]:
        """
    ...
    @show_on_help.add_command("files")
    def zip_files(self, files: Iterable[path], base_dir: str, zip_name: path, zip_format: str, create_folder_in_zip_file: bool = False, folder_name_in_zip_file: str = None):
        """
        ...
        with self.create_temp_directory_with("zip_files") as folder_abspath:
            if create_folder_in_zip_file:
                zip_root = self.create_empty_directory(self.abs_path(folder_abspath, folder_name_in_zip_file))
            else:
                zip_root = folder_abspath
            for f in files:
                abs_f = self.abs_path(f)
                dst = os.path.join(zip_root, self.get_relative_path_wrt(abs_f, reference=self.abs_path(base_dir)))
