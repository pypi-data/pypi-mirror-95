...
class CustomTable:
    ...
    def _add_linespacing(self, s12):
        ...
        for i in range(self.settings['linespacing']):
            for colNum in range(len(self.data)):
                result += f'{s12} ' + ' ' * \
                    (self.colMaxLen[colNum] if self.colMaxLen[colNum] <= (
                        self.settings['cellwrap'] - 2) else (self.settings['cellwrap'] - 2)) + ' '
            result += s12 + '\n'
        return result
    ...
    def _get(self, rq):
        ...
        for rowNum in range(self.row):
            colNum = 0
            # the top bar
            result += self._get_struct(self.style[4], self.style[5],
                                       self.style[7], self.style[6], colNum)
            # the value
        ...
        colNum = 0
        result += self._get_struct(self.style[8], self.style[9],
                                   self.style[11], self.style[10], colNum)
        return result
