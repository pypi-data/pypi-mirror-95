...
class HTTPServer:
    """
    HTTP Server that supports Asynchronous Coroutines.
    Note: HTTPServer Only supports python 3.5 and above. 
    ...
    async def _serve_one_client(self, reader, writer):
        ...
        if addr not in self.serving:
            self.serving.add(addr)
        else:
            writer.close()
            return
