...
#                    sitemapTags = soup.find_all("url")
                ...
                url_list.append([parsed_uri.netloc, str(round(len(request.content)/1024,2)) + "KB", len(sitemapTags), average_time, str(request.history)])
                #print(request.status_code)
                #click.echo(url + '\t %s \t %s' % (len(request.content), request.history))
            #except:
            #    print(url, '\x1b[1;30;41m' + 'Website Timedout' + '\x1b[0m')
        table = PrettyTable(['URL', 'Size', 'Pages', 'Average time', 'Status Code'])
        for rec in url_list:
            table.add_row(rec)
        table.align["URL"]="l"
        table.align["Size"]="r"
