...
class ClusterBuilder:
    ...
    def __init__(self, albert, embedder, batch_size, all_comments, all_chunks):
        self.albert = albert
            ...
            for cluster_head in full_cluster[question]:
                ...
                for item in full_cluster[question][cluster_head]:
                    try:
                        ...
                        chunk = chunk_cid[0]
                        comment = self.all_comments[chunk_cid[1]]['text']  # comment text
                        comment_date = self.all_comments[chunk_cid[1]]['created_date'] \
                                       if 'created_date' in self.all_comments[chunk_cid[1]] \
                                       else datetime.now().isoformat() # comment created date
                        comment_uuid = self.all_comments[chunk_cid[1]]['comment_uuid']
                        user_uuid = self.all_comments[chunk_cid[1]]['user_uuid']
                        user_anonymous = self.all_comments[chunk_cid[1]]['user_anonymous']
                        cluster_elements.append([item[0], chunk, comment, comment_date, comment_uuid, user_uuid, user_anonymous])
                    except Exception as e: 
                        # logger.error(str(e))
                        continue
