    ...
    def save_cluster(self, cluster_dic, forum_id, client_id):
        ...
        for q_text in cluster_dic:
            ...
            if self.db.exists('Question', text=q_text, forum_id=forum_id):
                ...
                node_types = ['question', 'cluster_head', 'cluster_element', 'chunk', 'comment', 'user']
                for r in results:
            ...
            for h_text in cluster_dic[q_text]:
                ...
                for el in cluster_dic[q_text][h_text]['cluster_elements']:
                    ...
                    try:
                        ...
                        create_datetime = datetime.strptime(el[3], "%Y-%m-%dT%H:%M:%S")
                    ...
                    user_node = self.db.create_or_update_node('User', uid=el[5] if el[5] else 'Unknown', anonymous=el[6])

                    if do_delete and user_node.id in to_delete_ids:
                        to_delete_ids.remove(user_node.id)
                        to_keep['User'].add(user_node.id)
                        
                    c_node.user.connect(user_node)
...
def convert_nested_dict_to_list(obj, values):
    return [_convert_nested_dict_to_list(o, values.copy()) for o in obj.values()]
