...
bapy = Package(filename=__file__, _version='0.22.10')
...
def nested_del(data: Any, keys: Optional[Union[Iterable, Callable]] = None,
               values: Optional[Union[Iterable, Callable]] = None, nested: bool = False) -> dict:
    """
    ...
    def exclude(d: Any, _k: bool = True):
        if isinstance(keys if _k else values, Iterable):
            for _item in (_keys if _k else _values):
                if d == _item:
                    return True
        elif isinstance(keys if _k else values, Callable):
            return keys(d) if _k else values(d)
        elif keys is None and _k:
            return False
        elif values is None and not _k:
            return False
    ...
    with warnings.catch_warnings(record=False):
        ...
        if isinstance(data, tuple) and hasattr(data, '_asdict'):
            return nested_del(data._asdict(), keys, values, nested)
        elif dataclasses.is_dataclass(data):
            return nested_del(dataclasses.asdict(data), keys, values, nested)
            ...
            return nested_del({key: value for key, value in (obj_cls_value | obj_value).items() if key in cls_attrs},
                              keys, values, nested)
        elif isinstance(data, (MutableMapping, list, set, tuple)):
            mutable = isinstance(data, MutableMapping)
            rv = dict() if mutable else list()
            for item in data:
                value = data.get(item) if mutable else item
                excl_values = (values and exclude(value, False)) if mutable else (keys and exclude(item))
                if not ((keys and exclude(item, )) or excl_values):
                    ...
                    if isinstance(value, (MutableMapping, list, set, tuple)) \
                        ...
                        if nested or (not nested and nested_count < len(data)):
                            value = nested_del(value, keys, values, nested)
                            nested_count += 1
                    else:
                        if hasattr(value, 'text'):
                            value = getattr(value, 'text')
                    rv.update({item: _copy(value)}) if mutable else rv.append(_copy(value))
            if not mutable:
                type(data)(rv)
            return rv
...
def nested_del1(data: Any, keys: Optional[Union[Iterable, Callable]] = None,
               values: Optional[Union[Iterable, Callable]] = None, nested: bool = False) -> dict:
    """
    Nested delete keys or values in dict.
