...
class Enum(enum.Enum):
    ...
    def new(cls, name, attrs: Iterable, doc: str = str(), start: Any = 1,
        ...
        #     ('DEBUG', 10)
        #     >>> Path.PathMode.__doc__
        #     'Group/Other File/Dir PathMode'
        #     >>> Path.PathMode['FILE']
        #     <PathMode.FILE: 511>
        #     >>> Path.PathOption.FILES
...
Caller = NamedTuple('Caller', args=dict, code=list, coro=bool, file=Path, func=Union[Callable, property], funcname=str,
                    glob=dict, loc=dict, lineno=int, module=str, name=str, package=str, qual=str, vars=CallerVars)


def caller(index: int = 2) -> Caller:
    """
    1: self, 2: caller, 3: caller back

    coro only for index > 1 (qual will be None to avoid recursion).
    ...
    """
    index_init = index
    def _has(d: dict, name: str) -> tuple[Optional[bool], Optional[Union[Callable, property]], Optional[str]]:
        _coro = None
        _prop = None
        _qual = None
        f = d.get(name)
        # if f:
        #     _qual = getattr(f, f'__qualname__', _qual)
        for i in ['self', 'cls']:
            if o := d.get(i):
                if name in dir(o):
                    if i == 'self':
                        ...
                        if isinstance(getattr(cls, name), property):
                            ...
                            with warnings.catch_warnings(record=True):
                                ...
                                if index_init != 1:
                                    _coro = inspect.isawaitable(getattr(o, name))
                                try:
                                    if _coro:
                                        _qual = eval(f'o.{name}.__qualname__')
                    ...
                    if cls:
                        f = getattr(cls, name)
                        break
        return _coro if _coro else any(
            [inspect.isawaitable(f), inspect.iscoroutinefunction(f), inspect.isasyncgenfunction(f),
             inspect.isasyncgen(f)]), f, getattr(f, f'__qualname__', _qual)

    def include(data, var, cls: bool = False):
        # Class property : dir: True, dict: True.
        # Obj property : dir: True, dict: False.
        value = data.get(var)
        if not var.startswith('__') and not inspect.ismodule(value):
            if inspect.isroutine(value) or (cls and isinstance(value, property)):
                return False
            return value
        return None

    def _vars(data, cls: bool = False):
        d = data.copy()
        rv = dict()
        for var in d:
            if value := include(d, var, cls):
                if hasattr(value, '__dict__'):
                    obj_cls_value = dict()
                    obj_value = vars(value)
                    if inspect.isclass(value):
                        cls = True
                    else:
                        if (obj_cls := getattr(value, '__class__')) and hasattr(value, '__dict__'):
                            obj_cls_dict = vars(obj_cls)
                            # cls_dir = [cls_var for cls_var in cls_dict if include(cls_dict, cls_var, True)]
                            # Add class attributes not in obj attributes.
                            obj_cls_value = {v: obj_cls_dict.get(v) for v in obj_cls_dict
                                             if v not in obj_value and include(obj_cls_dict, v, True)}

                            # obj_value |= {v: cls_dict.get(v) for v in cls_dir if v not in obj_value}
                    value = _vars(obj_cls_value | obj_value, cls)
                rv |= {var: value}
        return rv
    ...
    loc = frame.frame.f_locals.copy()
    glob = frame.frame.f_globals.copy()
    # coro, func, qual = _has(glob | loc, frame.function)
    coro = False
    func = None
    qual = None
    file = Path(frame.filename)
    return Caller(del_key(args), frame.code_context, coro, file, func, frame.function, glob, loc, frame.lineno,
                  inspect.getmodulename(file.text), glob.get('__name__', str()), glob.get('__package__', str()), qual,
                  CallerVars(_vars(glob), _vars(loc)))
...
class Env(AsDict):
    ...
    def __post_init__(self):
        ...
        # self._Env.read_env((Path.cwd() / Path.PathSuffix.ENV.value).text, verbose=True, override=True)
...
class SemEnum(EnumDict):
    ...
    async def sem(self, func: Callable, /, *args, priority: Priority = Priority.LOW,
        ...
        def _args(data: dict):
            _data = data.copy()
            _vars = ['ip', ]
            return ', '.join([f'{_var}: {_val}' for _var in _vars if (_val := _data.get(_var))])
        ...
        try:
            ...
            if 'asyncio' in frame_1.filename:
                frame = frame_2
                if obj.back_back_locals:
                    loc = obj.back_back_locals
            if 'asyncio' in frame_2.filename:
                frame = frame_1
                if obj.back_locals:
                    loc = obj.back_locals
            arg = _args(loc)
            frame_path = Path(frame.filename)
            called = f'({frame_path.parent.name}/{frame_path.name} | {frame.function}: {frame.lineno} | {arg=}) '
        ...
        if priority == Priority.HIGH and self.value[Priority.LOW].locked():
            ...
            using = priority.name
        add = f'{using=}', f'{called=}'
        await _log.awarning(*await self.text(func, priority, *add, suffix='Waiting'))
        async with sem:
...
def del_key(data: dict, key: Iterable = ('self', 'cls',), exception: bool = False) -> dict:
    d = data.copy()
...
class Mongo:
    ...
    def col(self, name: str = None) -> MongoCol:
        icc(name, caller().coro, aioloop(), aioloopid(), tasks())
        task = asyncio.current_task().get_name()
        self.log.debug('Start', f'{name=}', f'{task=}')
        name = name if name else self.col_name
        name = f'{self.col_test_prefix}_{name}' if self.test or Mongo._db_name not in name else name
        uri, kwargs = self.url
        func = MongoClientMotor if caller().coro else MongoClientPy
        rv = func(uri, **kwargs).get_database(self.db_name).get_collection(name)
        self.log.debug(f'End', f'{name=}', f'{task=}')
        return rv
    ...
    #     ic(type(self._db[aio]))
    #     return self._db[aio]

    # def db_aio(self):
    #     self._db_aio = MongoClientMotor(uri, **kwargs).get_database(self.db_name)
    ...
    async def find_one_and_update(self, _id: Any, value: MongoFieldValue, col: str = None):
        print()
        icc(_id, aioloop(), aioloopid(), tasks())
