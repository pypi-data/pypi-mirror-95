...
IPaddress = Union[IPv4Address, ipaddress.IPv6Address]
...
def clsinfo(data: Any, rv_data: Optional[AttributeKind] = None, start: str = None) -> \
        Optional[Union[dict[str, Attribute], tuple]]:
    """
    Class attrs info.

    Examples:
        >>> from bapy import Path, AttributeKind, clsinfo
        >>> clsinfo(Path)
        >>> clsinfo(Path, AttributeKind.DATA)
        >>> clsinfo(Path, AttributeKind.DATA, '__')

    Args:
        data: class or object.
        rv_data: return list of data attrs.
        start: startswith not to include.

    Returns:
        Optional[Union[dict[str, Attribute], list]]:
    """
    data = data if inspect.isclass(data) else getattr(data, '__class__')
    if data:
        if rv_data:
            return tuple([item.name for item in inspect.classify_class_attrs(data)
                          if item.kind == rv_data.value and (not item.name.startswith(start) if start else True)])
        return {item.name: Attribute(item.defining_class, AttributeKind[iter_split(item.kind)[0].upper()],
                                     item.object) for item in inspect.classify_class_attrs(data)}
...
class AsDictLarge:
    ...
    def dict_public(self) -> dict[str, Any]:
        """
...
class Url(furl.furl):
    ...
    def lumenbiomics(http: bool = False, username: str = None, password: str = None,
        ...
        g = PathSuffix.GIT.value if suffix else str()
...
class EnumsDict(enums.Enum):

    @classmethod
    def asdict(cls) -> dict:
        return {key: value._value_ for key, value in cls.members.items()}
...
class EnumsDictLower(EnumsDict):
    """Auto Enum class with name lower as the value"""

    # noinspection PyUnusedLocal
    @staticmethod
    def enum_generate_next_value(
            name: str, start: Optional[enums.T], count: int, values: typing.List[enums.T]
    ) -> enums.T:
...
class Path(Pathlib, pathlib.PurePosixPath, AsDict, DataProxy):
    ...
    def templates(self) -> dict:
        ...
        if self.is_dir():
            return {item.stem: self.J2(item) for item in self.glob(f'*.{PathSuffix[self.J2.__name__].value}')}
        return dict()
    ...
    def scan(self, option: PathOption = PathOption.FILES,
             output: PathOutput = PathOutput.BOX, suffix: PathSuffix = PathSuffix.NO_SUFFIX,
    ...
    def scripts(self) -> list:
        ...
        if scripts.is_dir():
            ...
            if self.scripts_suffix:
                return [item for item in scripts.iterdir() if item.suffix.endswith(PathSuffix.BASH.value)
                        or item.suffix.endswith(PathSuffix.SH.value)]
...
def caller(index: int = 2, _vars: bool = True, _glob: bool = False, _loc: bool = False) -> Caller:
    """
    ...
    def exclude(d):
        ...
        if any([inspect.getmodule(Callable) == typing, inspect.ismodule(d), inspect.isroutine(d), isinstance(d, (property, )), inspect.isclass(d),
                isinstance(d, Caller), isinstance(d, CallerVars)]):
            return True
    ...
    # coro, func_obj, qual = None, None, None
    ...
    return Caller(del_key(args), frame.code_context, coro, file, func_obj, frame.function, glob if _glob else dict(),
                  ...
                  CallerVars(nested_del(glob, exclude, exclude), nested_del(loc, exclude, exclude))
...
def path_installed(filename: Path) -> PathInstalled:
    ...
    return PathInstalled(filename, home, installed, log_dir, log_dir.touch_add(f'{name}{PathSuffix.LOG.value}'),
                         home.touch_add(PathSuffix.MONGO.value), name, p, prefix, project, repo, work)
...
class Env(AsDict):
    ...
    def __post_init__(self):
        ...
        # self._Env.read_env((Path.cwd() / PathSuffix.ENV.value).text, verbose=True, override=True)
...
class Ip(AsDictData):
    ...
    @property
    async def IPClass(self) -> Optional[IPaddress]:
        """
...
bapy = Package(filename=__file__, _version='0.22.6')
...
def nested_del(data: Any, keys: Optional[Union[Iterable, Callable]] = None,
               values: Optional[Union[Iterable, Callable]] = None) -> dict:
    """
    Nested delete keys or values in dict.

    Args:
        data: dict.
        keys: Iterable of callable which returns True if delete.
        values: Iterable of callable which returns True if delete.
    ...
    def exclude(d: Any, _k: bool = True):
        if isinstance(keys if _k else values, Iterable):
            for item in (_keys if _k else _values):
                if d == item:
                    return True
        elif isinstance(keys if _k else values, Callable):
            return keys(d)

    def update(d: MutableMapping) -> dict:
        _rv = dict()

        for _k, _v in d.items():
            if not ((inspect.getmodule(Callable) == typing) or (keys and exclude(_k)) or (values and exclude(_v, False))):
                if isinstance(_v, (MutableMapping, list, set, tuple)) or (dataclasses.is_dataclass(data))  \
                        or (not inspect.isclass(data) and hasattr(data, '__dict__') and hasattr(data, '__class__')):
                    _rv[_k] = nested_del(_v, keys, values)
                else:
                    _rv[_k] = _copy(_v)
        return _rv

    if keys and isinstance(keys, Iterable):
        _keys = set(iter_split(keys))

    if values and isinstance(values, Iterable):
        _values = set(iter_split(values))

    cls = getattr(data, '__class__')
    if isinstance(data, MutableMapping):
        return update(data)
    elif isinstance(data, tuple) and hasattr(data, '_asdict'):
        return update(data._asdict())
    elif dataclasses.is_dataclass(data):
        ic(data)
        return update(dataclasses.asdict(data))
    elif hasattr(data, '__dict__') and cls:
        obj_value = vars(data)
        obj_cls_dict = vars(cls)
        # Add class attributes not in obj attributes.
        obj_cls_value = {k: v for k, v in obj_cls_dict.items() if k not in obj_value}
        cls_attrs = clsinfo(cls, AttributeKind.DATA, '__')
        return update({key: value for key, value in (obj_cls_value | obj_value).items() if key in cls_attrs})
    elif isinstance(data, MutableMapping):
        return update(data)
    elif isinstance(data, (list, set, tuple)):
        rv = list()
        for key in data:
            if not (keys and exclude(key)):
                rv.append(nested_del(key, keys, values))
        return type(data)(rv)
...
class NmapStateEnum(EnumsDictLower):
    CLOSED = enums.auto()
    FILTERED = enums.auto()
    OPEN = enums.auto()
...
@dataclasses.dataclass
class NmapServiceScript(AsDictData):
    name: Union[list, str]
    value: dict


@dataclasses.dataclass
class NmapPort(AsDictData):
    number: int
    state: NmapStateEnum
    proto: NmapProtoEnum
...
@dataclasses.dataclass
class NmapCodec(AsDictData):
    parsed: dict[str, dict]


@dataclasses.dataclass
class NmapCommand(AsDictData):
    ...
    async def run(self, ip: str = str(), name: str = str(), port: NmapCommandPortTyping = None) -> NmapParse:
        self.ip = ip if ip else self.ip
        if self.samples:
        ...
        self.parse = NmapParse.parse(ip=self.ip, name=name, nmaprun=nmaprun, rc=cmd_out.rc, stderr=cmd_out.stderr)
        return self.parse
