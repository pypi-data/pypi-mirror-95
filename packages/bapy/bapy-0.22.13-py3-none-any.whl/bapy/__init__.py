...
class Obj:
    ...
    def __post_init__(self):
        if self.data is None:
            self.data = Obj
        # self.__exclude_attr__ = self.clsinspect(AttributeKind.PROPERTY, Obj)
        self.__exclude_copy__ = [environs.Env, LogR, Path, git.Remote, socket.AddressFamily, socket.SocketKind,
                                 git.SymbolicReference, git.config.GitConfigParser, ]
    ...
    def asdict(self, count: int = 1, defaults: bool = False) -> Any:
        if self.enumcls:
            ...
            self.data = self.defaults_dataclass
        elif self.dataclass_instance:
            self.data = self.defaults_dataclass if defaults else \
                self.defaults_dataclass | vars(self.data) | self.properties
        elif self.dict_cls:
            self.data = self.defaults_cls
        elif self.dict_instance:
            ic(self.data, self.defaults_cls)
            self.data = self.defaults_cls if defaults else \
                self.defaults_cls | vars(self.data) | self.properties
            ic(self.data)

        if self.mlst:
            rv = dict() if (mm := isinstance(self.data, MutableMapping)) else list()
            for key in self.data:
                if (inc := self.include(key, self.data if mm else None)) is not None:
                    value = inc[1]
                    if self.depth is None or count < self.depth:
                        value = self.new(value).asdict(count=count + 1)
                    rv.update({key: value}) if mm else rv.append(value)
            return dict_sort(rv) if mm else type(self.data)(rv)
        if (inc := self.include(self.data)) is not None:
            if inspect.isgetsetdescriptor(inc[1]) or self.coro:
                return inc[1]
    ...
    def clsinspect(self, kind: Optional[AttributeKind] = None,
                   cls: Any = None) -> Optional[Union[dict[str, Attribute], list]]:
        """
        Class attrs info.
        ...
        """
        cls = self.getcls if cls is None else cls
        if kind:
            return sorted([item.name for item in inspect.classify_class_attrs(cls) if item.kind == kind.value])
        return {item.name: Attribute(item.defining_class, AttributeKind[iter_split(item.kind)[0].upper()],
                                     item.object) for item in inspect.classify_class_attrs(cls)}
    ...
    @runwarning
    def coro(self) -> bool:
        return any([inspect.isawaitable(self.data), inspect.iscoroutinefunction(self.data),
    ...
    def enuminstance(self) -> bool:
        ...
        return not self.cls and any([issubclass(self.getcls, cls) for cls in classes]) \
               and any([isinstance(self.data, cls) for cls in classes]) and hasattr(self.data, 'name') \
               and hasattr(self.data, 'value')
    ...
    def include(self, key: Any = None, data: Any = None) -> Optional[tuple]:
        def exclude_mro(name: str = '__exclude_attr__', v: Any = None) -> Union[bool, set, str]:
            excl = {attr for item in [*self.mro, Obj(), ]
                    for attr in getattr(item, '__exclude_copy__' if v is not None else name, list())}
            if v is not None:
                if isinstance(v, tuple(excl)):
                    return repr(v)
                return v
            return excl

        def exclude(_data: Obj) -> Optional[bool]:
            d = self.new(_data)
            return any([d.getmodule == typing, d.module, d.routine, d.cls, d.start])

        if (not Obj(key).str or (key not in exclude_mro())) and not exclude(key):
            if data is not None:
                value = data.get(key) if self.new(data).mutablemapping else getattr(data, key)
                if not exclude(value):
                    return key, exclude_mro(v=value)
                return None
            return key, exclude_mro(v=key)
        return None
    ...
    def new(self, data: Any = None, /, **kwargs):
    ...
    def properties(self) -> dict:
        if self.prop:
            return {key: inc[1] for key in self.clsinspect(AttributeKind.PROPERTY)
                    if (inc := self.include(key, self.data)) is not None}
        return dict()
...
class AsDict:
    ...
    __exclude_attr__ = ['asdict', 'public', 'values', ]
...
class Url(furl.furl, AsDict):
    ...
    def lumenbiomics(http: bool = False, username: str = None, password: str = None,
                     repo: str = None, suffix: bool = False, stdout: bool = False):
        """
    ...
    @staticmethod
    @app.command(context_settings=context)
    def nference(stdout: bool = False):
        """
        Nferx Url.
    ...
    @staticmethod
    @app.command(context_settings=context)
    def nferxops(stdout: bool = False):
        """
        Nferx Url.
    ...
    @staticmethod
    @app.command(context_settings=context)
    def repository(stdout: bool = False):
        """
        Repository repo Url.
    ...
    @staticmethod
    @app.command(context_settings=context)
    def repotest(stdout: bool = False):
        """
        Repotest Url.
    ...
    @staticmethod
    @app.command(context_settings=context)
    def repositorytest(stdout: bool = False):
        """
    ...
    @staticmethod
    @app.command(context_settings=context)
    def wiki(space: str = group, number: int = int(), page: str = str(), stdout: bool = True):
        """
        Confluence Wiki Url.
...
class Path(Pathlib, pathlib.PurePosixPath, AsDict, DataProxy):
    ...
    def mkdir_add(self, name, mode=0o700, parents=True, exist_ok=True):
        """
    ...
    def touch_add(self, name, mode=0o600, exist_ok=True):
        """
...
def caller(index: int = 2, _vars: bool = True, _glob: bool = False, _loc: bool = False,
    ...
    with warnings.catch_warnings(record=False):
        ...
        return Caller(del_key(args), frame.code_context, coro, file, func_obj, frame.function,
                      glob if _glob else dict(),
                      loc if _loc else dict(), frame.lineno, inspect.getmodulename(file.text),
                      glob.get('__name__', str()),
                      glob.get('__package__', str()), qual, asyncio.current_task().get_name() if aioloop() else str(),
                      CallerVars(*[Obj(item, depth=depth, swith='_').asdict() for item in [glob, loc]])
...
bapy = Package(filename=__file__, _version='0.22.13')
