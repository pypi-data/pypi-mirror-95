...
class DataFrameSummary(object):
    ...
    def _get_pairplot(self,data):
        for i in data.columns:
             if (data[i].nunique()/data.shape[0])*100 > 20 : ##unique nos in a column are more than 20% then it is a continuous variable
                continuous.append(i)
             else:
                categorical.append(i)
        cn=data.drop(columns=categorical,axis=1)
