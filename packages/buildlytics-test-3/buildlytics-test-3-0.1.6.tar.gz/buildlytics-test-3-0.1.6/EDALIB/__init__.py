...
class DataFrameSummary(object):
    ...
    @classmethod
    def _get_pairplot(self,data=None,continuous=[],categorical=[]):
        self.data=data
        print("Hello!",continuous)
        if(self.data==None):
            return "Send the Data Please"
        for i in self.data.columns:
            if(self.data[i].nunique/data.shape[0])*100>20:
               continuous.append(i)
            else:
               categorical.append(i)
        cn=self.data.drop(columns=categorical,axis=1)
