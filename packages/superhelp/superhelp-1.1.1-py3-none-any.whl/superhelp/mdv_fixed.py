...
def col(text, c, bg=0, no_reset=0):
    """
    print col('foo', 124) -> red 'foo' on the terminal
    c = color, text the value to colorize """
    reset = reset_col
    if no_reset:
        reset = ""
    for _strt, _end, _col in (
        ...
        if _strt in text:
            ...
            text = text.replace(_end, uoff + col("", c, no_reset=1))

    text = "\033[38;5;%sm%text%text" % (c, text, reset)
    if bg:
        pass
        # text = col_bg(bg) + text
    return text
...
def rewrap(el, text, ind, pref):
    ...
    # forgot why I didn'text use textwrap from the beginning. In case there is a
...
def split_blocks(text_block, width, cols, part_fmter=None):
...
class AnsiPrinter(Treeprocessor):
    ...
    def run(self, doc):
        ...
        def formatter(el, out, hierarchy_level=0, pref="", parent=None):
            """
            Main recursion.
            ...
            if el.tag == "blockquote":
                for el1 in el.getchildren():
                    iout = []
                    formatter(el1, iout, hierarchy_level + 2, parent=el)
                    pr = col(bquote_pref, H1)
                    sp = " " * (hierarchy_level + 2)
                    for l in iout:
            ...
            if el.tag == "hr":
                return out.append(tags.hr("", hierarchy_level=hierarchy_level))
...
#                     t = html_parser.unescape(t)  ## GPS
                ...
                ind = left_indent * hierarchy_level
                ...
                if pref:
                    ...
                    h = globals()["H%s" % (((hierarchy_level - 2) % 5) + 1)]
                    if pref == list_pref:
                        pref = col(pref, h)
                ...
                if (
                    ...
                    _out = tag_fmt_func(t.lstrip(), hierarchy_level=hierarchy_level)
                    out[-1] += _out
                else:
                    out.append(tag_fmt_func(t, hierarchy_level=hierarchy_level))
                ...
                if el.tag == "li":
                    ...
                    for nested in "ul", "ol":
                        if childs and childs[-1].tag == nested:
                            ...
                            formatter(ul, out, hierarchy_level + 1, parent=el)
                return
            ...
            if el.tag == "table":
                ...
                def fmt(cell, parent):
                    ...
                    _cell = []
                    formatter(cell, out=_cell, hierarchy_level=0, parent=parent)
                    return "\n".join(_cell)
            ...
            for c in el:
                ...
                formatter(c, out, hierarchy_level + 1, parent=el)
