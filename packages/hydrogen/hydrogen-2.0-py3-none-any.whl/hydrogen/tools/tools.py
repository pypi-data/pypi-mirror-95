...
def plot_flowDf(flowDf, df_freq, showBool, freqBool, ax, type):    #showBool is just a bool to show the plot or not Necessary to account for calls from GUI or script.
    ...
    """
    ax.cla()
    plot1=flowDf.reset_index().plot(x='index',y='flow', ax=ax, legend=True, color='blue', label='flow')
    ...
    if freqBool:
        plot2 = df_freq.reset_index().plot(x='index', y=type, ax=ax, legend=True, secondary_y=True, mark_right=False, alpha=0.5, color='lightgreen', label='frequency distribution')
    ...
    xticks = []
    xticksLabels = []
    for i in range(0, int(len(flowDf)/3600) + 1):
