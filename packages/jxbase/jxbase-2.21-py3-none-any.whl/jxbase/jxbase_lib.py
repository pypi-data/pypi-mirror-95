...
def is_windows():
	"""
	说明：判断操作系统是否 windows，是的话返回 True, 否则返回 False
...
def is_linux():
	"""
	说明：判断操作系统是否 linux，是的话返回 True, 否则返回 False
...
def convert_df_type(df, xtype=float, column_arr=None):
	...
	for col in column_arr:
		try:
			...
			for row in row_arr:
				if is_number(all_df.loc[row,col]) == False:
...
def magic_square_4k_2_df(n = 6):
	...
	temp_df = df1.loc[0:k,0:k]
	temp_df = temp_df.copy(deep = True)
	df1.loc[0:k,0:k] = df3.loc[0:k,0:k]
	df3.loc[0:k,0:k] = temp_df

	temp_df = df1.loc[k,k:(k + k)]
	temp_df = temp_df.copy(deep = True)
	df1.loc[k,k:(k + k)] = df3.loc[k,k:(k + k)]
	df3.loc[k,k:(k + k)] = temp_df

	temp_df = df1.loc[(k + 1):(k + 1 + k),0:k]
	temp_df = temp_df.copy(deep = True)
	df1.loc[(k + 1):(k + 1 + k),0:k] = df3.loc[(k + 1):(k + 1 + k),0:k]
	df3.loc[(k + 1):(k + 1 + k),0:k] = temp_df

	if k > 1:
		temp_df = df2.loc[:,2:(k + 1)]
		temp_df = temp_df.copy(deep = True)
		df2.loc[:,2:(k + 1)] = df4.loc[:,2:(k + 1)]
		df4.loc[:,2:(k + 1)] = temp_df
