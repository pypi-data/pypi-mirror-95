...
class DynDiGraph(nx.DiGraph):
    ...
    **Edges:**
    ...
    def nodes_iter(self, t=None, data=False):
        ...
        if t is not None:
            if not data:
                return [n for n, d in list(self.degree(t=t).items()) if d > 0]
            else:
                return {n: self._node[n] for n, d in list(self.degree(t=t).items()) if d > 0}
    ...
    def nodes(self, t=None, data=False):
        ...
        """
        if data:
            return [(k, v) for k, v in list(self.nodes_iter(t=t, data=data).items())]
        else:
