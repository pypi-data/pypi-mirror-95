	...
	if not response.success:
		if response.error == f"Path {alias}:{path} does not exist.":
			if install:
				...
				output = syst3m.utils.__execute_script__(f'ssh {alias} " curl https://raw.githubusercontent.com/vandenberghinc/{ALIAS}/{ALIAS}/master/requirements/installer.remote | bash " ')
