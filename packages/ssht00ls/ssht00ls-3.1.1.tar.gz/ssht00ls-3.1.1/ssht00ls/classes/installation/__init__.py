...
class Installation(object):
	...
	def install(self, 
		...
		# success.
		return r3sponse.success_response(f"Successfully installed ssh for user [{username}].")
	...
	def check_installed(self, 
		...
		fp = Formats.FilePath(f"{home}.ssh/")
		if not fp.exists():
			return r3sponse.error_response(f"Required ssh configuration file [{fp.path}] for user [{username}] is not installed.")
		else:
			fp.permission.set(permission=700, sudo=sudo)
			fp.ownership.set(owner=username, group=None, sudo=sudo)
		...
		# success.
		return r3sponse.success_response(f"SSH is successfully installed for user [{username}].")
