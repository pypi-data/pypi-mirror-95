...
def sync():
	...
	for path in dir.paths(recursive=False):
		removed = False
		fp = Formats.FilePath(gfp.name(path=path).replace("\\","/"))
		if not fp.exists(): 
...
class Daemon(syst3m.objects.Thread):
	...
	def run(self):
		...
		while True:
			...
			if status == "none" or not os.path.exists(self.path): 
			...
			if do:
				...
				if "*running*" in status:
					cache.set(id=self.cache_path, group="daemons", data=f"*running* (timestamp={Formats.Date().seconds_timestamp})")
	...
	def delete(self, path, remote=False, subpath=True):
		...
		if remote:
			...
			response = self.utils.execute(
				...
				get_output=True,)
			if not response["success"]: return response
			#response = ssh.utils.test_ssh_path(alias=self.alias, path=path)
			#if response.error != None and f"{path} does not exist" not in response.error:
			#	return r3sponse.error_response(f"Failed to delete {self.alias}:{path}, error: {response.error}")
			if response.output != "":
				return r3sponse.error_response(f"Failed to delete {self.alias}:{str_id}, error: {output}")
			return r3sponse.success_response(f"Successfully deleted {self.alias}:{str_id}")
		...
		# remote to local.
			...
			if not response["success"]: return response
			if response.output != "":
				return r3sponse.error_response(f"Failed to delete {str_id}, error: {output}")
			return r3sponse.success_response(f"Successfully deleted {str_id}")
	def index(self, short=False):
		...
		for path, mtime in index.items():
			...
			try:
				...
				return r3sponse.error_response(f"Should not happen(#278363). (subpath: {subpath}), (path: {path}), (index: {json.dumps(index,indent=4)}), (remote index: {json.dumps(remote_index,indent=4)}).")
		for path, mtime in remote_index.items():
			...
			try:
				...
				return r3sponse.error_response(f"Should not happen(#739639). (subpath: {subpath}), (path: {path}), (index: {json.dumps(index,indent=4)}), (remote index: {json.dumps(remote_index,indent=4)}).")
	...
	def process_mismatches(self, clean_index, remote_clean_index, mismatches):
		...
		for subpath, info in mismatches.items():
			...
			if rmtime == None and lmtime == None: 
				...
				return r3sponse.error_response(f"No remote & local modification time present. (path: {subpath}), (rmtime: {rmtime}), (lmtime: {lmtime}), (last_rmtime: {last_rmtime}), (last_lmtime: {last_lmtime}), index: {json.dumps(clean_index,indent=4)}, remote index: {json.dumps(remote_clean_index,indent=4)}.")
			...
			# one missing.
				...
				# should not happen.
					...
					self.set_last_index(clean_index) ; self.set_last_index(remote_clean_index, remote=True)
					return r3sponse.error_response(f"Should not happen (#3243443). (path: {subpath}), (rmtime: {rmtime}), (lmtime: {lmtime}), (last_rmtime: {last_rmtime}), (last_lmtime: {last_lmtime}), index: {json.dumps(clean_index,indent=4)}, remote index: {json.dumps(remote_clean_index,indent=4)}.")
			...
			# both present.
				...
				# should not happen.
					...
					self.set_last_index(clean_index) ; self.set_last_index(remote_clean_index, remote=True)
					return r3sponse.error_response(f"Unable to compare rmtime: {rmtime} & lmtime: {lmtime}. (path: {subpath}), (rmtime: {rmtime}), (lmtime: {lmtime}), (last_rmtime: {last_rmtime}), (last_lmtime: {last_lmtime}), index: {json.dumps(clean_index,indent=4)}, remote index: {json.dumps(remote_clean_index,indent=4)}.")

			# exceptions.
			else:
				self.set_last_index(clean_index) ; self.set_last_index(remote_clean_index, remote=True)
				return r3sponse.error_response(f"Should not happen (#407294). (path: {subpath}), (rmtime: {rmtime}), (lmtime: {lmtime}), (last_rmtime: {last_rmtime}), (last_lmtime: {last_lmtime}), index: {json.dumps(clean_index,indent=4)}, remote index: {json.dumps(remote_clean_index,indent=4)}.")
			...
			if directory and local_to_remote != None and remote_to_local != None and "delete" not in options and not empty_directory:
				...
				if local_to_remote and remote_to_local:
					self.set_last_index(clean_index) ; self.set_last_index(remote_clean_index, remote=True)
					return r3sponse.error_response(f"Can not synchronize both remote to local & local to remote (rmtime: {rmtime}) (lmtime: {lmtime}), (last_rmtime: {last_rmtime}) (last_lmtime: {last_lmtime}), index: {json.dumps(clean_index,indent=4)}, remote index: {json.dumps(remote_clean_index,indent=4)}.")
	...
	def __sync__(self, multiprocessing=True, max_batch_size=150):
		...
		if multiprocessing:
			...
			if depths != {}:
				return r3sponse.error_response(f"Should not happen(#234938). (depths: {depths}), (updates: {json.dumps(updates,indent=4)}).")
		...
		if not response["success"]: return response
			...
			return r3sponse.error_response(f"Failed to synchronize [{self.alias}:{self.remote}] & [{self.path}], mismatches: {json.dumps(response.mismatches, indent=4)}].")
	...
	def synchronized(self): # does not work.
		...
		if "receiving incremental file list" not in output or "total size is" not in output or "bytes/sec" not in output:
			return r3sponse.error_response(f"Unkown rsync output: [{output}].")
		b = output.split("\n")
		try:
			output = output[len(b[0])+1:-(len(b[len(b)-1])+len(b[len(b)-2])+3)]
		except Exception as e:
			return r3sponse.error_response(f"Failed to clean rsync output: [{output}]. Error: {e}; ")
		print("OUTPUT:",output)
		return r3sponse.success_response(f"Successfully checked [{self.alias}:{self.remote}] & [{self.path}]", {
			"synchronized":output == "",
		})