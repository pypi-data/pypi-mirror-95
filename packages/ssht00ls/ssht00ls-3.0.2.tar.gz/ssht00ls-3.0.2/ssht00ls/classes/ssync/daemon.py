...
# stop a daemon.
def stop_daemon(path, timeout=60, sleeptime=1):
...
class Daemon(syst3m.objects.Thread):
	...
	def index(self, short=False):
		...
		# index.
		remote_response = self.ssync.index(path=self.path, alias=self.alias, checks=False, log_level=self.log_level)
	...
	def delete(self, path, remote=False, subpath=True):
		...
		if isinstance(path, str):
			...
			new = []
			for i in path: 
				i = self.utils.serialize_path(gfp.clean(path))
	...
	def sync(self, attempts=1, delay=3):
		...
		return response
		#
	def __sync__(self, multiprocessing=True, max_batch_size=100):
