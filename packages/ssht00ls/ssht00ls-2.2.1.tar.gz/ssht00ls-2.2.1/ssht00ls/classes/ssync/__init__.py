...
class SSync(object):
	...
	def index(self, path=None, alias=None, log_level=0, checks=True, accept_new_host_keys=True):
		...
		# local.
			...
			for path in Files.Directory(path=path).paths(recursive=True):
				id = path
				if os.path.isdir(path): id += " (d)"
				index[id] = Formats.FilePath(path).mtime(format="seconds")
	...
	def pull(self,
		...
		command = f"rsync -{Formats.Boolean(directory).convert(true='a', false='')}zq {alias}:{remote} {path} {exclude_str}{delete_str}"
	...
	def push(self,
		...
		for i in exclude: exclude_str += f"--exclude {i} "
		delete_str = Formats.Boolean(delete).convert(true="--delete", false="")
		command = f"rsync -{Formats.Boolean(directory).convert(true='a', false='')}zq {path} {alias}:{remote} {exclude_str}{delete_str}"
		...
		def sync(self):
			...
			if not response.success: return response
				...
				if self.log_level <= 0:
					...
					return r3sponse.success_response(f"Failed to synchronize [{self.alias}:{self.remote}] & [{self.path}], failed to update mismatches: {json.dumps(mismatches, indent=4)}].")
