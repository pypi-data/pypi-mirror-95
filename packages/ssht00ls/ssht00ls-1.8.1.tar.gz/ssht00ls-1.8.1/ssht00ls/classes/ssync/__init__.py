...
class SSync(object):
	...
	def unmount(self, 
		...
		#
	def index(self, path=None, alias=None):
	...
	def pull(self,
		...
		# tests.
		response = ssht00ls.agent.add(path=CONFIG["aliases"][alias]["key"], passphrase=passphrase)
		if not response["success"]: return response
		response = utils.test_ssh(alias=alias, accept_new_host_keys=accept_new_host_keys)
		if not response.success: return response
		...
		delete_str = utils.convert_bool(delete, true="--delete", false="")
