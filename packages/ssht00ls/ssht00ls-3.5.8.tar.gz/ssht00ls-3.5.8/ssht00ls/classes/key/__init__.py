...
class Key(object):
	...
	def generate(self, path=None, passphrase=None, comment=""):
		...
		# checks.
		directory = path
		if directory[len(directory)-1] != "/": return r3sponse.error("The path must be a directory and end with a [/].")
		response = r3sponse.check_parameters({
			"directory":directory,
		})
		if not response["success"]: return response
	...
	def check(self, username=None, public_keys=[], reversed=False):
		...
		ssh_dir = FilePath(f"{HOME_BASE}/{username}/.ssh/", check_existance=False)
		auth_keys = FilePath(f"{HOME_BASE}/{username}/.ssh/authorized_keys", check_existance=False)
