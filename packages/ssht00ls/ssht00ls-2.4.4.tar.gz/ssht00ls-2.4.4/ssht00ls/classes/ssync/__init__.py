...
class SSync(object):
	...
	class Daemon(syst3m.objects.Thread):
		...
		def index(self, short=False):
			...
			else: return self.process_index(index, remote_clean_index, mismatches)
		...
		def synchronized(self, clean_index, remote_clean_index, all_paths):
			...
			if self.log_level >= 5: print(f'synchronized: {synchronized} clean_index: {json.dumps(clean_index,indent=4)}, remote_clean_index: {json.dumps(remote_clean_index,indent=4)} mismatches: {json.dumps(mismatches, indent=4)}.')
		...
		def set_last_index(self, index, remote=False, depth=10):
