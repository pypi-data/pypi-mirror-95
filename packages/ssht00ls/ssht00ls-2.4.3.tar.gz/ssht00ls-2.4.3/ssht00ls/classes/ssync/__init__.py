...
class SSync(object):
	...
	def index(self, path=None, alias=None, log_level=0, checks=True, accept_new_host_keys=True):
		...
		# local.
			...
			for _, dirs, _ in os.walk(path):
				for dir in dirs:
					dir = gfp.clean(path=f"{path}/{dir}/")
					id = dir+" (d)"
					if id not in ids: 
						index[id] = gfp.mtime(path=dir, format="seconds")
						ids.append(id)
	...
	class Daemon(syst3m.objects.Thread):
		...
		def index(self, short=False):
			...
			if short != False:
				synchronized = self.synchronized(clean_index, remote_clean_index, all_paths)
			...
			else: return self.process_index(index, remote_clean_index, all_paths)
		...
		def process_index(self, index, remote_index, paths):
			...
			for path in paths:
				...
				try: last_lmtime = self.last_index[subpath]
				except KeyError: last_lmtime = None
				try: last_rmtime = self.last_remote_index[subpath]
				...
				# one missing.
					...
					if rmtime == None and lmtime != None and last_lmtime == None:
						if self.log_level >= 3:
							print(f"Local created a file (path: {path}) (rmtime: {rmtime}), (lmtime: {lmtime}), (last_lmtime: {last_lmtime}).")
						local_to_remote = True

					# local deleted a file.
					elif rmtime != None and lmtime == None and last_lmtime != None:
						if self.log_level >= 3:
							print(f"Local deleted a file (path: {path}) (rmtime: {rmtime}), (lmtime: {lmtime}), (last_lmtime: {last_lmtime}).")
						local_to_remote = True
					...
					# remote created a file.
						...
						if self.log_level >= 3:
							print(f"Remote created a file (path: {path}) (rmtime: {rmtime}), (lmtime: {lmtime}), (last_lmtime: {last_lmtime}).")
						remote_to_local = True

					# remote deleted a file.
					elif rmtime == None and lmtime != None and last_lmtime != None:
						if self.log_level >= 3:
							print(f"Remote deleted a file (path: {path}) (rmtime: {rmtime}), (lmtime: {lmtime}), (last_lmtime: {last_lmtime}).")
						remote_to_local = True
						options.append("delete")
		...
		def synchronized(self, clean_index, remote_clean_index, all_paths):
			...
			for subpath in all_paths:
				...
				if not synchronized: 
					...
					break
			if self.log_level >= 3:
				print(f'synchronized: {synchronized} clean_index: {json.dumps(clean_index,indent=4)}, remote_clean_index: {json.dumps(remote_clean_index,indent=4)} mismatches: {json.dumps(mismatches, indent=4)}.')
			return synchronized
