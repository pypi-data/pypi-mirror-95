...
def size(path, alias=None, log_level=0):
	if alias == None:
		...
		return execute(
			...
			error=f"Failed ti retrieve the size of {alias}:{path}.",
			log_level=log_level,)
...
def execute( 
	...
	try:
		...
		#if " exit status " in str(e.output):
		#	return r3sponse.error_response(f"Failed to execute command [{command}], (exit status: {str(e.output).split(' exit status ')[1].split('.')[0].split(' ')[0]}).")
		#else:
		#	return r3sponse.error_response(f"Failed to execute command [{command}].")
