...
# test ssh functions.
def test_ssh(alias=None, accept_new_host_keys=True,):
...
def test_ssh_path(alias=None, path=None, accept_new_host_keys=True,):
...
def test_ssh_dir(alias=None, path=None, accept_new_host_keys=True,):
	response = test_ssh(alias=alias, accept_new_host_keys=accept_new_host_keys)
	if not response.success: return response
	lpath = "\'"+path+"\'"
	output = syst3m.utils.__execute_script__(f"""ssh {alias} ''' python3 /usr/local/lib/ssht00ls/classes/utils/isdir.py --path '{path}' ''' """)
	if output.replace("\n","") == "True":
