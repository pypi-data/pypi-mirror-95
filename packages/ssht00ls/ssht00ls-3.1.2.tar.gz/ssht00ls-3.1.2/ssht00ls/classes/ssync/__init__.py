...
class SSync(object):
	...
	def mount(self, 
		...
		if mode in ["mount"] and not forced and os.path.exists(path):
		...
		if mode in ["mount"]:
			...
			response = ssh.utils.test_ssh_dir(path=remote, alias=alias)
			remote_exists, local_exists = True, os.path.exists(path)
			...
			if not remote_exists and not local_exists:
				return r3sponse.error_response(f"Both local and remote directories {path} & {alias}:{remote} do not exist.")
			if not local_exists:
				os.system(f"mkdir -p {path}")
				if not os.path.exists(path): 
					return r3sponse.error_response(f"Failed to create directory {path}.")
			if not remote_exists:
				response = ssh.utils.test_ssh_dir(path=remote, alias=alias, create=True)
				if not response["success"]: 
					return r3sponse.error_response(f"Failed to create directory {alias}:{remote}, error: {response['error']}")

		# start daemon.
		cache.set(id=cache_path, group="daemons", data=f"*running* (timestamp={Formats.Date().seconds_timestamp})")
		...
		# handler.
		if mode == "sync":
			return r3sponse.success_response(f"Successfully synchronized {alias}:{remote} to {path}.")
		else:
			return r3sponse.success_response(f"Successfully mounted {alias}:{remote} to {path}.")
	...
	def index(self, path=None, alias=None, log_level=0, checks=True, accept_new_host_keys=True):
		...
		if alias != None:
			...
			if checks:
				...
				if not response["success"]: return response
				response = ssh.utils.test_ssh_path(alias=alias, path=path, accept_new_host_keys=accept_new_host_keys)
				if not response["success"]: return response
	...
	def daemon(self, 
		...
		if serialized != {}:
			alias, remote, path, start, mode, log_level, sandbox, sleeptime = Files.Dictionary(path=False, dictionary=serialized).unpack({
				"alias":None,
				"remote":None,
				"path":None,
				"start":True,
