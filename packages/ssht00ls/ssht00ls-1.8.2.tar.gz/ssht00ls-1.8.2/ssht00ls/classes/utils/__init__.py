	...
	if not response.success:
		if response.error == f"Path {alias}:{path} does not exist.":
			if install:
				...
				output = syst3m.utils.__execute_script__(f'ssh {alias} " curl https://raw.githubusercontent.com/vandenberghinc/{ALIAS}/master/requirements/installer.remote | bash " ')
				response = test_ssht00ls(alias=alias, accept_new_host_keys=accept_new_host_keys, install=False)
				loader.stop(response.success)
				return response
			else:
