...
class SSync(object):
	...
	def index(self, path=None, alias=None, log_level=0, checks=True, accept_new_host_keys=True):
		...
		if alias != None:
			...
			# index.
			if log_level >= 6:
				print(f"""printf 'yes' | ssh {DEFAULT_SSH_OPTIONS} {alias} ' export IPINFO_API_KEY="{IPINFO_API_KEY}" && python3 /usr/local/lib/ssht00ls/classes/ssync/index.py --path {path} --json --non-interactive --no-checks ' """)
	...
	class Daemon(syst3m.objects.Thread):
		...
		def delete(self, path, remote=False, subpath=True):
			...
			if remote:
				os.system()
				response = self.utils.execute(
					command=f"""ssh {DEFAULT_SSH_OPTIONS} {self.alias} " printf 'y' | rm -fr '{path}' " """,
					error=f"Failed to delete {path}, error: {response.error}",
					message=f"Successfully deleted {self.alias}:{path}",)
				if not response["success"]: return response
		...
		def __sync__(self, multiprocessing=True, max_batch_size=50):
