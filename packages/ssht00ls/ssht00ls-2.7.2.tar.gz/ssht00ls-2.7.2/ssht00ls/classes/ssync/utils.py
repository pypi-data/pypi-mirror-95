...
def pull(
	...
	# options.
	exclude_str = Files.Array(array=exclude).string(joiner=" --exclude", start=" --exclude")
	include_str = Formats.Array(array=include).string(joiner=" --exclude ", start=" --exclude")
	delete_str = Formats.Boolean(delete).convert(true="--delete", false="")
	lremote = serialize_path(remote)
	lpath = serialize_path(path)
	_command_ = f"rsync -{Formats.Boolean(directory).convert(true='a', false='')}zqt {alias}:{lremote} {lpath} {exclude_str}{delete_str}"
...
def push(
	...
	lremote = serialize_path(remote)
	lpath = serialize_path(path)
	_command_ = f"rsync -{Formats.Boolean(directory).convert(true='a', false='')}zqt {lpath} {alias}:{lremote} {exclude_str}{delete_str}"
