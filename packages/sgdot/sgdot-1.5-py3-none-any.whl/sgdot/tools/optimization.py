...
def ga_optimization(grid,
    ...
    for generation in range(1, number_of_generations + 1):
        ...
        for index, row in population_df[population_df['generation']
                                        == generation].iterrows():
            population_df.loc[index, 'relative_fitness'] =\
                population_df.loc[index, 'fitness'] - lowest_fitness_in_generation
...
def sa_swap_step(grid, temperature, swap_option):
    ...
    This method is used as a step of the optimizing function
    'sa_optimization'.
...
def lp_optimization(grid,
    ...
    k_lu = lambda i, j: lp_lu_to_list(i, j)                 # noqa: E731
    i_lu = lambda k: lp_list_to_lu(k)[0]                    # noqa: E731
    j_lu = lambda k: lp_list_to_lu(k)[1]                    # noqa: E731
    ij = lambda i, j: i * number_of_nodes + j               # noqa: E731
...
def lp_list_to_lu(k):
    ...
    s = lambda k : int(k*(k+1)/2)               # noqa: E731
    s_inv = lambda k: (-1 + np.sqrt(1+8*k))/2   # noqa: E731
