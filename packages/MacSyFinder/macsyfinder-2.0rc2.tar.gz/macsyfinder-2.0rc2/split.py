...
def parse_args(args):
    ...
    parser.add_argument('genome_path',
                        help='Path to the genomes file (in fasta format), eg : path/to/file.fst or file.fst')
...
def main(args=None, log_level=None):
    """
     main entry point to macsysplit
    ...
    macsypy.init_logger(log_file=os.path.join(parsed_args.outdir, 'integron_split.out'),
    ...
    if not log_level:
        ...
        log_level = config.log_level - (10 * parsed_args.verbose) + (10 * parsed_args.quiet)
